...
/**
 * Represents a Cell within a {@link XWPFTable}. The Cell is the thing that
 * holds the actual content (paragraphs etc)
 */
public class XWPFTableCell implements IBody, ICell {
    ...
    static {
        // populate enum maps
        alignMap = new EnumMap<XWPFVertAlign, STVerticalJc.Enum>(
                XWPFVertAlign.class);
    ...
    public void setText(String text) {
        CTP ctP = (ctTc.sizeOfPArray() == 0) ? ctTc.addNewP() : ctTc
                .getPArray(0);
        ...
        if (e instanceof XWPFParagraph) {
            ...
            for (XWPFTableRow row : eTable.getRows()) {
                for (XWPFTableCell cell : row.getTableCells()) {
                    List<IBodyElement> localBodyElements = cell
                            .getBodyElements();
                    for (int i = 0; i < localBodyElements.size(); i++) {
                        boolean localIsLast = (i == localBodyElements.size() - 1) ? true
                                : false;
                        appendBodyElementText(text, localBodyElements.get(i),
                                localIsLast);
                    }
                }
            }
