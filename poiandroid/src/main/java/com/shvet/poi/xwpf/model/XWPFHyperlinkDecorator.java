...
/**
 * Decorator class for XWPFParagraph allowing to add hyperlinks found in
 * paragraph to its text.
 * <p/>
...
public class XWPFHyperlinkDecorator extends XWPFParagraphDecorator {
    ...
    public XWPFHyperlinkDecorator(XWPFParagraph prgrph,
                                  XWPFParagraphDecorator nextDecorator, boolean outputHyperlinkUrls) {
        ...
        for (CTHyperlink link : paragraph.getCTP().getHyperlinkArray()) {
            ...
            if (outputHyperlinkUrls
                    && paragraph.getDocument().getHyperlinkByID(link.getId()) != null) {
                hyperlinkText.append(" <"
                        + paragraph.getDocument()
                        .getHyperlinkByID(link.getId()).getURL() + ">");
            }
        }
    }
