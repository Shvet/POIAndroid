...
import com.shvet.poi.POIXMLDocument;
import com.shvet.poi.POIXMLDocumentPart;
import com.shvet.poi.POIXMLException;
import com.shvet.poi.POIXMLProperties;
import com.shvet.poi.POIXMLRelation;
...
import com.shvet.poi.openxml4j.opc.OPCPackage;
import com.shvet.poi.openxml4j.opc.PackagePart;
import com.shvet.poi.openxml4j.opc.PackagePartName;
import com.shvet.poi.openxml4j.opc.PackageRelationship;
import com.shvet.poi.openxml4j.opc.PackageRelationshipTypes;
import com.shvet.poi.openxml4j.opc.PackagingURIHelper;
import com.shvet.poi.openxml4j.opc.TargetMode;
import com.shvet.poi.poifs.crypt.HashAlgorithm;
import com.shvet.poi.util.IOUtils;
import com.shvet.poi.util.IdentifierManager;
import com.shvet.poi.util.Internal;
import com.shvet.poi.util.PackageHelper;
import com.shvet.poi.xwpf.model.XWPFHeaderFooterPolicy;
 ...
 * <p/>
 * <p>
 * This class tries to hide some of the complexity of the underlying file
 * format, but as it's not a mature and stable API yet, certain parts of the XML
 * structure come through. You'll therefore almost certainly need to refer to
 * the OOXML specifications from
 * http://www.ecma-international.org/publications/standards/Ecma-376.htm at some
 * point in your use.
 * </p>
 */
public class XWPFDocument extends POIXMLDocument implements Document, IBody {
     ...
     */
    private IdentifierManager drawingIdManager = new IdentifierManager(0L,
            4294967295L);
    /**
    ...
    protected void onDocumentRead() throws IOException {
        try {
            DocumentDocument doc = DocumentDocument.Factory
                    .parse(getPackagePart().getInputStream());
            ...
            for (POIXMLDocumentPart p : getRelations()) {
                ...
                if (relation.equals(XWPFRelation.STYLES.getRelation())) {
                    ...
                    this.styles.onDocumentRead();
                        ...
                        .equals(XWPFRelation.NUMBERING.getRelation())) {
                    ...
                    // TODO Create according XWPFComment class, extending
                    // POIXMLDocumentPart
                    CommentsDocument cmntdoc = CommentsDocument.Factory.parse(p
                            .getPackagePart().getInputStream());
                    for (CTComment ctcomment : cmntdoc.getComments()
                            .getCommentArray()) {
                ...
                } else if (relation.equals(XWPFRelation.GLOSSARY_DOCUMENT
                        .getRelation())) {
                    // We don't currently process the glossary itself
                    // Until we do, we do need to load the glossary child parts
                    // of it
                    for (POIXMLDocumentPart gp : p.getRelations()) {
                        // Trigger the onDocumentRead for all the child parts
                        // Otherwise we'll hit issues on Styles, Settings etc on
                        // save
                        try {
                            Method onDocumentRead = gp.getClass()
                                    .getDeclaredMethod("onDocumentRead");
    ...
    private void initHyperlinks() {
        ...
        try {
            Iterator<PackageRelationship> relIter = getPackagePart()
                    .getRelationshipsByType(
                            XWPFRelation.HYPERLINK.getRelation()).iterator();
            while (relIter.hasNext()) {
                PackageRelationship rel = relIter.next();
                hyperlinks.add(new XWPFHyperlink(rel.getId(), rel
    ...
    private void initFootnotes() throws XmlException, IOException {
        for (POIXMLDocumentPart p : getRelations()) {
            ...
            if (relation.equals(XWPFRelation.FOOTNOTE.getRelation())) {
                ...
                for (CTFtnEdn ctFtnEdn : endnotesDocument.getEndnotes()
                        .getEndnoteArray()) {
                    endnotes.put(ctFtnEdn.getId().intValue(), new XWPFFootnote(
                            this, ctFtnEdn));
                }
            }
        }
    ...
    protected void onDocumentCreate() {
        ...
        settings = (XWPFSettings) createRelationship(XWPFRelation.SETTINGS,
                XWPFFactory.getInstance());

        POIXMLProperties.ExtendedProperties expProps = getProperties()
    ...
    public CTStyles getStyle() throws XmlException, IOException {
        ...
        if (parts.length != 1) {
            throw new IllegalStateException(
                    "Expecting one Styles document part, but found "
                            + parts.length);
        }

        StylesDocument sd = StylesDocument.Factory.parse(parts[0]
                .getInputStream());
        return sd.getStyles();
    }
    ...
    public List<PackagePart> getAllEmbedds() throws OpenXML4JException {
        ...
        for (PackageRelationship rel : getPackagePart().getRelationshipsByType(
                OLE_OBJECT_REL_TYPE)) {
    ...
    /**
     * Finds that for example the 2nd entry in the body list is the 1st
     * paragraph
     */
    private int getBodyElementSpecificPos(int pos,
                                          List<? extends IBodyElement> list) {
    ...
    /**
     * get with the position of a table in the bodyelement array list the
     * position of this table in the table array list
     *
     ...
     * {@link org.apache.xmlbeans.XmlCursor.TokenType#START} tag of an
     * subelement of the documents body. When this method is done, the cursor
     * passed as parameter points to the
     * {@link org.apache.xmlbeans.XmlCursor.TokenType#END} of the newly inserted
     * paragraph.
     *
     * @param cursor
    ...
    /**
     * Get the position of the paragraph, within the list of all the body
     * elements.
     *
    ...
    protected void commit() throws IOException {
        ...
        xmlOptions.setSaveSyntheticDocumentElement(new QName(CTDocument1.type
                .getName().getNamespaceURI(), "document"));
    ...
    private int getRelationIndex(XWPFRelation relation) {
        ...
        int i = 1;
        for (Iterator<POIXMLDocumentPart> it = relations.iterator(); it
                .hasNext(); ) {
            POIXMLDocumentPart item = it.next();
            if (item.getPackageRelationship().getRelationshipType()
                    .equals(relation.getRelation())) {
                i++;
            }
        }
        return i;
    ...
    public XWPFParagraph createParagraph() {
        XWPFParagraph p = new XWPFParagraph(ctDocument.getBody().addNewP(),
                this);
        bodyElements.add(p);
        paragraphs.add(p);
        return p;
    ...
    public XWPFNumbering createNumbering() {
        if (numbering == null) {
            NumberingDocument numberingDoc = NumberingDocument.Factory
                    .newInstance();

            XWPFRelation relation = XWPFRelation.NUMBERING;
            int i = getRelationIndex(relation);

            XWPFNumbering wrapper = (XWPFNumbering) createRelationship(
                    relation, XWPFFactory.getInstance(), i);
    ...
    public XWPFStyles createStyles() {
        if (styles == null) {
            ...
            XWPFStyles wrapper = (XWPFStyles) createRelationship(relation,
    ...
    public XWPFFootnotes createFootnotes() {
        if (footnotes == null) {
            FootnotesDocument footnotesDoc = FootnotesDocument.Factory
                    .newInstance();

            XWPFRelation relation = XWPFRelation.FOOTNOTE;
            int i = getRelationIndex(relation);

            XWPFFootnotes wrapper = (XWPFFootnotes) createRelationship(
                    relation, XWPFFactory.getInstance(), i);
    ...
    public void setParagraph(XWPFParagraph paragraph, int pos) {
		...
		/*
		 * TODO update body element, update xwpf element, verify that incoming
		 * paragraph belongs to this document or if not, XML was copied properly
		 * (namespace-abbreviations, etc.)
		 */
    }
    ...
    public void createTOC() {
        ...
        for (XWPFParagraph par : paragraphs) {
            ...
            if (parStyle != null && parStyle.startsWith("Heading")) {
                try {
                    int level = Integer.parseInt(parStyle.substring("Heading"
                            .length()));
     ...
     * Verifies that the documentProtection tag in settings.xml file <br/>
     * specifies that the protection is enforced (w:enforcement="1") <br/>
     * and that the kind of protection is trackedChanges
     * (w:edit="trackedChanges")<br/>
     * <br/>
     * sample snippet from settings.xml
     * <p/>
     * <pre>
     ...
     * @param hashAlgo the hash algorithm - only md2, m5, sha1, sha256, sha384 and
     *                 sha512 are supported. if null, it will default default to sha1
     */
    public void enforceReadonlyProtection(String password,
                                          HashAlgorithm hashAlgo) {
        settings.setEnforcementEditValue(STDocProtect.READ_ONLY, password,
                hashAlgo);
    }
     ...
     *                 sha512 are supported. if null, it will default default to sha1
     */
    public void enforceFillingFormsProtection(String password,
                                              HashAlgorithm hashAlgo) {
     ...
     *                 sha512 are supported. if null, it will default default to sha1
     */
    public void enforceCommentsProtection(String password,
                                          HashAlgorithm hashAlgo) {
        settings.setEnforcementEditValue(STDocProtect.COMMENTS, password,
                hashAlgo);
    }
     ...
     *                 sha512 are supported. if null, it will default default to sha1
     */
    public void enforceTrackedChangesProtection(String password,
                                                HashAlgorithm hashAlgo) {
        settings.setEnforcementEditValue(STDocProtect.TRACKED_CHANGES,
                password, hashAlgo);
    }
    ...
    /**
     * Enforces fields update on document open (in Word). In the settings.xml
     * file <br/>
     * sets the updateSettings value to true (w:updateSettings w:val="true")
     * <p/>
     * NOTICES:
     * <ul>
     * <li>Causing Word to ask on open:
     ...
     *
     * @param enable <code>true</code> to turn on revision tracking,
     *               <code>false</code> to turn off revision tracking
     */
    ...
    /**
     * Returns the current zoom factor in percent values, i.e. 100 is normal
     * zoom.
     *
     * @return A percent value denoting the current zoom setting of this
     * document.
     */
     ...
     *
     * @return a {@link List} of {@link XWPFPictureData}. The returned
     * {@link List} is unmodifiable. Use #a
     */
    ...
    public String addPictureData(byte[] pictureData, int format)
        ...
        if (xwpfPicData == null) {
            ...
            PackageRelationship relShip = getPackagePart().addRelationship(
                    partName, targetMode, relation);
    ...
    public String addPictureData(InputStream is, int format)
            throws InvalidFormatException {
        try {
    ...
    public int getNextPicNameNumber(int format) throws InvalidFormatException {
        ...
        PackagePartName createPartName = PackagingURIHelper
                .createPartName(proposal);
    ...
    /**
     * returns the Part, to which the body belongs, which you need for adding
     * relationship to other parts Actually it is needed of the class
     * XWPFTableCell. Because you have to know to which part the tableCell
     * belongs.
     *
    ...
    /**
     * get the PartType of the body, for example DOCUMENT, HEADER, FOOTER,
     * FOOTNOTE,
     *
