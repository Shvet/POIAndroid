...
import com.shvet.poi.xwpf.usermodel.XWPFDocument;
import com.shvet.poi.xwpf.usermodel.XWPFFactory;
import com.shvet.poi.xwpf.usermodel.XWPFFooter;
import com.shvet.poi.xwpf.usermodel.XWPFHeader;
import com.shvet.poi.xwpf.usermodel.XWPFHeaderFooter;
import com.shvet.poi.xwpf.usermodel.XWPFParagraph;
import com.shvet.poi.xwpf.usermodel.XWPFRelation;
...
public class XWPFHeaderFooterPolicy {
    ...
    private CTHdrFtr buildFtr(Enum type, String pStyle,
                              XWPFHeaderFooter wrapper, XWPFParagraph[] pars) {
    ...
    private CTHdrFtr buildHdr(Enum type, String pStyle,
                              XWPFHeaderFooter wrapper, XWPFParagraph[] pars) {
    ...
    private CTHdrFtr buildHdrFtr(String pStyle, XWPFParagraph[] paragraphs) {
        ...
        if (paragraphs != null) {
            ...
            byte[] rsidrdefault = doc.getDocument().getBody().getPArray(0)
                    .getRsidRDefault();
            p.setRsidP(rsidr);
    ...
    /**
     * MB 24 May 2010. Created this overloaded buildHdrFtr() method because
     * testing demonstrated that the XWPFFooter or XWPFHeader object returned by
     * calls to the createHeader(int, XWPFParagraph[]) and createFooter(int,
     * XWPFParagraph[]) methods or the getXXXXXHeader/Footer methods where
     * headers or footers had been added to a document since it had been
     * created/opened, returned an object that contained no XWPFParagraph
     * objects even if the header/footer itself did contain text. The reason was
     * that this line of code; CTHdrFtr ftr = CTHdrFtr.Factory.newInstance();
     * created a brand new instance of the CTHDRFtr class which was then
     * populated with data when it should have recovered the CTHdrFtr object
     * encapsulated within the XWPFHeaderFooter object that had previoulsy been
     * instantiated in the createHeader(int, XWPFParagraph[]) or
    ...
    private void setFooterReference(Enum type, XWPFHeaderFooter wrapper) {
        CTHdrFtrRef ref = doc.getDocument().getBody().getSectPr()
