...
import com.shvet.poi.xwpf.usermodel.XWPFComment;
import com.shvet.poi.xwpf.usermodel.XWPFParagraph;

import org.openxmlformats.schemas.wordprocessingml.x2006.main.CTMarkupRange;

/**
 * Decorator class for XWPFParagraph allowing to add comments found in paragraph
 * to its text
 *
...
public class XWPFCommentsDecorator extends XWPFParagraphDecorator {
    ...
    public XWPFCommentsDecorator(XWPFParagraph paragraph,
        ...
        for (CTMarkupRange anchor : paragraph.getCTP()
                .getCommentRangeStartArray()) {
            if ((comment = paragraph.getDocument().getCommentByID(
                    anchor.getId().toString())) != null)
                commentText.append("\tComment by " + comment.getAuthor() + ": "
                        + comment.getText());
        }
    }
