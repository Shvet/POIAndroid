...
/**
 * A row within an {@link XWPFTable}. Rows mostly just have sizings and
 * stylings, the interesting content lives inside the child
 * {@link XWPFTableCell}s
 */
public class XWPFTableRow {
    ...
    /**
     * create a new XWPFTableCell and add it to the tableCell-list of this
     * tableRow
     *
     * @return the newly created XWPFTableCell
     */
    public XWPFTableCell createCell() {
        XWPFTableCell tableCell = new XWPFTableCell(ctRow.addNewTc(), this,
                table.getBody());
    ...
    public int getHeight() {
        ...
        return properties.sizeOfTrHeightArray() == 0 ? 0 : properties
                .getTrHeightArray(0).getVal().intValue();
    }
    ...
    public void setHeight(int height) {
        CTTrPr properties = getTrPr();
        CTHeight h = properties.sizeOfTrHeightArray() == 0 ? properties
                .addNewTrHeight() : properties.getTrHeightArray(0);
    ...
    public List<XWPFTableCell> getTableCells() {
        if (tableCells == null) {
            ...
            // TODO: it is possible to have an SDT that contains a cell in
            // within a row
            // need to modify this code so that it pulls out SDT wrappers around
            // cells, too.
    ...
    /**
     * returns the XWPFTableCell which belongs to the CTTC cell if there is no
     * XWPFTableCell which belongs to the parameter CTTc cell null will be
     * returned
     */
     ...
     * pages. The logic for this attribute is a little unusual: a true value
     * means DON'T allow rows to split, false means allow rows to split.
     *
     * @param split - if true, don't allow rows to be split. If false, allow rows
     *              to be split.
     */
