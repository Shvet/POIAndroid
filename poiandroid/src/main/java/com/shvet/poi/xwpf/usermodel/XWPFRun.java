...
import com.shvet.poi.wp.usermodel.CharacterRun;
...
public class XWPFRun implements ISDTContents, IRunElement, CharacterRun {
        ...
        for (CTDrawing ctDrawing : r.getDrawingArray()) {
            for (CTAnchor anchor : ctDrawing.getAnchorArray()) {
                if (anchor.getDocPr() != null) {
                    getDocument().getDrawingIdManager().reserve(
                            anchor.getDocPr().getId());
                }
            }
            for (CTInline inline : ctDrawing.getInlineArray()) {
                if (inline.getDocPr() != null) {
                    getDocument().getDrawingIdManager().reserve(
                            inline.getDocPr().getId());
                }
            }
        }
    ...
    /**
     * Add the xml:spaces="preserve" attribute if the string has leading or
    ...
    static void preserveSpaces(XmlString xs) {
        ...
        if (text != null && (text.startsWith(" ") || text.endsWith(" "))) {
            ...
            c.toNextToken();
            c.insertAttributeWithValue(new QName(
     ...
     * <code>null</code> if parent structure (paragraph > document) is
     * not properly set.
     */
     ...
     * versa). Setting it to <code>false</code> (or an equivalent) shall result
     * in the current setting remaining unchanged. However, when used as direct
    ...
    public void setText(String value, int pos) {
        if (pos > run.sizeOfTArray())
            throw new ArrayIndexOutOfBoundsException(
                    "Value too large for the parameter position in XWPFRun.setText(String value,int pos)");
        CTText t = (pos < run.sizeOfTArray() && pos >= 0) ? run.getTArray(pos)
                : run.addNewT();
     ...
     * </p>
     *
     * @param value <code>true</code> if the italic property is applied to this
     *              run
     */
    ...
    public UnderlinePatterns getUnderline() {
        CTRPr pr = run.getRPr();
        return (pr != null && pr.isSetU() && pr.getU().getVal() != null) ? UnderlinePatterns
                .valueOf(pr.getU().getVal().intValue())
     ...
     * <p/>
     * This formatting property is a toggle property, which specifies that its
     * behaviour differs between its use within a style definition and its use
     * as direct formatting. When used as part of a style definition, setting
     * this property shall toggle the current state of that property as
     * specified up to this point in the hierarchy (i.e. applied to not applied,
     * and vice versa). Setting it to false (or an equivalent) shall result in
     * the current setting remaining unchanged. However, when used as direct
     * formatting, setting this property to true or false shall set the absolute
     * state of the resulting property.
     * </p>
     * <p/>
     ...
     * </p>
     *
     * @param value <code>true</code> if the strike property is applied to this
     *              run
     */
    ...
    public void setDoubleStrikethrough(boolean value) {
        CTRPr pr = run.isSetRPr() ? run.getRPr() : run.addNewRPr();
        CTOnOff dstrike = pr.isSetDstrike() ? pr.getDstrike() : pr
                .addNewDstrike();
    ...
    public void setSmallCaps(boolean value) {
        CTRPr pr = run.isSetRPr() ? run.getRPr() : run.addNewRPr();
        CTOnOff caps = pr.isSetSmallCaps() ? pr.getSmallCaps() : pr
                .addNewSmallCaps();
    ...
    public void setImprinted(boolean value) {
        CTRPr pr = run.isSetRPr() ? run.getRPr() : run.addNewRPr();
        CTOnOff imprinted = pr.isSetImprint() ? pr.getImprint() : pr
                .addNewImprint();
    ...
    public void setSubscript(VerticalAlign valign) {
        CTRPr pr = run.isSetRPr() ? run.getRPr() : run.addNewRPr();
        CTVerticalAlignRun ctValign = pr.isSetVertAlign() ? pr.getVertAlign()
                : pr.addNewVertAlign();
    ...
    public void setCharacterSpacing(int twips) {
        CTRPr pr = run.isSetRPr() ? run.getRPr() : run.addNewRPr();
        CTSignedTwipsMeasure spc = pr.isSetSpacing() ? pr.getSpacing() : pr
                .addNewSpacing();
        spc.setVal(BigInteger.valueOf(twips));
    }

    /**
     * Gets the fonts which shall be used to display the text contents of this
     * run. Specifies a font which shall be used to format all characters in the
     * ASCII range (0 - 127) within the parent run
     *
    ...
    /**
     * Gets the font family for the specified font char range. If fcr is null,
     * the font char range "ascii" is used
     *
    ...
    /**
     * Specifies the fonts which shall be used to display the text contents of
     * this run. The default handling for fcr == null is to overwrite the ascii
     * font char range with the given font family and also set all not specified
     * font ranges
     *
     * @param fontFamily
    ...
    public int getFontSize() {
        CTRPr pr = run.getRPr();
        return (pr != null && pr.isSetSz()) ? pr.getSz().getVal()
                .divide(new BigInteger("2")).intValue() : -1;
    }
     ...
     * of the document's contents.
     * <p>
     * The behavior of this break character (the location where text shall be
     * restarted after this break) shall be determined by its type values.
     * </p>
     *
     * @see BreakType
     */
     ...
     * <p>
     * The behavior of this break character (the location where text shall be
     * restarted after this break) shall be determined by its type (in this case
     * is BreakType.TEXT_WRAPPING as default) and clear attribute values.
     * </p>
     *
     * @see BreakClear
     */
    ...
    /**
     * Specifies that a carriage return shall be placed at the current location
     * in the run content. A carriage return is used to end the current line of
     * text in Wordprocess. The behavior of a carriage return in run content
     * shall be identical to a break character with null type and clear
     * attributes, which shall end the current line and find the next available
     * line on which to continue. The carriage return character forced the
     * following text to be restarted on the next available line in the
     * document.
     */
    ...
    /**
     * Adds a picture to the run. This method handles attaching the picture data
     * to the overall file.
     *
     * @param pictureData The raw picture data
     * @param pictureType The type of the picture, eg {@link Document#PICTURE_TYPE_JPEG}
     * @param width       width in EMUs. To convert to / from points use
     *                    {@link com.shvet.poi.util.Units}
     * @param height      height in EMUs. To convert to / from points use
     *                    {@link com.shvet.poi.util.Units}
     * @throws com.shvet.poi.openxml4j.exceptions.InvalidFormatException
     * @throws IOException
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_EMF
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_WMF
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_PICT
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_JPEG
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_PNG
     * @see com.shvet.poi.xwpf.usermodel.Document#PICTURE_TYPE_DIB
     */
    public XWPFPicture addPicture(InputStream pictureData, int pictureType,
        ...
        if (parent.getPart() instanceof XWPFHeaderFooter) {
            ...
            picData = (XWPFPictureData) headerFooter
        ...
        try {
            ...
            long id = getParent().getDocument().getDrawingIdManager()
                    .reserveNew();
            docPr.setId(id);
    ...
    /**
     * Returns the embedded pictures of the run. These are pictures which
     * reference an external, embedded picture image such as a .png or .jpg
     */
    ...
    /**
     * Returns the string version of the text, with tabs and carriage returns in
     * place of their xml equivalents.
     */
    public String text() {
        ...
        while (c.toNextSelection()) {
            ...
            // Complex type evaluation (currently only for extraction of check
            // boxes)
            if (o instanceof CTFldChar) {
                ...
                if (ctfldChar.getFldCharType() == STFldCharType.BEGIN) {
                    if (ctfldChar.getFfData() != null) {
                        for (CTFFCheckBox checkBox : ctfldChar.getFfData()
                                .getCheckBoxList()) {
            ...
            if (o instanceof CTFtnEdnRef) {
                ...
                String footnoteRef = ftn.getDomNode().getLocalName()
                        .equals("footnoteReference") ? "[footnoteRef:"
                        + ftn.getId().intValue() + "]" : "[endnoteRef:"
                        + ftn.getId().intValue() + "]";
    ...
    /**
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/ff533743(v=office.12).aspx">[MS-OI29500]
     * Run Fonts</a>
     */
    public enum FontCharRange {
        ascii /* char 0-127 */, cs /* complex symbol */, eastAsia /* east asia */, hAnsi /*
                                                                                         * high
																						 * ansi
																						 */
    }
