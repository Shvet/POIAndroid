 ...
 * {@link org.apache.poi.hssf.record.ContinueRecord}s. For such functionality,
 * consider using {@link RecordInputStream}
 */
public class StringUtil {
    public static final Charset UTF8 = Charset.forName("UTF-8");
    protected static final Charset ISO_8859_1 = Charset.forName("ISO-8859-1");
    protected static final Charset UTF16LE = Charset.forName("UTF-16LE");
    private static final int symbolMap_f020[] = {' ', // 0xf020 space
            '!', // 0xf021 exclam
    ...
    private static final int symbolMap_f0a0[] = {8364, // 0xf0a0 not defined /
            // euro symbol
    ...
    /**
     * Given a byte array of 16-bit unicode characters in Little Endian format
     * (most important byte last), return a Java String representation of it.
     * <p/>
     * { 0x16, 0x00 } -0x16
     *
     * @param string the byte array to be converted
     * @param offset the initial offset into the byte array. it is assumed that
     *               string[ offset ] and string[ offset + 1 ] contain the first
     *               16-bit unicode character
     * @param len    the length of the final string
     * @return the converted string, never <code>null</code>.
     * @throws ArrayIndexOutOfBoundsException if offset is out of bounds for the byte array (i.e., is
     *                                        negative or is greater than or equal to string.length)
     * @throws IllegalArgumentException       if len is too large (i.e., there is not enough data in
     *                                        string to create a String of that length)
     */
    ...
    /**
     * Read 8 bit data (in ISO-8859-1 codepage) into a (unicode) Java String and
     * return. (In Excel terms, read compressed 8 bit unicode as a string)
     *
     ...
     * <li>byte[]/char[] characterData</li>
     * </ol>
     * For this encoding, the is16BitFlag is always present even if nChars==0. <br/>
     * This method should be used when the nChars field is <em>not</em> stored
     * as a ushort immediately before the is16BitFlag. Otherwise,
     * {@link #readUnicodeString(LittleEndianInput)} can be used.
     */
     ...
     * This method should be used when the nChars field is <em>not</em> stored
     * as a ushort immediately before the is16BitFlag. Otherwise,
     * {@link #writeUnicodeString(LittleEndianOutput, String)} can be used.
     */
    ...
    /**
     * @return the number of bytes that would be written by
     * {@link #writeUnicodeString(LittleEndianOutput, String)}
     */
     ...
     * ISO-8859-1 codepage). (In Excel terms, write compressed 8 bit unicode)
     *
     * @param input  the String containing the data to be written
     * @param output the byte array to which the data is to be written
     * @param offset an offset into the byte arrat at which the data is start when
     *               written
     */
    public static void putCompressedUnicode(String input, byte[] output,
                                            int offset) {
    ...
    /**
     * Takes a unicode string, and returns it as little endian (most important
     * byte last) bytes in the supplied byte array. (In Excel terms, write
     * uncompressed unicode)
     *
     * @param input  the String containing the unicode data to be written
     * @param output the byte array to hold the uncompressed unicode, should be
     ...
     *
     * @param message the string with embedded formatting info eg.
     *                "This is a test %2.2"
        ...
        for (int i = 0; i < message.length(); i++) {
            if (message.charAt(i) == '%') {
                if (currentParamNumber >= params.length) {
                    ...
                    formattedMessage.append(params[currentParamNumber++]
                            .toString());
                }
            } else {
    ...
    private static int matchOptionalFormatting(Number number,
                                               String formatting, StringBuffer outputTo) {
        NumberFormat numberFormat = NumberFormat.getInstance(Locale.US);
        if ((0 < formatting.length())
                && Character.isDigit(formatting.charAt(0))) {
            numberFormat.setMinimumIntegerDigits(Integer.parseInt(formatting
                    .charAt(0) + ""));
            if ((2 < formatting.length()) && (formatting.charAt(1) == '.')
                    && Character.isDigit(formatting.charAt(2))) {
                numberFormat.setMaximumFractionDigits(Integer
    ...
    public static boolean isUnicodeString(final String value) {
        return !value
                .equals(new String(value.getBytes(ISO_8859_1), ISO_8859_1));
    }

    /**
     * Some strings may contain encoded characters of the unicode private use
     * area. Currently the characters of the symbol fonts are mapped to the
     * corresponding characters in the normal unicode range.
     *
     * @param string the original string
     * @return the string with mapped characters
     * @see <a
     * href="http://www.alanwood.net/unicode/private_use_area.html#symbol">Private
     * Use Area (symbol)</a>
     * @see <a href="http://www.alanwood.net/demos/symbol.html">Symbol font -
     * Unicode alternatives for Greek and special characters in HTML</a>
     */
    public static String mapMsCodepointString(String string) {
        ...
        for (int offset = 0; offset < length; ) {
            ...
            sb.appendCodePoint(uniCodepoint == null ? msCodepoint
                    : uniCodepoint);
    ...
    public static synchronized void mapMsCodepoint(int msCodepoint,
