...
public final class OOXMLLite {
     ...
     */
    private static void collectTests(File root, File arg, List<Class<?>> out,
                                     String ptrn, String exclude) throws ClassNotFoundException {
        if (arg.isDirectory()) {
            ...
            String cls = path.substring(prefix.length() + 1).replace(
                    File.separator, ".");
            if (!cls.matches(ptrn))
                return;
    ...
    private static Map<String, Class<?>> getLoadedClasses(String ptrn) {
        // make the field accessible, we defer this from static initialization
        // to here to
        // allow JDKs which do not have this field (e.g. IBM JDK) to at least
        // load the class
        // without failing, see
        ...
        try {
            Vector<Class<?>> classes = (Vector<Class<?>>) _classes
                    .get(appLoader);
    ...
    void build() throws IOException, ClassNotFoundException {
        ...
        if (!result.wasSuccessful()) {
            throw new RuntimeException(
                    "Tests did not succeed, cannot build ooxml-lite jar");
        }
        ...
        Pattern p = Pattern
                .compile("schemaorg_apache_xmlbeans/(system|element)/.*\\.xsb");
        try {
