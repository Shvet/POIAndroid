...
/**
 * The escher client anchor specifies which rows and cells the shape is bound to
 * as well as the offsets within those cells. Each cell is 1024 units wide by
 * 256 units long regardless of the actual size of the cell. The
 * EscherClientAnchorRecord only applies to the top-most shapes. Shapes
 * contained in groups are bound using the EscherChildAnchorRecords.
 *
...
public class EscherClientAnchorRecord extends EscherRecord {
    ...
    /**
     * bit[0] - fMove (1 bit): A bit that specifies whether the shape will be
     * kept intact when the cells are moved. bit[1] - fSize (1 bit): A bit that
     * specifies whether the shape will be kept intact when the cells are
     * resized. If fMove is 1, the value MUST be 1. bit[2-4] - reserved, MUST be
     * 0 and MUST be ignored bit[5-15]- Undefined and MUST be ignored.
     * <p/>
    ...
    public int fillFields(byte[] data, int offset,
        ...
        {
            // Not sure exactly what the format is quite yet, likely a reference
            // to a PLC
        } else {
    ...
    public int serialize(int offset, byte[] data,
        ...
        System.arraycopy(remainingData, 0, data, offset
                + (shortRecord ? 16 : 26), remainingData.length);
    ...
    public int getRecordSize() {
        return 8 + (shortRecord ? 8 : 18)
                + (remainingData == null ? 0 : remainingData.length);
    }
    ...
    public String toString() {
        ...
        return getClass().getName() + ":" + nl + "  RecordId: 0x"
                + HexDump.toHex(RECORD_ID) + nl + "  Version: 0x"
                + HexDump.toHex(getVersion()) + nl + "  Instance: 0x"
                + HexDump.toHex(getInstance()) + nl + "  Flag: " + field_1_flag
                + nl + "  Col1: " + field_2_col1 + nl + "  DX1: " + field_3_dx1
                + nl + "  Row1: " + field_4_row1 + nl + "  DY1: " + field_5_dy1
                + nl + "  Col2: " + field_6_col2 + nl + "  DX2: " + field_7_dx2
                + nl + "  Row2: " + field_8_row2 + nl + "  DY2: " + field_9_dy2
    ...
    public String toXml(String tab) {
        ...
        builder.append(tab)
                .append(formatXmlRecordHeader(getClass().getSimpleName(),
                        HexDump.toHex(getRecordId()),
                        HexDump.toHex(getVersion()),
                        HexDump.toHex(getInstance()))).append(tab).append("\t")
                .append("<Flag>").append(field_1_flag).append("</Flag>\n")
                .append(tab).append("\t").append("<Col1>").append(field_2_col1)
                .append("</Col1>\n").append(tab).append("\t").append("<DX1>")
                .append(field_3_dx1).append("</DX1>\n").append(tab)
                .append("\t").append("<Row1>").append(field_4_row1)
                .append("</Row1>\n").append(tab).append("\t").append("<DY1>")
                .append(field_5_dy1).append("</DY1>\n").append(tab)
                .append("\t").append("<Col2>").append(field_6_col2)
                .append("</Col2>\n").append(tab).append("\t").append("<DX2>")
                .append(field_7_dx2).append("</DX2>\n").append(tab)
                .append("\t").append("<Row2>").append(field_8_row2)
                .append("</Row2>\n").append(tab).append("\t").append("<DY2>")
                .append(field_9_dy2).append("</DY2>\n").append(tab)
                .append("\t").append("<ExtraData>").append(extraData)
                .append("</ExtraData>\n");
