...
/**
 * Escher container records store other escher records as children. The
 * container records themselves never store any information beyond the standard
 * header used by all escher records. This one record is used to represent many
...
public final class EscherContainerRecord extends EscherRecord {
    ...
    private static POILogger log = POILogFactory
            .getLogger(EscherContainerRecord.class);
     ...
     * BOF ... DrawingRecord ... ObjRecord|TxtObjRecord ... EOF ... BOF(Chart
     * begin) ... DrawingRecord ... ObjRecord|TxtObjRecord ... EOF So, when we
     * call EscherAggregate.createAggregate() we have not all needed data. When
     * we got warning "WARNING: " + bytesRemaining +
     * " bytes remaining but no space left" we should save value of
     * bytesRemaining and add it to container size when we serialize it
     */
    private int _remainingLength;

    public int fillFields(byte[] data, int pOffset,
        ...
        while (bytesRemaining > 0 && offset < data.length) {
            ...
            if (offset >= data.length && bytesRemaining > 0) {
                ...
                if (log.check(POILogger.WARN)) {
                    log.log(POILogger.WARN, "Not enough Escher data: "
                            + bytesRemaining
    ...
    public void setChildRecords(List<EscherRecord> childRecords) {
        if (childRecords == _childRecords) {
            throw new IllegalStateException(
                    "Child records private data member has escaped");
        }
        _childRecords.clear();
    ...
    /**
     * Returns all of our children which are also EscherContainers (may be 0, 1,
     * or vary rarely 2 or 3)
     */
    ...
    public String toXml(String tab) {
        ...
        builder.append(tab).append(
                formatXmlRecordHeader(getRecordName(),
