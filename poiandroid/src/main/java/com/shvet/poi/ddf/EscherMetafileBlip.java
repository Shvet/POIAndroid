...
import com.shvet.poi.hssf.usermodel.HSSFPictureData;
...
public final class EscherMetafileBlip extends EscherBlipRecord {
    ...
    private static final POILogger log = POILogFactory
            .getLogger(EscherMetafileBlip.class);
    ...
    private static byte[] inflatePictureData(byte[] data) {
        try {
            ...
            log.log(POILogger.WARN,
                    "Possibly corrupt compression or non-compressed data", e);
            return data;
        }
    }
    ...
    public Rectangle getBounds() {
        return new Rectangle(field_3_rcBounds_x1, field_3_rcBounds_y1,
                field_3_rcBounds_x2 - field_3_rcBounds_x1, field_3_rcBounds_y2
                - field_3_rcBounds_y1);
    }
    ...
    public String toString() {
        ...
        return getClass().getName()
                ...
                + '\n'
                + "  UID: 0x"
                + HexDump.toHex(field_1_UID)
                + '\n'
                + (field_2_UID == null ? "" : ("  UID2: 0x"
                + HexDump.toHex(field_2_UID) + '\n'))
                + "  Uncompressed Size: "
                + HexDump.toHex(field_2_cb)
                + '\n'
                + "  Bounds: "
                + getBounds()
                + '\n'
                ...
                + '\n'
                + "  Compressed Size: "
                + HexDump.toHex(field_5_cbSave)
                + '\n'
                + "  Compression: "
                + HexDump.toHex(field_6_fCompression)
                + '\n'
                + "  Filter: "
                + HexDump.toHex(field_7_fFilter)
                + '\n'
                + "  Extra Data:"
                + '\n'
                + extraData
                + (remainingData == null ? null
                : ("\n" + " Remaining Data: " + HexDump.toHex(
                remainingData, 32)));
    }
    ...
    public String toXml(String tab) {
        ...
        builder.append(tab)
                ...
                .append("<Bounds>").append(getBounds()).append("</Bounds>\n")
                .append(tab).append("\t").append("<SizeInEMU>")
                .append(getSizeEMU()).append("</SizeInEMU>\n").append(tab)
                .append("\t").append("<CompressedSize>0x")
                .append(HexDump.toHex(field_5_cbSave))
                .append("</CompressedSize>\n").append(tab).append("\t")
                .append("<Compression>0x")
                .append(HexDump.toHex(field_6_fCompression))
                .append("</Compression>\n").append(tab).append("\t")
                .append("<Filter>0x").append(HexDump.toHex(field_7_fFilter))
                .append("</Filter>\n").append(tab).append("\t")
