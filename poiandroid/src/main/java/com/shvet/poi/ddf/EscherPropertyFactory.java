...
/**
 * Generates a property given a reference into the byte array storing that
 * property.
 *
...
public final class EscherPropertyFactory {
     ...
     */
    public List<EscherProperty> createProperties(byte[] data, int offset,
                                                 short numProperties) {
            ...
            if (propertyType == EscherPropertyMetaData.TYPE_BOOLEAN)
                ...
                if (!isComplex)
                    ...
                    if (propertyType == EscherPropertyMetaData.TYPE_ARRAY)
                        results.add(new EscherArrayProperty(propId,
                                new byte[propData]));
                    else
                        results.add(new EscherComplexProperty(propId,
                                new byte[propData]));
                }
            }
            pos += 6;
        ...
        // Get complex data
        for (Iterator<EscherProperty> iterator = results.iterator(); iterator
                .hasNext(); ) {
            ...
            if (p instanceof EscherComplexProperty) {
                if (p instanceof EscherArrayProperty) {
                    ...
                    byte[] complexData = ((EscherComplexProperty) p)
                            .getComplexData();
                    ...
                    if (leftover < complexData.length) {
                        throw new IllegalStateException(
                                "Could not read complex escher property, lenght was "
                                        + complexData.length
                                        + ", but had only " + leftover
                                        + " bytes left");
                    }

                    System.arraycopy(data, pos, complexData, 0,
                            complexData.length);
