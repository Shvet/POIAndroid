...
/**
 * Storage for documents that are too small to use regular DocumentBlocks for
 * their data
 */
public final class SmallDocumentBlock implements BlockWritable,
    ...
    private SmallDocumentBlock(final POIFSBigBlockSize bigBlockSize,
                               final byte[] data, final int index) {
        this(bigBlockSize);
     ...
     * @return an array of SmallDocumentBlock instances, filled from the array
     */
    public static SmallDocumentBlock[] convert(POIFSBigBlockSize bigBlockSize,
                                               byte[] array, int size) {
        SmallDocumentBlock[] rval = new SmallDocumentBlock[(size + _block_size - 1)
                / _block_size];
        int offset = 0;
    ...
    /**
     * fill out a List of SmallDocumentBlocks so that it fully occupies a set of
     * big blocks
     *
    ...
    public static int fill(POIFSBigBlockSize bigBlockSize,
                           List<SmallDocumentBlock> blocks) {
     ...
     * @throws IOException                    on errors reading from the DocumentBlocks
     * @throws ArrayIndexOutOfBoundsException if, somehow, the store contains less data than size
     *                                        indicates
     */
    public static SmallDocumentBlock[] convert(POIFSBigBlockSize bigBlockSize,
                                               BlockWritable[] store, int size) throws IOException,
     ...
     *
     * @param blocks the raw data containing the SmallDocumentBlock data
     * @return a List of SmallDocumentBlock's extracted from the input
     */
    public static List<SmallDocumentBlock> extract(
            POIFSBigBlockSize bigBlockSize, ListManagedBlock[] blocks)
            throws IOException {
        int _blocks_per_big_block = getBlocksPerBigBlock(bigBlockSize);
    ...
    public static DataInputBlock getDataInputBlock(SmallDocumentBlock[] blocks,
        ...
        return new DataInputBlock(blocks[firstBlockIndex]._data,
                firstBlockOffset);
    }
