...
public class EntryUtils {
    ...
    public static void copyNodeRecursively(Entry entry, DirectoryEntry target)
        ...
        // "copyNodeRecursively called with "+entry.getName()+
        // ","+target.getName());
     ...
     *
     * @param sourceRoot is the source Directory to copy from
     * @param targetRoot is the target Directory to copy to
     */
     ...
     */
    public static void copyNodes(DirectoryEntry sourceRoot,
                                 DirectoryEntry targetRoot, List<String> excepts) throws IOException {
    ...
    /**
     * Copies nodes from one POIFS to the other, minus the excepts. This
     * delegates the filtering work to {@link FilteringDirectoryNode}, so
     * excepts can be of the form "NodeToExclude" or
    ...
    public static void copyNodes(OPOIFSFileSystem source,
        ...
        copyNodes(new FilteringDirectoryNode(source.getRoot(), excepts),
    ...
    /**
     * Checks to see if the two Directories hold the same contents. For this to
     * be true, they must have entries with the same names, no entries in one
     * but not the other, and the size+contents of each entry must match, and
     * they must share names. To exclude certain parts of the Directory from
     * being checked, use a {@link FilteringDirectoryNode}
     */
    public static boolean areDirectoriesIdentical(DirectoryEntry dirA,
        ...
        for (Entry a : dirA) {
            try {
                ...
                boolean match;
                if (a.isDirectoryEntry()) {
                    match = areDirectoriesIdentical((DirectoryEntry) a,
                            (DirectoryEntry) b);
                } else {
                    match = areDocumentsIdentical((DocumentEntry) a,
                            (DocumentEntry) b);
                }
                if (!match)
                    return false;
    ...
    public static boolean areDocumentsIdentical(DocumentEntry docA,
                                                DocumentEntry docB) throws IOException {
