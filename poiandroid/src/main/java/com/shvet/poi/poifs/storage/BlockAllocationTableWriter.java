...
import com.shvet.poi.poifs.filesystem.BATManaged;
...
/**
 * This class manages and creates the Block Allocation Table, which is basically
 * a set of linked lists of block indices.
 * <p/>
 * Each block of the filesystem has an index. The first block, the header, is
 * skipped; the first block after the header is index 0, the next is index 1,
 * and so on.
 * <p/>
 * A block's index is also its index into the Block Allocation Table. The entry
 * that it finds in the Block Allocation Table is the index of the next block in
 * the linked list of blocks making up a file, or it is set to -2: end of list.
 *
 * @author Marc Johnson (mjohnson at apache dot org)
 */
public final class BlockAllocationTableWriter implements BlockWritable,
        BATManaged {
    ...
    public int createBlocks() {
        ...
        while (true) {
            int calculated_bat_blocks = BATBlock.calculateStorageRequirements(
                    _bigBlockSize, bat_blocks + xbat_blocks + _entries.size());
            int calculated_xbat_blocks = HeaderBlockWriter
                    .calculateXBATStorageRequirements(_bigBlockSize,
