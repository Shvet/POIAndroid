...
import com.shvet.poi.poifs.property.RootProperty;
...
public class NPOIFSMiniStore extends BlockStore {
    ...
    protected ByteBuffer getBlockAt(final int offset) throws IOException {
        ...
        if (dataBlock == null) {
            throw new IndexOutOfBoundsException("Big block " + bigBlockNumber
                    + " outside stream");
        }
    ...
    protected ByteBuffer createBlockIfNeeded(final int offset)
        ...
        if (firstInStore) {
            _filesystem._get_property_table().getRoot()
                    .setStartBlock(newBigBlock);
    ...
    protected int getFreeBlock() throws IOException {
        int sectorsPerSBAT = _filesystem.getBigBlockSizeDetails()
        ...
        BATBlock newSBAT = BATBlock.createEmptyBATBlock(
                _filesystem.getBigBlockSizeDetails(), false);
    ...
    /**
     * Writes the SBATs to their backing blocks, and updates the mini-stream
     * size in the properties. Stream size is based on full blocks used, not the
     * data within the streams
     */
    protected void syncWithDataSource() throws IOException {
        ...
        for (BATBlock sbat : _sbat_blocks) {
            ...
            if (!sbat.hasFreeSectors()) {
                blocksUsed += _filesystem.getBigBlockSizeDetails()
                        .getBATEntriesPerBlock();
            } else {
