...
/**
 * This abstract class describes a way to read, store, chain and free a series
 * of blocks (be they Big or Small ones)
 */
public abstract class BlockStore {
     ...
     */
    protected abstract ByteBuffer getBlockAt(final int offset)
            throws IOException;

    /**
     * Extends the file if required to hold blocks up to the specified offset,
     * and return the block from there.
     */
    protected abstract ByteBuffer createBlockIfNeeded(final int offset)
            throws IOException;
     ...
     * file/stream if needed, and if doing so, allocate new FAT blocks to
     * address the extra space.
     */
    ...
    /**
     * Used to detect if a chain has a loop in it, so we can bail out with an
     * error rather than spinning away for ever...
     */
