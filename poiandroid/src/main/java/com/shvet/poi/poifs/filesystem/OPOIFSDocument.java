...
import com.shvet.poi.poifs.storage.DataInputBlock;
import com.shvet.poi.poifs.storage.DocumentBlock;
import com.shvet.poi.poifs.storage.ListManagedBlock;
import com.shvet.poi.poifs.storage.RawDataBlock;
import com.shvet.poi.poifs.storage.SmallDocumentBlock;
 ...
 */
public final class OPOIFSDocument implements BATManaged, BlockWritable,
        POIFSViewable {
    ...
    public OPOIFSDocument(String name, RawDataBlock[] blocks, int length)
        ...
        if (blocks.length == 0) {
            ...
            _bigBigBlockSize = (blocks[0].getBigBlockSize() == POIFSConstants.SMALLER_BIG_BLOCK_SIZE ? POIFSConstants.SMALLER_BIG_BLOCK_SIZE_DETAILS
                    : POIFSConstants.LARGER_BIG_BLOCK_SIZE_DETAILS);
        }

        _big_store = new BigBlockStore(_bigBigBlockSize,
                convertRawBlocksToBigBlocks(blocks));
     ...
     * @param length the actual length of the POIFSDocument
     */
    public OPOIFSDocument(String name, POIFSBigBlockSize bigBlockSize,
                          ListManagedBlock[] blocks, int length) throws IOException {
        _size = length;
        ...
        if (Property.isSmall(_size)) {
            ...
            _small_store = new SmallBlockStore(bigBlockSize,
                    convertRawBlocksToSmallBlocks(blocks));
        } else {
    ...
    public OPOIFSDocument(String name, ListManagedBlock[] blocks, int length)
            throws IOException {
        this(name, POIFSConstants.SMALLER_BIG_BLOCK_SIZE_DETAILS, blocks,
                length);
    }
    ...
    public OPOIFSDocument(String name, POIFSBigBlockSize bigBlockSize,
        ...
        DocumentBlock[] bigBlocks = blocks.toArray(new DocumentBlock[blocks
                .size()]);
        ...
        if (_property.shouldUseSmallBlocks()) {
            _small_store = new SmallBlockStore(bigBlockSize,
                    SmallDocumentBlock.convert(bigBlockSize, bigBlocks, _size));
     ...
     */
    public OPOIFSDocument(String name, int size,
                          POIFSBigBlockSize bigBlockSize, POIFSDocumentPath path,
        ...
        _property.setDocument(this);
        if (_property.shouldUseSmallBlocks()) {
            _small_store = new SmallBlockStore(_bigBigBlockSize, path, name,
                    size, writer);
            _big_store = new BigBlockStore(_bigBigBlockSize,
            ...
            _small_store = new SmallBlockStore(_bigBigBlockSize,
                    EMPTY_SMALL_BLOCK_ARRAY);
            _big_store = new BigBlockStore(_bigBigBlockSize, path, name, size,
                    writer);
        }
    }

    public OPOIFSDocument(String name, int size, POIFSDocumentPath path,
                          POIFSWriterListener writer) {
        this(name, size, POIFSConstants.SMALLER_BIG_BLOCK_SIZE_DETAILS, path,
                writer);
    }

    // TODO - awkward typing going on here
    private static DocumentBlock[] convertRawBlocksToBigBlocks(
            ListManagedBlock[] blocks) throws IOException {
    ...
    private static SmallDocumentBlock[] convertRawBlocksToSmallBlocks(
     ...
     * @param offset the offset into our storage to read from This method is
     *               currently (Oct 2008) only used by test code. Perhaps it can be
     *               deleted
     */
    ...
    /**
     * @return <code>null</code> if <tt>offset</tt> points to the end of the
     * document stream
     */
    DataInputBlock getDataInputBlock(int offset) {
        if (offset >= _size) {
            if (offset > _size) {
                throw new RuntimeException("Request for Offset " + offset
                        + " doc size is " + _size);
            }
            return null;
        }
        if (_property.shouldUseSmallBlocks()) {
            return SmallDocumentBlock.getDataInputBlock(
                    _small_store.getBlocks(), offset);
        }
    ...
    private static final class SmallBlockStore {
         ...
         */
        SmallBlockStore(POIFSBigBlockSize bigBlockSize,
        ...
        SmallDocumentBlock[] getBlocks() {
            if (isValid() && _writer != null) {
                ...
                DocumentOutputStream dstream = new DocumentOutputStream(stream,
                        _size);

                _writer.processPOIFSWriterEvent(new POIFSWriterEvent(dstream,
                        _path, _name, _size));
                _smallBlocks = SmallDocumentBlock.convert(_bigBlockSize,
                        stream.toByteArray(), _size);
            }
            return _smallBlocks;
        }
    ...
    private static final class BigBlockStore {
        ...
        DocumentBlock[] getBlocks() {
            if (isValid() && _writer != null) {
                ...
                _writer.processPOIFSWriterEvent(new POIFSWriterEvent(dstream,
                        _path, _name, _size));
                bigBlocks = DocumentBlock.convert(_bigBlockSize,
                        stream.toByteArray(), _size);
            }
            return bigBlocks;
        }
        ...
        void writeBlocks(OutputStream stream) throws IOException {
            if (isValid()) {
                if (_writer != null) {
                    DocumentOutputStream dstream = new DocumentOutputStream(
                            stream, _size);

                    _writer.processPOIFSWriterEvent(new POIFSWriterEvent(
                            dstream, _path, _name, _size));
                    dstream.writeFiller(
                            countBlocks() * _bigBlockSize.getBigBlockSize(),
