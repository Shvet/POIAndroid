...
import com.shvet.poi.poifs.crypt.DataSpaceMapUtils;
import com.shvet.poi.poifs.crypt.EncryptionInfo;
import com.shvet.poi.poifs.crypt.EncryptionVerifier;
import com.shvet.poi.poifs.crypt.Encryptor;
...
import com.shvet.poi.util.LittleEndianOutputStream;
import com.shvet.poi.util.TempFile;
...
public class StandardEncryptor extends Encryptor {
    ...
    public void confirmPassword(String password, byte keySpec[],
        ...
        SecretKey secretKey = StandardDecryptor.generateSecretKey(password,
                ver, getKeySizeInBytes());
        setSecretKey(secretKey);
        Cipher cipher = getCipher(secretKey, null);

        try {
            byte encryptedVerifier[] = cipher.doFinal(verifier);
            MessageDigest hashAlgo = CryptoFunctions.getMessageDigest(ver
                    .getHashAlgorithm());
            ...
            // hash of the randomly generated Verifier value. The length of the
            // array MUST be the size of
            // the encryption block size multiplied by the number of blocks
            // needed to encrypt the hash of the
            // Verifier. If the encryption algorithm is RC4, the length MUST be
            // 20 bytes. If the encryption
            // algorithm is AES, the length MUST be 32 bytes. After decrypting
            // the EncryptedVerifierHash
            ...
            byte encryptedVerifierHash[] = cipher.doFinal(Arrays.copyOf(
                    calcVerifierHash, encVerHashSize));
    ...
    private Cipher getCipher(SecretKey key, String padding) {
        ...
        return CryptoFunctions.getCipher(key, ver.getCipherAlgorithm(),
                ver.getChainingMode(), null, Cipher.ENCRYPT_MODE, padding);
    }
    ...
    protected class StandardCipherOutputStream extends FilterOutputStream
            implements POIFSWriterListener {
        protected final File fileOut;
        protected final DirectoryNode dir;
        protected long countBytes;

        protected StandardCipherOutputStream(DirectoryNode dir)
                throws IOException {
            super(null);
            ...
            // The final data block MUST be padded to the next integral multiple
            // of the
            // KeyData.blockSize value. Any padding bytes can be used. Note that
            // the StreamSize
            // field of the EncryptedPackage field specifies the number of bytes
            // of
        ...
        public void processPOIFSWriterEvent(POIFSWriterEvent event) {
            try {
                ...
                // StreamSize (8 bytes): An unsigned integer that specifies the
                // number of bytes used by data
                // encrypted within the EncryptedData field, not including the
                // size of the StreamSize field.
                // Note that the actual size of the \EncryptedPackage stream (1)
                // can be larger than this
                // value, depending on the block size of the chosen encryption
                // algorithm
