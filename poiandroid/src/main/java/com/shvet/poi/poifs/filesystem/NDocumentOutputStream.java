...
public final class NDocumentOutputStream extends OutputStream {
    ...
    private ByteArrayOutputStream _buffer = new ByteArrayOutputStream(
            POIFSConstants.BIG_BLOCK_MINIMUM_DOCUMENT_SIZE);
    ...
    /**
     * Create an OutputStream from the specified DocumentEntry. The specified
     * entry will be emptied.
     *
    ...
    public NDocumentOutputStream(DirectoryEntry parent, String name)
        ...
        if (!(parent instanceof DirectoryNode)) {
            throw new IOException("Cannot open internal directory storage, "
                    + parent + " not a Directory Node");
        }
        _document_size = 0;
        _closed = false;

        // Have an empty one created for now
        DocumentEntry doc = parent.createDocument(name,
    ...
    public void close() throws IOException {
        ...
        if (_buffer != null) {
            ...
            _document.replaceContents(new ByteArrayInputStream(_buffer
                    .toByteArray()));
        } else {
