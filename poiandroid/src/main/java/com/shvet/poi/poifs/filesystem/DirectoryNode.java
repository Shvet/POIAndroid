...
public class DirectoryNode extends EntryNode implements DirectoryEntry,
    ...
    /**
     * create a DirectoryNode. This method is not public by design; it is
     * intended strictly for the internal use of this package
     *
    ...
    DirectoryNode(final DirectoryProperty property,
                  final OPOIFSFileSystem filesystem, final DirectoryNode parent) {
     ...
     */
    DirectoryNode(final DirectoryProperty property,
                  final NPOIFSFileSystem nfilesystem, final DirectoryNode parent) {
        this(property, parent, null, nfilesystem);
    }

    private DirectoryNode(final DirectoryProperty property,
                          final DirectoryNode parent, final OPOIFSFileSystem ofilesystem,
        ...
        if (parent == null) {
            ...
            _path = new POIFSDocumentPath(parent._path,
    ...
    /**
     * If this is OPOIFS based, return the NPOIFSFileSystem that this belong to,
     * otherwise Null if NPOIFS based
     *
     * @return the filesystem that this belongs to
     */
    ...
    /**
     * If this is NPOIFS based, return the NPOIFSFileSystem that this belong to,
     * otherwise Null if OPOIFS based
     *
     * @return the filesystem that this belongs to
     */
     ...
     *                     DirectoryEntry
     */
    public DocumentInputStream createDocumentInputStream(final Entry document)
            throws IOException {
    ...
    boolean changeName(final String oldName, final String newName) {
        ...
        if (child != null) {
            rval = ((DirectoryProperty) getProperty()).changeName(
                    child.getProperty(), newName);
            if (rval) {
    ...
    boolean deleteEntry(final EntryNode entry) {
        boolean rval = ((DirectoryProperty) getProperty()).deleteChild(entry
                .getProperty());
    ...
    /**
     * get an iterator of the Entry instances contained directly in this
     * instance (in other words, children only; no grandchildren etc.)
     *
     * @return iterator; never null, but hasNext() may return false immediately
     * (i.e., this DirectoryEntry is empty). All objects retrieved by
     * next() are guaranteed to be implementations of Entry.
     */
    ...
    /**
     * get the names of all the Entries contained directly in this instance (in
     * other words, names of children only; no grandchildren etc).
     *
     * @return the names of all the entries that may be retrieved with
     * getEntry(String), which may be empty (if this DirectoryEntry is
     * empty)
     */
    ...
    /**
     * find out how many Entry instances are contained directly within this
     * DirectoryEntry
     *
     * @return number of immediately (no grandchildren etc.) contained Entry
     * instances
     */
     ...
     * @return the specified Entry, if it is directly contained in this
     * DirectoryEntry
     * @throws FileNotFoundException if no Entry with the specified name exists in this
     *                               DirectoryEntry
     */
     ...
     * @param stream the InputStream from which to create the new DocumentEntry
     * @return the new DocumentEntry
     * @throws IOException
     */
    ...
    public DocumentEntry createDocument(final String name, final int size,
        ...
        if (_nfilesystem != null) {
            return createDocument(new NPOIFSDocument(name, size, _nfilesystem,
                    writer));
        } else {
    ...
    /**
     * extensions use this method to verify internal rules regarding deletion of
     * the underlying store.
     *
     * @return true if it's ok to delete the underlying store, else false
     */
