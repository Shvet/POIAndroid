...
public class CryptoFunctions {
    private static final int InitialCodeArray[] = {0xE1F0, 0x1D0F, 0xCC9C,
            0x84C0, 0x110C, 0x0E10, 0xF1CE, 0x313E, 0x1872, 0xE139, 0xD40F,
            0x84F9, 0x280C, 0xA96A, 0x4EC3};
    private static final byte PadArray[] = {(byte) 0xBB, (byte) 0xFF,
            (byte) 0xFF, (byte) 0xBA, (byte) 0xFF, (byte) 0xFF, (byte) 0xB9,
            (byte) 0x80, (byte) 0x00, (byte) 0xBE, (byte) 0x0F, (byte) 0x00,
            (byte) 0xBF, (byte) 0x0F, (byte) 0x00};
     ...
     * agile encryption MUST be generated by using the following method, which
     * is derived from PKCS #5: Password-Based Cryptography Version 2.0
     * [RFC2898].
     * <p/>
     * Let H() be a hashing algorithm as determined by the
     * PasswordKeyEncryptor.hashAlgorithm element, H_n be the hash data of the
     * n-th iteration, and a plus sign (+) represent concatenation. The password
     * MUST be provided as an array of Unicode characters. Limitations on the
     * length of the password and the characters used by the password are
     * implementation-dependent. The initial password hash is generated as
     * follows:
     * <p/>
     * - H_0 = H(salt + password)
     * <p/>
     ...
     * \EncryptionInfo stream (1) as specified in section 2.3.4.10. The hash is
     * then iterated by using the following approach:
     * <p/>
     * - H_n = H(iterator + H_n-1)
     * <p/>
     * where iterator is an unsigned 32-bit value that is initially set to
     * 0x00000000 and then incremented monotonically on each iteration until
     * PasswordKey.spinCount iterations have been performed. The value of
     * iterator on the last iteration MUST be one less than
     * PasswordKey.spinCount.
     * <p/>
     * For POI, H_final will be calculated by
     * {@link #generateKey(byte[], HashAlgorithm, byte[], int)}
     *
     * @param password
     * @param hashAlgorithm
     * @param salt
    ...
    public static byte[] hashPassword(String password,
                                      HashAlgorithm hashAlgorithm, byte salt[], int spinCount) {
        return hashPassword(password, hashAlgorithm, salt, spinCount, true);
    }

    /**
     * Generalized method for read and write protection hash generation. The
     * difference is, read protection uses the order iterator then hash in the
     * hash loop, whereas write protection uses first the last hash value and
     * then the current iterator value
     *
     * @param password
     * @param hashAlgorithm
     * @param salt
     * @param spinCount
     * @param iteratorFirst if true, the iterator is hashed before the n-1 hash value, if
     *                      false the n-1 hash value is applied first
     * @return the hashed password
     */
    public static byte[] hashPassword(String password,
                                      HashAlgorithm hashAlgorithm, byte salt[], int spinCount,
        ...
        try {
            for (int i = 0; i < spinCount; i++) {
                ...
                hashAlg.digest(hash, 0, hash.length); // don't create hash
                // buffer everytime new
            }
     ...
     * initialization vector MUST be generated by using the following method,
     * where H() is a hash function that MUST be the same as specified in
     * section 2.3.4.11 and a plus sign (+) represents concatenation: 1. If a
     * blockKey is provided, let IV be a hash of the KeySalt and the following
     * value: blockKey: IV = H(KeySalt + blockKey) 2. If a blockKey is not
     * provided, let IV be equal to the following value: KeySalt:IV = KeySalt.
     * 3. If the number of bytes in the value of IV is less than the the value
     * of the blockSize attribute corresponding to the cipherAlgorithm
     * attribute, pad the array of bytes by appending 0x36 until the array is
     * blockSize bytes. If the array of bytes is larger than blockSize bytes,
     * truncate the array to blockSize bytes.
     **/
     ...
     * ... continued ...
     * <p/>
     * The final hash data that is used for an encryption key is then generated
     * by using the following method:
     * <p/>
     * - H_final = H(H_n + blockKey)
     * <p/>
     * where blockKey represents an array of bytes used to prevent two different
     * blocks from encrypting to the same cipher text.
     * <p/>
     * If the size of the resulting H_final is smaller than that of
     * PasswordKeyEncryptor.keyBits, the key MUST be padded by appending bytes
     * with a value of 0x36. If the hash value is larger in size than
    ...
    public static byte[] generateKey(byte[] passwordHash,
                                     HashAlgorithm hashAlgorithm, byte[] blockKey, int keySize) {
        MessageDigest hashAlgo = getMessageDigest(hashAlgorithm);
    ...
    public static Cipher getCipher(SecretKey key,
                                   CipherAlgorithm cipherAlgorithm, ChainingMode chain, byte[] vec,
                                   int cipherMode) {
        return getCipher(key, cipherAlgorithm, chain, vec, cipherMode, null);
    }

    /**
     * Initialize a new cipher object with the given cipher properties If the
     * given algorithm is not implemented in the JCE, it will try to load it
     * from the bouncy castle provider.
     *
     ...
     * @throws EncryptedDocumentException if the initialization failed or if an algorithm was
     *                                    specified, which depends on a missing bouncy castle provider
     */
    ...
    /**
     * Returns a new byte array with a truncated to the given size. If the hash
     * has less then size bytes, it will be filled with 0x36-bytes
     *
    ...
    /**
     * Returns a new byte array with a truncated to the given size. If the hash
     * has less then size bytes, it will be filled with 0-bytes
     *
     * @param hash the to be truncated/filled hash byte array
     * @param size the size of the returned byte array
     * @return the padded hash
    ...
    private static byte[] getBlockX(byte[] hash, int size, byte fill) {
        ...
        System.arraycopy(hash, 0, result, 0,
                Math.min(result.length, hash.length));
        return result;
    }
    ...
    public static void registerBouncyCastle() {
        ...
        try {
            ...
            Class<Provider> clazz = (Class<Provider>) cl
                    .loadClass(bcProviderName);
            Security.addProvider(clazz.newInstance());
        } catch (Exception e) {
            throw new EncryptedDocumentException(
                    "Only the BouncyCastle provider supports your encryption settings - please add it to the classpath.");
        }
    }

    /**
     * This method generates the xor verifier for word documents &lt; 2007
     * (method 2). Its output will be used as password input for the newer word
     * generations which utilize a real hashing algorithm like sha1.
     *
     * @param password the password
     * @return the hashed password
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/dd905229.aspx">2.3.7.4
     * Binary Document Password Verifier Derivation Method 2</a>
     * @see <a
     * href="http://blogs.msdn.com/b/vsod/archive/2010/04/05/how-to-set-the-editing-restrictions-in-word-using-open-xml-sdk-2-0.aspx">How
     * to set the editing restrictions in Word using Open XML SDK 2.0</a>
     * @see <a
     ...
     * How the new powerful cryptography implemented in Word 2007 turns it
     * into a perfect tool for document password removal.</a>
     */
    public static int createXorVerifier2(String password) {
        ...
        if (!"".equals(password)) {
            ...
            password = password.substring(0,
                    Math.min(password.length(), maxPasswordLength));

            // Construct a new NULL-terminated string consisting of single-byte
            // characters:
            // -- > Get the single-byte values by iterating through the Unicode
            // characters of the truncated Password.
            // --> For each character, if the low byte is not equal to 0, take
            // it. Otherwise, take the high byte.
            ...
            // --> Initialize from the initial code array (see below), depending
            // on the passwords length.
            int highOrderWord = InitialCodeArray[arrByteChars.length - 1];

            // --> For each character in the password:
            // --> For every bit in the character, starting with the least
            // significant and progressing to (but excluding)
            // the most significant, if the bit is set, XOR the keys high-order
            // word with the corresponding word from
    ...
    /**
     * This method generates the xored-hashed password for word documents &lt;
     * 2007.
     */
    ...
    /**
     * Convenience function which returns the reversed xored-hashed password for
     * further processing in word documents 2007 and newer, which utilize a real
     * hashing algorithm like sha1.
     */
    public static String xorHashPasswordReversed(String password) {
        int hashedPassword = createXorVerifier2(password);

        return String.format(Locale.ROOT, "%1$02X%2$02X%3$02X%4$02X",
                (hashedPassword >>> 0) & 0xFF, (hashedPassword >>> 8) & 0xFF,
                (hashedPassword >>> 16) & 0xFF, (hashedPassword >>> 24) & 0xFF);
    }
     ...
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/dd926947.aspx">2.3.7.1
     * Binary Document Password Verifier Derivation Method 1</a>
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/dd905229.aspx">2.3.7.4
     * Binary Document Password Verifier Derivation Method 2</a>
     */
     ...
     * @return the xor key
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/dd924704.aspx">2.3.7.2
     * Binary Document XOR Array Initialization Method 1</a>
     * @see <a
     * href="http://msdn.microsoft.com/en-us/library/dd905229.aspx">2.3.7.4
     * Binary Document Password Verifier Derivation Method 2</a>
     */
     ...
     * Binary Document XOR Array Initialization Method 1</a>
     * @see <a
     * href="http://docs.libreoffice.org/oox/html/binarycodec_8cxx_source.html">Libre
     * Office implementation</a>
     */
    public static byte[] createXorArray1(String password) {
        ...
        byte baseKeyLE[] = {(byte) (xorKey & 0xFF),
                (byte) ((xorKey >>> 8) & 0xFF)};
    ...
    private static short rotateLeftBase15Bit(short verifier) {
		/*
		 * IF (Verifier BITWISE AND 0x4000) is 0x0000 SET Intermediate1 TO 0
		 * ELSE SET Intermediate1 TO 1 ENDIF
		 */
        short intermediate1 = (short) (((verifier & 0x4000) == 0) ? 0 : 1);
		/*
		 * SET Intermediate2 TO Verifier MULTIPLED BY 2 SET most significant bit
		 * of Intermediate2 TO 0
		 */
