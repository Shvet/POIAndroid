...
import com.shvet.poi.poifs.crypt.standard.EncryptionRecord;
import com.shvet.poi.poifs.filesystem.DirectoryEntry;
import com.shvet.poi.poifs.filesystem.DocumentEntry;
import com.shvet.poi.poifs.filesystem.POIFSWriterEvent;
import com.shvet.poi.poifs.filesystem.POIFSWriterListener;
...
public class DataSpaceMapUtils {
    public static void addDefaultDataSpace(DirectoryEntry dir)
            throws IOException {
        DataSpaceMapEntry dsme = new DataSpaceMapEntry(new int[]{0},
        ...
        DataSpaceDefinition dsd = new DataSpaceDefinition(
                new String[]{"StrongEncryptionTransform"});
        createEncryptionEntry(dir,
                "\u0006DataSpaces/DataSpaceInfo/StrongEncryptionDataSpace", dsd);

        TransformInfoHeader tih = new TransformInfoHeader(1,
                "{FF9A3F03-56EF-4613-BDD5-5A41C1D07246}",
                "Microsoft.Container.EncryptionTransform", 1, 0, 1, 0, 1, 0);
        ...
        DataSpaceVersionInfo dsvi = new DataSpaceVersionInfo(
                "Microsoft.Container.DataSpaces", 1, 0, 1, 0, 1, 0);
    ...
    public static DocumentEntry createEncryptionEntry(DirectoryEntry dir,
        ...
        for (int i = 0; i < parts.length - 1; i++) {
            dir = dir.hasEntry(parts[i]) ? (DirectoryEntry) dir
                    .getEntry(parts[i]) : dir.createDirectory(parts[i]);
        }

        final byte buf[] = new byte[5000];
        LittleEndianByteArrayOutputStream bos = new LittleEndianByteArrayOutputStream(
                buf, 0);
        out.write(bos);
        ...
        return dir.createDocument(fileName, bos.getWriteIndex(),
    ...
    public static String readUnicodeLPP4(LittleEndianInput is) {
        ...
        if (length % 4 == 2) {
            // Padding (variable): A set of bytes that MUST be of the correct
            // size such that the size of the
            // UNICODE-LP-P4 structure is a multiple of 4 bytes. If Padding is
    ...
    public static String readUtf8LPP4(LittleEndianInput is) {
        ...
        // Padding (variable): A set of bytes that MUST be of correct size such
        // that the size of the UTF-8-LP-P4
        // structure is a multiple of 4 bytes. If Padding is present, each byte
        // MUST be 0x00. If
        // the length is exactly 0x00000000, this specifies a null string, and
        // the entire structure uses
        // exactly 4 bytes. If the length is exactly 0x00000004, this specifies
    ...
    public static class DataSpaceMapEntry implements EncryptionRecord {
        ...
        public DataSpaceMapEntry(int referenceComponentType[],
                                 String referenceComponent[], String dataSpaceName) {
    ...
    public static class IRMDSTransformInfo implements EncryptionRecord {
        ...
        public IRMDSTransformInfo(TransformInfoHeader transformInfoHeader,
                                  int extensibilityHeader, String xrMLLicense) {
    ...
    public static class TransformInfoHeader implements EncryptionRecord {
        ...
        public TransformInfoHeader(int transformType, String transformerId,
                                   String transformerName, int readerVersionMajor,
                                   int readerVersionMinor, int updaterVersionMajor,
                                   int updaterVersionMinor, int writerVersionMajor,
        ...
        public void write(LittleEndianByteArrayOutputStream bos) {
            ...
            LittleEndianOutput sizeOut = bos
                    .createDelayedOutput(LittleEndianConsts.INT_SIZE);
    ...
    public static class DataSpaceVersionInfo implements EncryptionRecord {
        ...
        public DataSpaceVersionInfo(String featureIdentifier,
