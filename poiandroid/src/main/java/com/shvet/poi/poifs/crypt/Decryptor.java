...
public abstract class Decryptor {
     ...
     * <p>
     * Use {@link #getLength()} to get the size of that data that can be safely
     * read from the stream. Just reading to the end of the input stream is not
     * sufficient because there are normally padding bytes that must be
     * discarded
     * </p>
     *
     ...
     * Just reading to the end of the input stream is not sufficient because
     * there are normally padding bytes that must be discarded
     * <p/>
     * <p>
     * The length variable is initialized in
     * {@link #getDataStream(org.apache.poi.poifs.filesystem.DirectoryNode)}, an
     * attempt to call getLength() prior to getDataStream() will result in
