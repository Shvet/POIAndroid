...
package com.shvet.poi.poifs.dev;
...
import com.shvet.poi.util.IntList;
...
/**
 * A very low level debugging tool, for printing out core information on the
 * headers and FAT blocks. You probably only want to use this if you're trying
 * to understand POIFS, or if you're trying to track down the source of
...
public class POIFSHeaderDumper {
    ...
    public static void viewFile(final String filename) throws Exception {
        ...
        // Main FAT Table
        BlockAllocationTableReader batReader = new BlockAllocationTableReader(
                header_block.getBigBlockSize(), header_block.getBATCount(),
                header_block.getBATArray(), header_block.getXBATCount(),
                header_block.getXBATIndex(), data_blocks);
        ...
        // Mini Fat
        BlockAllocationTableReader sbatReader = SmallBlockTableReader
                ._getSmallDocumentBlockReader(bigBlockSize, data_blocks,
                        properties.getRoot(), header_block.getSBATStart());
    ...
    public static void displayHeader(HeaderBlock header_block) throws Exception {
        ...
        System.out.println(" Block size: "
                + header_block.getBigBlockSize().getBigBlockSize());
        System.out.println(" BAT (FAT) header blocks: "
                + header_block.getBATArray().length);
        System.out.println(" BAT (FAT) block count: "
                + header_block.getBATCount());
        if (header_block.getBATCount() > 0)
            System.out.println(" BAT (FAT) block 1 at: "
                    + header_block.getBATArray()[0]);
        System.out.println(" XBAT (FAT) block count: "
                + header_block.getXBATCount());
        System.out.println(" XBAT (FAT) block 1 at: "
                + header_block.getXBATIndex());
        System.out.println(" SBAT (MiniFAT) block count: "
                + header_block.getSBATCount());
        System.out.println(" SBAT (MiniFAT) block 1 at: "
                + header_block.getSBATStart());
        System.out.println(" Property table at: "
    ...
    public static void displayRawBlocksSummary(RawDataBlockList data_blocks)
        ...
        Method gbm = data_blocks.getClass().getSuperclass()
    ...
    public static void displayBATReader(String type,
                                        BlockAllocationTableReader batReader) throws Exception {
    ...
    public static void displayPropertiesSummary(PropertyTable properties) {
        System.out.println("Mini Stream starts at "
                + properties.getRoot().getStartBlock());
        System.out.println("Mini Stream length is "
                + properties.getRoot().getSize());
        System.out.println();
    ...
    public static void displayProperties(DirectoryProperty prop, String indent) {
        ...
        for (Property cp : prop) {
            if (cp instanceof DirectoryProperty) {
                ...
                System.out.print(nextIndent + "   " + cp.getSize()
                        + " bytes in ");
