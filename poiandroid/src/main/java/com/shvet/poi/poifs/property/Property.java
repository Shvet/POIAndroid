...
package com.shvet.poi.poifs.property;
...
import com.shvet.poi.util.ByteField;
...
/**
 * This abstract base class is the ancestor of all classes implementing POIFS
 * Property behavior.
 *
...
public abstract class Property implements Child, POIFSViewable {
    ...
    protected Property() {
        ...
        _previous_property = new IntegerField(_previous_property_offset,
                _NO_INDEX, _raw_data);
        _next_property = new IntegerField(_next_property_offset, _NO_INDEX,
                _raw_data);
        _child_property = new IntegerField(_child_property_offset, _NO_INDEX,
                _raw_data);
    ...
    protected Property(int index, byte[] array, int offset) {
        ...
        if (name_length < 1) {
            ...
            for (int j = 0; j < name_length; j++) {
                char_array[j] = (char) new ShortField(name_offset, _raw_data)
                        .get();
     ...
     * @return true if the length is less than _big_block_minimum_bytes
     */
     ...
     *
     * @param stream the OutputStream to which the data should be written.
    ...
    /**
     * Based on the currently defined size, should this property use small
     * blocks?
     *
    ...
    protected void setName(String name) {
        ...
        _name_size.set((short) ((limit + 1) * LittleEndianConsts.SHORT_SIZE),
                _raw_data);
    }
    ...
    /**
     * Sets the storage clsid, which is the Class ID of a COM object which reads
     * and writes this stream
     *
     ...
     * Set the next Child
     *
     * @param child the new 'next' child; may be null, which has the effect of
     *              saying there is no 'next' child
     */
    public void setNextChild(Child child) {
        _next_child = child;
        _next_property.set(
                (child == null) ? _NO_INDEX : ((Property) child).getIndex(),
                _raw_data);
    }
     ...
     *
     * @param child the new 'previous' child; may be null, which has the effect of
     *              saying there is no 'previous' child
     */
    public void setPreviousChild(Child child) {
        _previous_child = child;
        _previous_property.set(
                (child == null) ? _NO_INDEX : ((Property) child).getIndex(),
                _raw_data);
    }
