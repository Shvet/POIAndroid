...
import com.shvet.poi.poifs.filesystem.NotOLE2FileException;
import com.shvet.poi.poifs.filesystem.OfficeXmlFileException;
import com.shvet.poi.util.HexDump;
import com.shvet.poi.util.IOUtils;
import com.shvet.poi.util.IntegerField;
import com.shvet.poi.util.LittleEndian;
import com.shvet.poi.util.LittleEndianConsts;
import com.shvet.poi.util.LongField;
import com.shvet.poi.util.ShortField;
...
public final class HeaderBlock implements HeaderBlockConstants {
    ...
    /**
     * The data. Only ever 512 bytes, because 4096 byte files use zeros for the
     * extra header space.
     */
    private final byte[] _data;
    /**
     * Number of big block allocation table blocks (int). (Number of FAT Sectors
     * in Microsoft parlance).
     */
    private int _bat_count;
    /**
     * Start of the property set block (int index of the property set chain's
     * first big block).
     */
    private int _property_start;
    /**
     * start of the small block allocation table (int index of small block
     * allocation table's first big block)
     */
    private int _sbat_start;
    /**
     * Number of small block allocation table blocks (int) (Number of MiniFAT
     * Sectors in Microsoft parlance)
     */
    ...
    /**
     * Number of big block allocation table blocks (int) (Number of DIFAT
     * Sectors in Microsoft parlance)
     */
    ...
    private HeaderBlock(byte[] data) throws IOException {
        ...
        if (signature != _signature) {
            ...
            if (_data[0] == 0x09 && _data[1] == 0x02
                ...
                // BIFF3 raw stream
                throw new OldExcelFormatException(
                        "The supplied data appears to be in BIFF3 format. "
                                + "HSSF only supports the BIFF8 format, try OldExcelExtractor");
            }
            if (_data[0] == 0x09 && _data[1] == 0x04 && // sid=0x0409
                    _data[2] == 0x06 && _data[3] == 0x00 && // size=0x0006
                    _data[4] == 0x00 && _data[5] == 0x00) { // unused
                if (((_data[6] == 0x10 || _data[6] == 0x20 || _data[6] == 0x40) && _data[7] == 0x00)
                        || (_data[6] == 0x00 && _data[7] == 0x01)) {
                    // BIFF4 raw stream
                    throw new OldExcelFormatException(
                            "The supplied data appears to be in BIFF4 format. "
                                    + "HSSF only supports the BIFF8 format, try OldExcelExtractor");
                }
            }
    ...
    private static IOException alertShortRead(int pRead, int expectedReadSize) {
        ...
        return new IOException("Unable to read entire header; " + read + type
                + " read; expected " + expectedReadSize + " bytes");
    }
     ...
     *
     * @param startBlock the index of the first big block of the small block allocation
     *                   table
     */
    ...
    /**
     * Sets the number of BAT blocks that are used. This is the number used in
     * both the BAT and XBAT.
     */
    ...
    /**
     * Returns the offsets to the first (up to) 109 BAT sectors. Any additional
     * BAT sectors are held in the XBAT (DIFAT) sectors in a chain.
     *
    ...
    void writeData(final OutputStream stream) throws IOException {
        ...
        for (int i = POIFSConstants.SMALLER_BIG_BLOCK_SIZE; i < bigBlockSize
                .getBigBlockSize(); i++) {
            stream.write(0);
        }
    }
