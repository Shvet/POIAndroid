...
import com.shvet.poi.poifs.filesystem.BlockStore.ChainLoopDetector;
...
/**
 * This handles reading and writing a stream within a {@link NPOIFSFileSystem}.
 * It can supply an iterator to read blocks, and way to write out to existing
 * and new blocks. Most users will want a higher level version of this, which
 * deals with properties to track which stream this is. This only works on big
 * block streams, it doesn't handle small block ones. This uses the new NIO code
 * <p/>
...
public class NPOIFSStream implements Iterable<ByteBuffer> {
    ...
    /**
     * Constructor for an existing stream. It's up to you to know how to get the
     * start block (eg from a {@link HeaderBlock} or a {@link Property})
     */
    ...
    /**
     * Constructor for a new stream. A start block won't be allocated until you
     * begin writing to it.
     */
     ...
     * {@link POIFSConstants#END_OF_CHAIN} for a new stream that hasn't been
     * written to yet.
     */
    ...
    /**
     * Returns an iterator that'll supply one {@link ByteBuffer} per block in
     * the stream.
     */
    ...
    /**
     * Updates the contents of the stream to the new set of bytes. Note - if
     * this is property based, you'll still need to update the size in the
     * property yourself
     */
     ...
     */
    protected class StreamBlockByteBufferIterator implements
            Iterator<ByteBuffer> {
