...
/**
 * A DirectoryEntry filter, which exposes another DirectoryEntry less certain
 * parts. This is typically used when copying or comparing Filesystems.
 */
public class FilteringDirectoryNode implements DirectoryEntry {
    ...
    /**
     * Creates a filter round the specified directory, which will exclude
     * entries such as "MyNode" and "MyDir/IgnoreNode". The excludes can stretch
     * into children, if they contain a /.
     *
    ...
    private Entry wrapEntry(Entry entry) {
        ...
        if (childExcludes.containsKey(name) && entry instanceof DirectoryEntry) {
            return new FilteringDirectoryNode((DirectoryEntry) entry,
                    childExcludes.get(name));
        }
        return entry;
    }
