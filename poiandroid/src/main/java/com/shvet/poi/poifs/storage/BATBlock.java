...
package com.shvet.poi.poifs.storage;
...
/**
 * A block of block allocation table entries. BATBlocks are created only through
 * a static factory method: createBATBlocks.
 */
public final class BATBlock extends BigBlock {
    /**
     * For a regular fat block, these are 128 / 1024 next sector values. For a
     * XFat (DIFat) block, these are 127 / 1023 next sector values, then a
     * chaining value.
     */
    private int[] _values;
     ...
     * @param start_index the index of the first entry to be written to the block
     * @param end_index   the index, plus one, of the last entry to be written to the
     *                    block (writing is for all index k, start_index <= k <
     *                    end_index)
     */
     ...
     */
    public static BATBlock createBATBlock(final POIFSBigBlockSize bigBlockSize,
                                          ByteBuffer data) {
     ...
     */
    public static BATBlock createEmptyBATBlock(
            final POIFSBigBlockSize bigBlockSize, boolean isXBAT) {
        BATBlock block = new BATBlock(bigBlockSize);
        if (isXBAT) {
    ...
    /**
     * Create an array of BATBlocks from an array of int block allocation table
     * entries
     *
     * @param entries the array of int entries
     * @return the newly created array of BATBlocks
     */
    public static BATBlock[] createBATBlocks(
            final POIFSBigBlockSize bigBlockSize, final int[] entries) {
        int block_count = calculateStorageRequirements(bigBlockSize,
                entries.length);
        ...
        for (int j = 0; j < entries.length; j += _entries_per_block) {
            blocks[index++] = new BATBlock(bigBlockSize, entries, j,
                    (remaining > _entries_per_block) ? j + _entries_per_block
                            : entries.length);
    ...
    /**
     * Create an array of XBATBlocks from an array of int block allocation table
     * entries
     *
     * @param entries    the array of int entries
    ...
    public static BATBlock[] createXBATBlocks(
            final POIFSBigBlockSize bigBlockSize, final int[] entries,
            final int startBlock) {
        int block_count = calculateXBATStorageRequirements(bigBlockSize,
                entries.length);
        BATBlock[] blocks = new BATBlock[block_count];
        int index = 0;
        int remaining = entries.length;
        ...
        if (block_count != 0) {
            for (int j = 0; j < entries.length; j += _entries_per_xbat_block) {
                blocks[index++] = new BATBlock(bigBlockSize, entries, j,
                        (remaining > _entries_per_xbat_block) ? j
                                + _entries_per_xbat_block : entries.length);
                remaining -= _entries_per_xbat_block;
            }
            for (index = 0; index < blocks.length - 1; index++) {
                blocks[index]
                        .setXBATChain(bigBlockSize, startBlock + index + 1);
            }
            blocks[index].setXBATChain(bigBlockSize,
    ...
    /**
     * Calculate how many BATBlocks are needed to hold a specified number of BAT
     * entries.
     *
     * @param entryCount the number of entries
     * @return the number of BATBlocks needed
     */
    public static int calculateStorageRequirements(
            final POIFSBigBlockSize bigBlockSize, final int entryCount) {
        int _entries_per_block = bigBlockSize.getBATEntriesPerBlock();
        return (entryCount + _entries_per_block - 1) / _entries_per_block;
    }

    /**
     * Calculate how many XBATBlocks are needed to hold a specified number of
     * BAT entries.
     *
     * @param entryCount the number of entries
     * @return the number of XBATBlocks needed
     */
    public static int calculateXBATStorageRequirements(
            final POIFSBigBlockSize bigBlockSize, final int entryCount) {
        int _entries_per_xbat_block = bigBlockSize.getXBATEntriesPerBlock();
     ...
     * number of FAT (BAT) sectors specified. (We don't care if those BAT blocks
     * come from the 109 in the header, or from header + XBATS, it won't affect
     * the calculation)
     * <p/>
     * The actual file size will be between [size of fatCount-1 blocks] and
     * [size of fatCount blocks]. For 512 byte block sizes, this means we may
     * over-estimate by up to 65kb. For 4096 byte block sizes, this means we may
     * over-estimate by up to 4mb
     */
    public static long calculateMaximumSize(
            final POIFSBigBlockSize bigBlockSize, final int numBATs) {
    ...
    public static long calculateMaximumSize(final HeaderBlock header) {
        return calculateMaximumSize(header.getBigBlockSize(),
                header.getBATCount());
    }

    /**
     * Returns the BATBlock that handles the specified offset, and the relative
     * index within it. The List of BATBlocks must be in sequential order
     */
    public static BATBlockAndIndex getBATBlockAndIndex(final int offset,
        ...
        int whichBAT = (int) Math.floor(offset
    ...
    /**
     * Returns the BATBlock that handles the specified offset, and the relative
     * index within it, for the mini stream. The List of BATBlocks must be in
     * sequential order
     */
    public static BATBlockAndIndex getSBATBlockAndIndex(final int offset,
        ...
        int whichSBAT = (int) Math.floor(offset
                / bigBlockSize.getBATEntriesPerBlock());
        int index = offset % bigBlockSize.getBATEntriesPerBlock();
    ...
    private void setXBATChain(final POIFSBigBlockSize bigBlockSize,
                              int chainIndex) {
        int _entries_per_xbat_block = bigBlockSize.getXBATEntriesPerBlock();
     ...
     * {@link #hasFreeSectors()} is much quicker
     */
    ...
    public int getValueAt(int relativeOffset) {
        if (relativeOffset >= _values.length) {
            throw new ArrayIndexOutOfBoundsException("Unable to fetch offset "
                    + relativeOffset + " as the " + "BAT only contains "
                    + _values.length + " entries");
        }
     ...
     * @throws IOException on problems writing to the specified stream
     */
