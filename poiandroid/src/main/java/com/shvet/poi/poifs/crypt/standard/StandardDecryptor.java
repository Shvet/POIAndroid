...
import com.shvet.poi.poifs.crypt.EncryptionInfoBuilder;
...
import com.shvet.poi.util.BoundedInputStream;
...
public class StandardDecryptor extends Decryptor {
    ...
    protected static SecretKey generateSecretKey(String password,
                                                 EncryptionVerifier ver, int keySize) {
        HashAlgorithm hashAlgo = ver.getHashAlgorithm();

        byte pwHash[] = CryptoFunctions.hashPassword(password, hashAlgo,
                ver.getSalt(), ver.getSpinCount());

        byte[] blockKey = new byte[4];
        LittleEndian.putInt(blockKey, 0, 0);

        byte[] finalHash = CryptoFunctions.generateKey(pwHash, hashAlgo,
                blockKey, hashAlgo.hashSize);
    ...
    protected static byte[] fillAndXor(byte hash[], byte fillByte) {
        ...
        MessageDigest sha1 = CryptoFunctions
                .getMessageDigest(HashAlgorithm.sha1);
    ...
    public boolean verifyPassword(String password) {
        ...
        try {
            ...
            MessageDigest sha1 = CryptoFunctions.getMessageDigest(ver
                    .getHashAlgorithm());
            byte[] calcVerifierHash = sha1.digest(verifier);
            byte encryptedVerifierHash[] = ver.getEncryptedVerifierHash();
            byte decryptedVerifierHash[] = cipher
                    .doFinal(encryptedVerifierHash);

            // see 2.3.4.9 Password Verification (Standard Encryption)
            // ... The number of bytes used by the encrypted Verifier hash MUST
            // be 32 ...
            // TODO: check and trim/pad the hashes to 32
            byte[] verifierHash = Arrays.copyOf(decryptedVerifierHash,
    ...
    public InputStream getDataStream(DirectoryNode dir) throws IOException {
        DocumentInputStream dis = dir
                .createDocumentInputStream(DEFAULT_POIFS_ENTRY);

        _length = dis.readLong();

        // limit wrong calculated ole entries - (bug #57080)
        // standard encryption always uses aes encoding, so blockSize is always
        // 16
        ...
        return new BoundedInputStream(
                new CipherInputStream(boundedDis, cipher), _length);
    }

    /**
     * @return the length of the stream returned by
     * {@link #getDataStream(DirectoryNode)}
     */
    public long getLength() {
        if (_length == -1)
            throw new IllegalStateException(
                    "Decryptor.getDataStream() was not called");
        return _length;
    }
}
