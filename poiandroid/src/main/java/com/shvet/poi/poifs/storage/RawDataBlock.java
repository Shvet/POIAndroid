...
public class RawDataBlock implements ListManagedBlock {
     ...
     * @throws IOException on I/O errors, and if an insufficient amount of data is
     *                     read (the InputStream must be an exact multiple of the
     *                     block size)
     */
    ...
    public RawDataBlock(final InputStream stream, int blockSize)
        ...
        if (count == -1) {
            ...
            log.log(POILogger.ERROR, "Unable to read entire block; " + count
                    + type + " read before EOF; expected " + blockSize
                    + " bytes. Your document "
     ...
     *
     * @return true if the EoF was hit during this block, or false if not. If
     * you have a dodgy short last block, then it's possible to both
     * have data, and also hit EoF...
     */
    public boolean eof() {
        return _eof;
    }
    ...
    /**
     * Did we actually find any data to read? It's possible, in the event of a
     * short last block, to both have hit the EoF, but also to have data
     */
