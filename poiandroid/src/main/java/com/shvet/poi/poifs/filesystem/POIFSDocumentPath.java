...
public class POIFSDocumentPath {
    private static final POILogger log = POILogFactory
            .getLogger(POIFSDocumentPath.class);

    private String[] components;
    private int hashcode = 0;

    /**
     * constructor for the path of a document that is not in the root of the
     * POIFSFileSystem
     *
     * @param components the Strings making up the path to a document. The Strings must
     *                   be ordered as they appear in the directory hierarchy of the
     *                   the document -- the first string must be the name of a
     *                   directory in the root of the POIFSFileSystem, and every Nth
     *                   (for N > 1) string thereafter must be the name of a directory
     *                   in the directory identified by the (N-1)th string.
     *                   <p/>
     *                   If the components parameter is null or has zero length, the
     *                   POIFSDocumentPath is appropriate for a document that is in the
     *                   root of a POIFSFileSystem
     * @throws IllegalArgumentException if any of the elements in the components parameter are
    ...
    /**
     * simple constructor for the path of a document that is in the root of the
     * POIFSFileSystem. The constructor that takes an array of Strings can also
     * be used to create such a POIFSDocumentPath by passing it a null or empty
     * String array
     */
     ...
     * @throws IllegalArgumentException if any of the Strings in components is null or zero length
     */

    public POIFSDocumentPath(final POIFSDocumentPath path,
                             final String[] components) throws IllegalArgumentException {
        if (components == null) {
    ...
    /**
     * equality. Two POIFSDocumentPath instances are equal if they have the same
     * number of component Strings, and if each component String is equal to its
     * coresponding component String
     *
    ...
    /**
     * <p>
     * Returns the path's parent or <code>null</code> if this path is the root
     * path.
     * </p>
     *
    ...
    /**
     * <p>
     * Returns a string representation of the path. Components are separated by
