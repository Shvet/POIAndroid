...
public class StandardEncryptionInfoBuilder implements EncryptionInfoBuilder {
    ...
    public void initialize(EncryptionInfo info, LittleEndianInput dis)
        ...
        if (info.getVersionMinor() == 2
                && (info.getVersionMajor() == 3 || info.getVersionMajor() == 4)) {
    ...
    public void initialize(EncryptionInfo info,
        ...
        if (cipherAlgorithm != CipherAlgorithm.aes128
            ...
            throw new EncryptedDocumentException(
                    "Standard encryption only supports AES128/192/256.");
        }
        ...
        if (hashAlgorithm != HashAlgorithm.sha1) {
            throw new EncryptedDocumentException(
                    "Standard encryption only supports SHA-1.");
        }
        ...
        if (!found) {
            throw new EncryptedDocumentException("KeySize " + keyBits
                    + " not allowed for Cipher " + cipherAlgorithm.toString());
        }
        header = new StandardEncryptionHeader(cipherAlgorithm, hashAlgorithm,
                keyBits, blockSize, chainingMode);
        verifier = new StandardEncryptionVerifier(cipherAlgorithm,
                hashAlgorithm, keyBits, blockSize, chainingMode);
        decryptor = new StandardDecryptor(this);
