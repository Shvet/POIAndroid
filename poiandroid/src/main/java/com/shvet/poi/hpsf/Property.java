...
import com.shvet.poi.util.CodePageUtil;
 ...
 * context of its {@link Section}. Each {@link Section} spans its own name space
 * of property IDs.
 * </p>
 * <p/>
 * <p>
 * The property's <strong>type</strong> determines how its <strong>value
 * </strong> is interpreted. For example, if the type is
 * {@link Variant#VT_LPSTR} (byte string), the value consists of a DWord telling
 * how many bytes the string contains. The bytes follow immediately, including
 * any null bytes that terminate the string. The type {@link Variant#VT_I4}
 * denotes a four-byte integer value, {@link Variant#VT_FILETIME} some date and
 * time (of a file).
 * </p>
 * <p/>
 * <p>
 * Please note that not all {@link Variant} types yet. This might change over
 * time but largely depends on your feedback so that the POI team knows which
 * variant types are really needed. So please feel free to submit error reports
 * or patches for the types you need.
 * </p>
 * <p/>
 * <p/>
 * Microsoft documentation: <a href=
 * "http://msdn.microsoft.com/library/en-us/stg/stg/property_set_display_name_dictionary.asp?frame=true"
...
public class Property {
     ...
     * @param codepage The section's and thus the property's codepage. It is needed
     *                 only when reading string values.
     * @throws UnsupportedEncodingException if the specified codepage is not supported.
     */
    public Property(final long id, final byte[] src, final long offset,
		...
		/*
         * ID 0 is a special case since it specifies a dictionary of property
    ...
    /**
     * <p>
     * Creates an empty property. It must be filled using the set method to be
     * usable.
     * </p>
     */
    protected Property() {
     ...
     * @param offset   At this offset within <var>src </var> the dictionary starts.
     * @param length   The dictionary contains at most this many bytes.
     * @param codepage The codepage of the string values.
     * @return The dictonary
     * @throws UnsupportedEncodingException if the dictionary's codepage is not (yet) supported.
     */
    protected Map<?, ?> readDictionary(final byte[] src, final long offset,
        ...
        if (offset < 0 || offset > src.length)
            throw new HPSFRuntimeException("Illegal offset " + offset
                    + " while HPSF stream contains " + length + " bytes.");
        int o = (int) offset;
        ...
        try {
            for (int i = 0; i < nrEntries; i++) {
				...
				/*
				 * The value (a string). The length is the either the number of
				 * (two-byte) characters if the character set is Unicode or the
				 * number of bytes if the character set is not Unicode. The
				 * length includes terminating 0x00 bytes which we have to strip
                ...
                switch (codepage) {
					 ...
					 * bytes.
					 */
                        b.append(new String(src, o, (int) sLength, Charset
                                .forName("ASCII")));
                        break;
                    }
					 ...
					 */
                        ...
                        b.append(new String(h, 0, nrBytes, CodePageUtil
					 ...
					 * of bytes.
					 */
                        b.append(new String(src, o, (int) sLength, VariantSupport
                                .codepageToEncoding(codepage)));
                        break;
                    }
                }
     ...
     * @throws WritingNotSupportedException if HPSF does not yet support the property's variant type.
     */
     ...
     * </p>
     * <p>
     * Please beware that a property with ID == 0 is a special case: It does not
     * have a type, and its value is the section's dictionary. Another special
     * case are strings: Two properties may have the different types
     * Variant.VT_LPSTR and Variant.VT_LPWSTR;
     * </p>
     *
    ...
    private boolean typesAreEqual(final long t1, final long t2) {
        return t1 == t2 || (t1 == Variant.VT_LPSTR && t2 == Variant.VT_LPWSTR)
