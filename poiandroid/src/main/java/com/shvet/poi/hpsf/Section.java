...
import com.shvet.poi.hpsf.wellknown.PropertyIDMap;
...
public class Section {
     ...
     * found in the property with ID 0.
     * </p>
     */
    ...
    /**
     * <p>
     * This member is <code>true</code> if the last call to
     * {@link #getPropertyIntValue} or {@link #getProperty} tried to access a
     ...
     * @param offset The position in the stream that points to the section's format
     *               ID.
     * @throws UnsupportedEncodingException if the section's codepage is not supported.
     */
    public Section(final byte[] src, final int offset)
		...
		/*
         * Read the offset from the stream's start and positions to the section
		 * header.
		 */
		...
		/*
		 * Read the properties. The offset is positioned at the first entry of
		 * the property list. There are two problems:
		 *
		 * 1. For each property we have to find out its length. In the property
		 * list we find each property's ID and its offset relative to the
		 * section's beginning. Unfortunately the properties in the property
		 * list need not to be in ascending order, so it is not possible to
		 * calculate the length as (offset of property(i+1) - offset of
		 * property(i)). Before we can that we first have to sort the property
		 * list by ascending offsets.
		 *
		 * 2. We have to read the property with ID 1 before we read other
		 * properties, at least before other properties containing strings. The
		 * reason is that property 1 specifies the codepage. If it is 1200, all
		 * strings are in Unicode. In other words: Before we can read any
		 * strings we have to know whether they are in Unicode or not.
		 * Unfortunately property 1 is not guaranteed to be the first in a
		 * section.
		 *
		 * The algorithm below reads the properties in two passes: The first one
		 * looks for property ID 1 and extracts the codepage number. The seconds
		 * pass reads the other properties.
		 */
		...
		/*
		 * Pass 2: Read all properties - including the codepage property, if
		 * available.
		 */
        int i1 = 0;
        for (final Iterator<PropertyListEntry> i = propertyList.iterator(); i
                .hasNext(); ) {
            ple = i.next();
            Property p = new Property(ple.id, src, this.offset + ple.offset,
                    ple.length, codepage);
            if (p.getID() == PropertyIDMap.PID_CODEPAGE)
                p = new Property(p.getID(), p.getType(),
    ...
    /**
     * <p>
     * Returns the format ID. The format ID is the "type" of the section. For
     * example, if the format ID of the first {@link Section} contains the bytes
     * specified by
     ...
     * {@link #wasNull} will return <code>true</code>.
     * </p>
     *
     ...
     * property is not available, 0 is returned. A subsequent call to
     * {@link #wasNull} will return <code>true</code> to let the caller
     * distinguish that case from a real property value of 0.
     * </p>
     *
     * @param id The property's ID
     * @return The property's value
     ...
     * looked up in the {@link Section}'s private dictionary. If it is not found
     * there, the method calls {@link SectionIDMap#getPIDString}.
     * </p>
     *
     ...
     * <li>
     * <p>
     * The sections have a different number of properties. However, properties
     * with ID 1 (codepage) are not counted.
     * </p>
     * </li>
     * <p/>
     * <li>
     ...
     * <li>
     * <p>
     * The properties have different values. The order of the properties is
     * irrelevant.
     * </p>
     * </li>
     * <p/>
    ...
    /**
     * <p>
     * Gets the section's dictionary. A dictionary allows an application to use
     * human-readable property names instead of numeric property IDs. It
     * contains mappings from property IDs to their associated string values.
     * The dictionary is stored as the property with ID 0. The codepage for the
     * strings in the dictionary is defined by property with ID 1.
     * </p>
     *
