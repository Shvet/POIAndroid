...
class UnicodeString {
    ...
    UnicodeString(byte[] data, int offset) {
        ...
        if (!validLength(length, data, dataOffset)) {
            ...
            // sometimes expected
            // to be on a 4 byte boundary. Try checking with that if so, rather
            // than blowing up with
    ...
    /**
     * Checks to see if the specified length seems valid, given the amount of
     * data available still to read, and the requirement that the string be
     * NULL-terminated
     */
