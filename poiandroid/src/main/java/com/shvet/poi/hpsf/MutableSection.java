...
public class MutableSection extends Section {
    /**
     * <p>
     * If the "dirty" flag is true, the section's size must be (re-)calculated
    ...
    /**
     * <p>
     * List to assemble the properties. Unfortunately a wrong decision has been
     * taken when specifying the "properties" field as an Property[]. It should
     * have been a {@link java.util.List}.
     * </p>
     */
    ...
    /**
     * <p>
     * Contains the bytes making out the section. This byte array is established
     * when the section's size is calculated and can be reused later. It is
     * valid only if the "dirty" flag is false.
     * </p>
     */
    ...
    /**
     * <p>
     * Constructs a <code>MutableSection</code> by doing a deep copy of an
    ...
    private static int writeDictionary(final OutputStream out,
        ...
        for (final Iterator<Long> i = dictionary.keySet().iterator(); i
                .hasNext(); ) {
            ...
            if (codepage == CodePageUtil.CP_UNICODE) {
                ...
                final byte[] ca = CodePageUtil.getBytesInCodePage(value,
                        codepage);
                ...
                /*
				 * Write the dictionary item in another codepage than Unicode.
				 */
                length += TypeWriter.writeUIntToStream(out, key.longValue());
                length += TypeWriter.writeUIntToStream(out, value.length() + 1);
                final byte[] ba = CodePageUtil.getBytesInCodePage(value,
                        codepage);
     ...
     * </p>
     *
     * @param formatID The section's format ID as a byte array. It components are in
     *                 big-endian format.
     ...
     * @param value The property's value. It will be written as a Unicode string.
    ...
    /**
     * <p>
     * Sets the value and the variant type of the property with the specified
     * ID. If a property with this ID is not yet present in the section, it will
     * be added. An already present property with the specified ID will be
     * overwritten. A default mapping will be used to choose the property's
     * type.
     * </p>
     *
     * @param id          The property's ID.
     ...
     * <p/>
     * <p>
     * Internally this is done by writing into three byte array output streams:
     * one for the properties, one for the property list and one for the section
     * as such. The two former are appended to the latter when they have
     ...
     * @throws WritingNotSupportedException if HPSF does not yet support writing a property's variant
     *                                      type.
     */
    public int write(final OutputStream out)
		 ...
		 */
        position += 2 * LittleEndian.INT_SIZE + getPropertyCount() * 2
		 ...
		 * streams:
		 */
        for (final ListIterator<Property> i = preprops.listIterator(); i
                .hasNext(); ) {
			 ...
			 * dictionary which has an implicit type only and an explicit value.
			 */
            if (id != 0)
				/*
    ...
    /**
     * <p>
     * Overwrites the super class' method to cope with a redundancy: the
     * property count is maintained in a separate member variable, but
     * shouldn't.
     * </p>
     *
     ...
     * effects. An application should never do this but always use this method.
     * </p>
     *
     * @param dictionary The dictionary
     * @throws IllegalPropertySetDataException if the dictionary's key and value types are not correct.
    ...
    public void setDictionary(final Map<Long, String> dictionary)
        ...
        if (dictionary != null) {
			 ...
			 * used in the dictionary is not yet defined, set it to Unicode.
			 */
    ...
    /**
     * <p>
     * Removes all properties from the section including 0 (dictionary) and 1
     * (codepage).
     * </p>
     */
    public void clear() {
