 ...
 * the easy-to-use {@link SummaryInformation} and
 * {@link DocumentSummaryInformation} classes. When parsing the data a property
 * set stream consists of (possibly coming from an {@link java.io.InputStream})
 * we want to read and process each byte only once. Since we don't know in
 * advance which kind of property set we have, we can expect only the most
 * general {@link PropertySet}. Creating a special subclass should be as easy as
 * calling the special subclass' constructor and pass the general
 * {@link PropertySet} in. To make things easy internally, the special class
 * just holds a reference to the general {@link PropertySet} and delegates all
 * method calls to it.
 * </p>
 * <p/>
 * <p>
 * A cleaner implementation would have been like this: The
 * {@link PropertySetFactory} parses the stream data into some internal object
 * first. Then it finds out whether the stream is a {@link SummaryInformation},
 * a {@link DocumentSummaryInformation} or a general {@link PropertySet}.
...
public abstract class SpecialPropertySet extends MutablePropertySet {
    /**
     * <p>
     * The "real" property set <code>SpecialPropertySet</code> delegates to.
     * </p>
     */
    ...
    /**
     * <p/>
     * Creates a <code>SpecialPropertySet</code>.
     *
    ...
    /**
     * @see org.apache.poi.hpsf.MutablePropertySet#write(org.apache.poi.poifs.filesystem.DirectoryEntry,
     * java.lang.String)
     */
    ...
    /**
     * Fetches the property with the given ID, then does its best to return it
     * as a String
     *
