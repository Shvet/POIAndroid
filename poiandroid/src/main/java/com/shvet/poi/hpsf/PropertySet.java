...
package com.shvet.poi.hpsf;

import com.shvet.poi.hpsf.wellknown.SectionIDMap;
 ...
 * are usually metadata of a Microsoft Office document.
 * </p>
 * <p/>
 * <p>
 * An application that wants to access these metadata should create an instance
 * of this class or one of its subclasses by calling the factory method
 * {@link PropertySetFactory#create} and then retrieve the information its needs
 * by calling appropriate methods.
 * </p>
 * <p/>
 * <p>
 * {@link PropertySetFactory#create} does its work by calling one of the
 * constructors {@link PropertySet#PropertySet(InputStream)} or
 * {@link PropertySet#PropertySet(byte[])}. If the constructor's argument is not
 * in the Horrible Property Set Format, i.e. not a property set stream, or if
 * any other error occurs, an appropriate exception is thrown.
 * </p>
 * <p/>
 * <p>
 * A {@link PropertySet} has a list of {@link Section}s, and each
 * {@link Section} has a {@link Property} array. Use {@link #getSections} to
 * retrieve the {@link Section}s, then call {@link Section#getProperties} for
 * each {@link Section} to get hold of the {@link Property} arrays.
 * </p>
 * Since the vast majority of {@link PropertySet}s contains only a single
 * {@link Section}, the convenience method {@link #getProperties} returns the
 * properties of a {@link PropertySet}'s {@link Section} (throwing a
 * {@link NoSingleSectionException} if the {@link PropertySet} contains more (or
 * less) than exactly one {@link Section}).</p>
 *
...
public class PropertySet {
     ...
     * </p>
     */
    static final byte[] BYTE_ORDER_ASSERTION = new byte[]{(byte) 0xFE,
            (byte) 0xFF};
    /**
     * <p>
     ...
     * </p>
     */
    static final byte[] FORMAT_ASSERTION = new byte[]{(byte) 0x00,
            (byte) 0x00};
    /**
     * <p>
     * Specifies this {@link PropertySet}'s byte order. See the HPFS
    ...
    /**
     * <p>
     * Specifies this {@link PropertySet}'s format. See the HPFS documentation
     * for details!
     * </p>
     */
    ...
    /**
     * <p>
     * Specifies this {@link PropertySet}'s "classID" field. See the HPFS
     * documentation for details!
     * </p>
     */
     ...
     * become public once the property set's writing functionality is
     * implemented.
     * </p>
     */
    ...
    /**
     * <p>
     * Creates a {@link PropertySet} instance from an {@link InputStream} in the
     ...
     * </p>
     *
     * @param stream Holds the data making out the property set stream.
     * @throws MarkUnsupportedException     if the stream does not support the
     *                                      {@link InputStream#markSupported} method.
     * @throws IOException                  if the {@link InputStream} cannot not be accessed as needed.
     * @throws NoPropertySetStreamException if the input stream does not contain a property set.
     * @throws UnsupportedEncodingException if a character encoding is not supported.
     */
     ...
     * @param offset The offset in <var>stream</var> where the stream data begin.
     *               If the stream data begin with the first byte in the array, the
     *               <var>offset</var> is 0.
     * @param length The length of the stream data.
     * @throws NoPropertySetStreamException if the byte array is not a property set stream.
     ...
     * </p>
     *
     * @param stream The byte array holding the stream data. The complete byte
     *               array contents is the stream data.
     * @throws NoPropertySetStreamException if the byte array is not a property set stream.
     * @throws UnsupportedEncodingException if the codepage is not supported.
     */
     ...
     * </p>
     *
     * @param stream The {@link InputStream} to check. In order to perform the
     *               check, the method reads the first bytes from the stream. After
     *               reading, the stream is reset to the position it had before
     *               reading. The {@link InputStream} must support the
     *               {@link InputStream#mark} method.
     * @return <code>true</code> if the stream is a property set stream, else
     * <code>false</code>.
     * @throws MarkUnsupportedException if the {@link InputStream} does not support the
     *                                  {@link InputStream#mark} method.
    ...
    public static boolean isPropertySetStream(final InputStream stream)
		...
		/*
		 * Mark the current position in the stream so that we can reset to this
		 * position if the stream does not contain a property set.
		 */
        ...
        final int bytes = stream.read(buffer, 0,
                Math.min(buffer.length, stream.available()));
        final boolean isPropertySetStream = isPropertySetStream(buffer, 0,
                bytes);
        stream.reset();
     ...
     * @param length The significant number of bytes in the byte array. Only this
     *               number of bytes will be checked.
     * @return <code>true</code> if the byte array is a property set stream,
     ...
     * @param offset The property set stream starts at this offset from the
     *               beginning of <var>src</var>
     * @param length Length of the property set stream.
     * @throws UnsupportedEncodingException if HPSF does not (yet) support the property set's character
     *                                      encoding.
     */
    private void init(final byte[] src, final int offset, final int length)
		...
		/*
		 * Read the sections, which are following the header. They start with an
		 * array of section descriptions. Each one consists of a format ID
		 * telling what the section contains and an offset telling how many
		 * bytes from the start of the stream the section begins.
		 */
		/*
		 * Most property sets have only one section. The Document Summary
		 * Information stream has 2. Everything else is a rare exception and is
		 * no longer fostered by Microsoft.
		 */
        sections = new ArrayList<Section>(sectionCount);

		/*
		 * Loop over the section descriptor array. Each descriptor consists of a
		 * ClassID and a DWord, and we have to increment "offset" accordingly.
		 */
     ...
     * </p>
     *
     * @return <code>true</code> if this {@link PropertySet} represents a
     * Summary Information, else <code>false</code>.
     */
     ...
     *
     * @return <code>true</code> if this {@link PropertySet} represents a
     * Document Summary Information, else <code>false</code>.
     */
     ...
     * property set. It is a shortcut for getting the {@link PropertySet}'s
     * {@link Section}s list and then getting the {@link Property} array from
     * the first {@link Section}.
     * </p>
     *
     * @return The properties of the only {@link Section} of this
     * {@link PropertySet}.
     * @throws NoSingleSectionException if the {@link PropertySet} has more or less than one
     *                                  {@link Section}.
     */
    ...
    /**
     * <p>
     * Convenience method returning the value of the property with the specified
     * ID. If the property is not available, <code>null</code> is returned and a
     * subsequent call to {@link #wasNull} will return <code>true</code> .
     * </p>
     *
     ...
     * <code>true</code> to let the caller distinguish that case from a real
     * property value of <code>false</code>.
     * </p>
     *
     * @param id The property ID
     * @return The property value
     ...
     * {@link #getPropertyIntValue} or {@link #getProperty} tried to access was
     ...
     * case from a property's real value of 0.
     * </p>
     *
     * @return <code>true</code> if the last call to
     * {@link #getPropertyIntValue} or {@link #getProperty} tried to
     * access a property that was not available, else <code>false</code>
     * .
     * @throws NoSingleSectionException if the {@link PropertySet} has more than one {@link Section}.
     */
    ...
    public Section getFirstSection() {
        if (getSectionCount() < 1)
            throw new MissingSectionException(
                    "Property set does not contain any sections.");
    ...
    public Section getSingleSection() {
        ...
        if (sectionCount != 1)
            throw new NoSingleSectionException("Property set contains "
    ...
    /**
     * <p>
     * Returns <code>true</code> if the <code>PropertySet</code> is equal to the
     * specified parameter, else <code>false</code>.
     * </p>
     *
    ...
    public boolean equals(final Object o) {
        ...
        if (byteOrder1 != byteOrder2 || !classID1.equals(classID2)
                || format1 != format2 || osVersion1 != osVersion2
