...
package com.shvet.poi.hssf.record.common;

import com.shvet.poi.hssf.record.RecordInputStream;
import com.shvet.poi.hssf.record.cont.ContinuableRecordInput;
 ...
 * records. It is considered more desirable then repeating it in all of them.
 * <p/>
 ...
 * v20091214
 */
public class UnicodeString implements Comparable<UnicodeString> { // TODO - make
    ...
    // is
    // removed
    private static POILogger _logger = POILogFactory
    ...
    public UnicodeString(RecordInputStream in) {
        ...
        if (isExtendedText() && (extensionLength > 0)) {
            field_5_ext_rst = new ExtRst(new ContinuableRecordInput(in),
                    extensionLength);
            if (field_5_ext_rst.getDataSize() + 4 != extensionLength) {
                _logger.log(POILogger.WARN, "ExtRst was supposed to be "
                        + extensionLength
     ...
     * because we don't truely understand rich text fields yet it's difficult to
    ...
    public boolean equals(Object o) {
        ...
        // OK lets do this in stages to return a quickly, first check the actual
        // string
        boolean eq = ((field_1_charCount == other.field_1_charCount)
                && (field_2_optionflags == other.field_2_optionflags) && field_3_string
                .equals(other.field_3_string));
        if (!eq)
            return false;

        // OK string appears to be equal but now lets compare formatting runs
        if ((field_4_format_runs == null)
                && (other.field_4_format_runs == null))
    ...
    /**
     * get the number of characters in the string, wrapped as needed to fit
     * within a short
     *
     * @return number of characters
     */
    ...
    /**
     * get the option flags which among other things return if this is a 16-bit
     * or 8 bit string
     *
    ...
    /**
     * set the option flags which among other things return if this is a 16-bit
     * or 8 bit string
     *
    ...
    /**
     * Swaps all use in the string of one font index for use of a different font
     * index. Normally only called when fonts have been removed / re-ordered
     */
    ...
    /**
     * Serialises out the String. There are special rules about where we can and
     * can't split onto Continue records.
     */
    ...
    public Object clone() {
        ...
        if (field_4_format_runs != null) {
            ...
            for (FormatRun r : field_4_format_runs) {
                str.field_4_format_runs.add(new FormatRun(r._character,
                        r._fontIndex));
            }
        }
        if (field_5_ext_rst != null) {
    ...
    public static class ExtRst implements Comparable<ExtRst> {
        ...
        protected ExtRst(LittleEndianInput in, int expectedLength) {
            ...
            if (reserved != 1) {
                _logger.log(POILogger.WARN,
                        "Warning - ExtRst has wrong magic marker, expecting 1 but found "
                                + reserved + " - ignoring");
            ...
            if (extraDataLength < 0) {
                _logger.log(POILogger.WARN, "Warning - ExtRst overran by "
                        + (0 - extraDataLength) + " bytes");
                extraDataLength = 0;
            }
        ...
        protected int getDataSize() {
            return 4 + 6 + (2 * phoneticText.length()) + (6 * phRuns.length)
                    + extraData.length;
        }
        ...
        public int compareTo(ExtRst o) {
            ...
            if (result != 0)
                return result;
            for (int i = 0; i < phRuns.length; i++) {
                result = phRuns[i].phoneticTextFirstCharacterOffset
                        - o.phRuns[i].phoneticTextFirstCharacterOffset;
                if (result != 0)
                    return result;
                result = phRuns[i].realTextFirstCharacterOffset
