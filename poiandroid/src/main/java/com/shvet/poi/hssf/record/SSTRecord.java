...
import com.shvet.poi.util.IntMapper;
...
public final class SSTRecord extends ContinuableRecord {
     ...
     * <p/>
     * The data consists of sets of string data. This string data is arranged as
     * follows:
     * <p/>
     * <CODE><pre>
     ...
     * </TABLE>
     * <p/>
     * We can handle eating the overhead associated with bits 2 or 3 (or both)
     * being set, but we have no idea what to do with the associated data. The
     * UnicodeString class can handle the byte[] vs short[] nature of the actual
     * string data
     *
    ...
    public SSTRecord(RecordInputStream in) {
        ...
        deserializer = new SSTDeserializer(field_3_strings);
        // Bug 57456: some Excel Sheets send 0 as field=1, but have some random
        // number in field_2,
    ...
    protected void serialize(ContinuableRecordOutput out) {
        SSTSerializer serializer = new SSTSerializer(field_3_strings,
                getNumStrings(), getNumUniqueStrings());
    ...
    /**
     * Creates an extended string record based on the current contents of the
     * current SST record. The offset within the stream to the SST record is
     * required because the extended string record points directly to the
     * strings in the SST record.
     * <p/>
     * NOTE: THIS FUNCTION MUST ONLY BE CALLED AFTER THE SST RECORD HAS BEEN
     * SERIALIZED.
     *
     * @param sstOffset The offset in the stream to the start of the SST record.
