...
import com.shvet.poi.hssf.record.CFHeaderRecord;
import com.shvet.poi.hssf.record.CalcCountRecord;
import com.shvet.poi.hssf.record.CalcModeRecord;
import com.shvet.poi.hssf.record.CellValueRecordInterface;
import com.shvet.poi.hssf.record.ColumnInfoRecord;
import com.shvet.poi.hssf.record.DVALRecord;
import com.shvet.poi.hssf.record.DefaultColWidthRecord;
import com.shvet.poi.hssf.record.DefaultRowHeightRecord;
import com.shvet.poi.hssf.record.DeltaRecord;
...
import com.shvet.poi.hssf.record.FeatHdrRecord;
import com.shvet.poi.hssf.record.FeatRecord;
import com.shvet.poi.hssf.record.GridsetRecord;
import com.shvet.poi.hssf.record.GutsRecord;
import com.shvet.poi.hssf.record.IndexRecord;
import com.shvet.poi.hssf.record.IterationRecord;
import com.shvet.poi.hssf.record.MergeCellsRecord;
import com.shvet.poi.hssf.record.NoteRecord;
import com.shvet.poi.hssf.record.PaneRecord;
import com.shvet.poi.hssf.record.PrintGridlinesRecord;
import com.shvet.poi.hssf.record.PrintHeadersRecord;
import com.shvet.poi.hssf.record.Record;
import com.shvet.poi.hssf.record.RecordBase;
import com.shvet.poi.hssf.record.RefModeRecord;
import com.shvet.poi.hssf.record.RowRecord;
import com.shvet.poi.hssf.record.SCLRecord;
import com.shvet.poi.hssf.record.SaveRecalcRecord;
import com.shvet.poi.hssf.record.SelectionRecord;
import com.shvet.poi.hssf.record.UncalcedRecord;
import com.shvet.poi.hssf.record.WSBoolRecord;
import com.shvet.poi.hssf.record.WindowTwoRecord;
import com.shvet.poi.hssf.record.aggregates.ChartSubstreamRecordAggregate;
import com.shvet.poi.hssf.record.aggregates.ColumnInfoRecordsAggregate;
import com.shvet.poi.hssf.record.aggregates.ConditionalFormattingTable;
import com.shvet.poi.hssf.record.aggregates.CustomViewSettingsRecordAggregate;
import com.shvet.poi.hssf.record.aggregates.DataValidityTable;
import com.shvet.poi.hssf.record.aggregates.MergedCellsTable;
import com.shvet.poi.hssf.record.aggregates.PageSettingsBlock;
import com.shvet.poi.hssf.record.aggregates.RecordAggregate;
import com.shvet.poi.hssf.record.aggregates.RecordAggregate.PositionTrackingVisitor;
import com.shvet.poi.hssf.record.aggregates.RecordAggregate.RecordVisitor;
import com.shvet.poi.hssf.record.aggregates.RowRecordsAggregate;
...
import com.shvet.poi.util.RecordFormatException;
 ...
 * This file contains the low level binary records starting at the sheets BOF
 * and ending with the sheets EOF. Use HSSFSheet for a high level
 * representation.
 * <p/>
 ...
 * @see com.shvet.poi.hssf.usermodel.HSSFSheet
 */
@Internal
public final class InternalSheet {
    ...
    private InternalSheet(RecordStream rs) {
        ...
        _records = records; // needed here due to calls to
        // findFirstRecordLocBySid before we're done
        int dimsloc = -1;
        ...
        while (rs.hasNext()) {
            ...
            if (RecordOrderer.isRowBlockRecord(recSid)) {
                ...
                if (rra != null) {
                    throw new RuntimeException(
                            "row/cell records found in the wrong place");
                }
            ...
            if (CustomViewSettingsRecordAggregate.isBeginRecord(recSid)) {
                ...
                // Also several times in bugzilla samples 46840-23373 and
                // 46840-23374
            ...
            if (PageSettingsBlock.isComponentRecord(recSid)) {
                if (_psBlock == null) {
                    ...
                    // one or more PSB records found after some intervening
                    // non-PSB records
            ...
            if (recSid == MergeCellsRecord.sid) {
                // when the MergedCellsTable is found in the right place, we
                // expect those records to be contiguous
                _mergedCellsTable.read(rs);
                continue;
            }

            if (recSid == BOFRecord.sid) {
                ChartSubstreamRecordAggregate chartAgg = new ChartSubstreamRecordAggregate(
                        rs);
                // if (false) {
                // // TODO - would like to keep the chart aggregate packed, but
                // one unit test needs attention
        ...
        if (_dimensions == null) {
            // Excel seems to always write the DIMENSION record, but tolerates
            ...
            if (rra == null) {
                ...
                if (log.check(POILogger.WARN)) {
                    log.log(POILogger.WARN,
                            "DIMENSION record not found even though row/cells present");
                }
                // Not sure if any tools write files like this, but Excel reads
                // them OK
            }
        ...
        _rowsAggregate = rra;
        // put merged cells table in the right place (regardless of where the
        // first MergedCellsRecord was found */
    ...
    private InternalSheet() {
        ...
        // 'Worksheet Protection Block' (after 'Page Settings Block' and before
        // DEFCOLWIDTH)
     ...
     * API. Pass in an array of Record objects, the sheet number (0 based) and a
     * record offset (should be the location of the sheets BOF record). A Sheet
     * object is constructed and passed back with all of its initialization set
     * to the passed in records and references to those records held. This
     * function is normally called via Workbook.
     *
    ...
    private static void spillAggregate(RecordAggregate ra,
                                       final List<RecordBase> recs) {
    ...
    /**
     * creates the CalcCount record and sets it to 100 (default number of
     * iterations)
     */
    ...
    /**
     * creates the RefMode record and sets it to A1 Mode (default reference
     * mode)
     */
    ...
    /**
     * creates the Iteration record and sets it to false (don't iteratively
     * calculate formulas)
     */
    ...
    /**
     * creates the SaveRecalc record and sets it to true (recalculate before
     * saving)
     */
    ...
    /**
     * creates the PrintHeaders record and sets it to false (we don't create
     * headers yet so why print them)
     */
    ...
    /**
     * creates the PrintGridlines record and sets it to false (that makes for
     * ugly sheets). As far as I can tell this does the same thing as the
     * GridsetRecord
     */
    ...
    /**
     * creates the Gridset record and sets it to true (user has mucked with the
     * gridlines)
     */
    ...
    /**
     * creates the Guts record and sets leftrow/topcol guttter and
     * rowlevelmax/collevelmax to 0
     */
    ...
    /**
     * creates the DefaultRowHeight Record and sets its options to 0 and
     * rowheight to 0xff
     */
    ...
    /**
     * creates the Dimensions Record and sets it to bogus values (you should set
     * this yourself or let the high level API do it for you)
     */
    ...
    /**
     * Clones the low level records of this sheet and returns the new sheet
     * instance. This method is implemented by adding methods for deep cloning
     * to all records that can be added to a sheet. The <b>Record</b> object
     * does not implement cloneable. When adding a new record, implement a
     * public clone method if and only if the record belongs to a sheet.
     */
    public InternalSheet cloneSheet() {
        ...
        for (int i = 0; i < _records.size(); i++) {
            ...
            if (rb instanceof RecordAggregate) {
                ((RecordAggregate) rb).visitContainedRecords(new RecordCloner(
                        clonedRecords));
                continue;
            }
            if (rb instanceof EscherAggregate) {
                /**
                 * this record will be removed after reading actual data from
                 * EscherAggregate
                 */
    ...
    public void updateFormulasAfterCellShift(FormulaShifter shifter,
        ...
        getRowsAggregate().updateFormulasAfterRowShift(shifter,
                externSheetIndex);
        if (condFormatting != null) {
            getConditionalFormattingTable().updateFormulasAfterCellShift(
                    shifter, externSheetIndex);
        }
    ...
    /**
     * Per an earlier reported bug in working with Andy Khan's excel read
     * library. This sets the values in the sheet's DimensionsRecord object to
     * be correct. Excel doesn't really care, but we want to play nice with
     * other libraries.
     *
     * @see com.shvet.poi.hssf.record.DimensionsRecord
     */
        ...
        if (log.check(POILogger.DEBUG)) {
            ...
            log.log(POILogger.DEBUG,
                    (new StringBuffer("firstrow")).append(firstrow)
                            .append("firstcol").append(firstcol)
                            .append("lastrow").append(lastrow)
                            .append("lastcol").append(lastcol).toString());
        }
        ...
        for (int k = 0; k < _records.size(); k++) {
            ...
            if (record instanceof BOFRecord) {
                if (!haveSerializedIndex) {
                    ...
                    // Can there be more than one BOF for a sheet? If not then
                    // we can
                    ...
                    if (_rowsAggregate != null) {
                        ...
                        ptv.visitRecord(_rowsAggregate.createIndexRecord(
                                currentPos, initRecsSize));
                    }
                }
            }
        }
     ...
     * LabelSSTRecord or NumberRecord).
     * <p/>
     * This method is "loc" sensitive. Meaning you need to set LOC to where you
     * want it to start searching. If you don't know do this:
     * setLoc(getDimsLoc). When adding several rows you can just start at the
     * last one by leaving loc at what this sets it to.
     *
     ...
     * <p/>
     * This method is not loc sensitive, it resets loc to = dimsloc so no
     * worries.
     *
     * @param row - the row of the value record you wish to remove
     * @param col - a record supporting the CellValueRecordInterface.
     * @see com.shvet.poi.hssf.record.CellValueRecordInterface
     */
     ...
     * worries.
     *
     * @param newval - a record supporting the CellValueRecordInterface. this will
     *               replace the cell value with the same row and column. If there
     *               isn't one, one will be added.
     */
    ...
    public void addRow(RowRecord row) {
        ...
        // If the row exists remove it, so that any cells attached to the row
        // are removed
     ...
     * first record (starting at LOC) which is a value record.
     * <p/>
     * <p/>
     * This method is "loc" sensitive. Meaning you need to set LOC to where you
     * want it to start searching. If you don't know do this:
     * setLoc(getDimsLoc). When adding several rows you can just start at the
     * last one by leaving loc at what this sets it to. For this method, set loc
     * to dimsloc to start with, subsequent calls will return values in
     * (physical) sequence or NULL when you get to the end.
     *
     ...
     * setLoc(getDimsLoc). When adding several rows you can just start at the
     * last one by leaving loc at what this sets it to. For this method, set loc
     * to dimsloc to start with. subsequent calls will return rows in (physical)
     * sequence or NULL when you get to the end.
     *
     * @return RowRecord representing the next row record or NULL if there are
     * no more
     */
    ...
    /**
     * get the NEXT (from LOC) RowRecord where rownumber matches the given
     * rownum. The first record that is a Row record (starting at LOC) that has
     * the same rownum as the given rownum will be returned.
     * <p/>
     * This method is "loc" sensitive. Meaning you need to set LOC to where you
     * want it to start searching. If you don't know do this:
     * setLoc(getDimsLoc). When adding several rows you can just start at the
    ...
    public boolean isGridsPrinted() {
        if (gridset == null) {
            gridset = createGridset();
            // Insert the newlycreated Gridset record at the end of the record
            // (just before the EOF)
    ...
    public void setDefaultRowHeight(short dch) {
        ...
        // set the bit that specifies that the default settings for the row
     ...
     * @see com.shvet.poi.hssf.record.DefaultColWidthRecord
     * @see com.shvet.poi.hssf.record.ColumnInfoRecord
    ...
    /**
     * get the index to the ExtendedFormatRecord "associated" with the column at
     * specified 0-based index. (In this case, an ExtendedFormatRecord index is
     * actually associated with a ColumnInfoRecord which spans 1 or more
     * columns) <br/>
     * Returns the index to the default ExtendedFormatRecord (0xF) if no
     * ColumnInfoRecord exists that includes the column index specified.
     *
     * @param columnIndex
     * @return index of ExtendedFormatRecord associated with ColumnInfoRecord
     * that includes the column index or the index of the default
     * ExtendedFormatRecord (0xF)
     */
    ...
    public void setDefaultColumnStyle(int column, int styleIndex) {
        setColumn(column, Short.valueOf((short) styleIndex), null, null, null,
                null);
    }

    private void setColumn(int column, Short xfStyle, Integer width,
                           Integer level, Boolean hidden, Boolean collapsed) {
        _columnInfos
                .setColumn(column, xfStyle, width, level, hidden, collapsed);
    }
     ...
     * @param indent     if true the group will be indented by one level, if false
     *                   indenting will be removed by one level.
     */
     ...
     * @see com.shvet.poi.hssf.record.SelectionRecord
     */
    ...
    public void setActiveCellRow(int row) {
        // shouldn't have a sheet w/o a SelectionRecord, but best to guard
        // anyway
        if (_selection != null) {
     ...
     * @return the record position of the matching record or -1 if no match is
     * made.
     */
     ...
     */
    public void createFreezePane(int colSplit, int rowSplit, int topRow,
                                 int leftmostColumn) {
        int paneLoc = findFirstRecordLocBySid(PaneRecord.sid);
        if (paneLoc != -1)
            _records.remove(paneLoc);

        // If both colSplit and rowSplit are zero then the existing freeze pane
        // is removed
     ...
     */
    public void createSplitPane(int xSplitPos, int ySplitPos, int topRow,
    ...
    public PaneInformation getPaneInformation() {
        ...
        return new PaneInformation(rec.getX(), rec.getY(), rec.getTopRow(),
                rec.getLeftColumn(), (byte) rec.getActivePane(),
                windowTwo.getFreezePanes());
    }
    ...
    /**
     * @param uncalced whether an uncalced record must be inserted or not at
     *                 generation
     */
    ...
    /**
     * Finds the DrawingRecord for our sheet, and attaches it to the
     * DrawingManager (which knows about the overall DrawingGroup for our
     * workbook). If requested, will create a new DrawRecord if none currently
     * exist
     *
     * @param drawingManager  The DrawingManager2 for our workbook
     * @param createIfMissing Should one be created if missing?
     * @return location of EscherAggregate record. if no EscherAggregate record
     * is found return -1
     */
    ...
    public void preSerialize() {
        for (RecordBase r : getRecords()) {
            if (r instanceof EscherAggregate) {
                // Trigger flattening of user model and corresponding update of
                // dgg record.
                r.getRecordSize();
            }
        }
