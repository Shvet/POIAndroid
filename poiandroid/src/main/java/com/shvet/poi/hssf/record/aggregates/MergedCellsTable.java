...
public final class MergedCellsTable extends RecordAggregate {
    private static int MAX_MERGED_REGIONS = 1027; // enforced by the 8224 byte
    // limit
    ...
    public int getRecordSize() {
        ...
        int result = nMergedCellsRecords
                * (4 + CellRangeAddressList.getEncodedSize(MAX_MERGED_REGIONS))
                + 4
    ...
    public void visitContainedRecords(RecordVisitor rv) {
        ...
        for (int i = 0; i < nFullMergedCellsRecords; i++) {
            ...
            rv.visitRecord(new MergeCellsRecord(cras, startIx,
                    MAX_MERGED_REGIONS));
        }
    ...
    private void checkIndex(int index) {
        if (index < 0 || index >= _mergedRegions.size()) {
            throw new IllegalArgumentException("Specified CF index " + index
                    ...
                    + (_mergedRegions.size() - 1) + ")");
        }
    }

    public void addArea(int rowFrom, int colFrom, int rowTo, int colTo) {
        _mergedRegions
                .add(new CellRangeAddress(rowFrom, rowTo, colFrom, colTo));
    }
