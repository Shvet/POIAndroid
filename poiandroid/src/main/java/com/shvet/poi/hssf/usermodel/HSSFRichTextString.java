...
import com.shvet.poi.hssf.record.common.UnicodeString.FormatRun;
...
/**
 * Rich text unicode string. These strings can have fonts applied to arbitary
 * parts of the string.
 * <p/>
 * <p/>
 * Note, that in certain cases creating too many HSSFRichTextString cells may
 * cause Excel 2003 and lower to crash when changing the color of the cells and
 * then saving the Excel file. Compare two snippets that produce equivalent
 * output:
 * <p/>
 * <p/>
 * <blockquote>
 ...
 * <p/>
 * <p/>
 * Excel always uses the latter approach: for a reach text containing N runs
 * Excel saves the font of the first run in the cell's style and subsequent N-1
 * runs override this font.
 * <p/>
 * <p/>
 ...
 */
public final class HSSFRichTextString implements
        Comparable<HSSFRichTextString>, RichTextString {
    /**
     ...
     * RichTextString is added to a cell
     */
    ...
    /**
     * Called whenever the unicode string is modified. When it is modified we
     * need to create a new SST index, so that other LabelSSTRecords will not be
     * affected by changes that we make to this string.
     */
    ...
    private void addToSSTIfRequired() {
        if (_book != null) {
            ...
            // The act of adding the string to the SST record may have meant
            // that
            // an existing string was returned for the index, so update our
            // local version
    ...
    public void applyFont(int startIndex, int endIndex, short fontIndex) {
        ...
        if (formatting != null) {
            while (formatting.hasNext()) {
                ...
                if ((r.getCharacterPos() >= startIndex)
                        && (r.getCharacterPos() < endIndex))
                    formatting.remove();
            }
        }

        _string.addFormatRun(new UnicodeString.FormatRun((short) startIndex,
                fontIndex));
        if (endIndex != length())
            _string.addFormatRun(new UnicodeString.FormatRun((short) endIndex,
                    currentFont));
    ...
    /**
     * Used internally by the HSSFCell to get the internal string value. Will
     * ensure the string is not shared
     */
    ...
    /**
     * Returns the raw, probably shared Unicode String. Used when tweaking the
     * styles, eg updating font positions. Changes to this string may well
     * effect other RichTextStrings too!
     */
     ...
     * @return The font that's currently being applied at that index or null if
     * no font is being applied or the index is out of range.
     */
    ...
    /**
     * @return The number of formatting runs used. There will always be at least
     * one of font NO_FONT.
     * @see #NO_FONT
     */
    ...
    /**
     * The index within the string to which the specified formatting run
     * applies.
     *
