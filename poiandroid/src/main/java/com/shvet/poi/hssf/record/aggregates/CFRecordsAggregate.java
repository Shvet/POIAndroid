...
import com.shvet.poi.hssf.record.CFHeader12Record;
import com.shvet.poi.hssf.record.CFHeaderBase;
 ...
 * </p>
 * <p>
 * Note that Excel versions before 2007 can only cope with a maximum of 3
 * Conditional Formatting rules per sheet. Excel 2007 or newer can cope with
 * unlimited numbers, as can Apache OpenOffice. This is an Excel limitation, not
 * a file format one.
 * </p>
 */
public final class CFRecordsAggregate extends RecordAggregate {
    ...
    private static final POILogger logger = POILogFactory
            .getLogger(CFRecordsAggregate.class);
    ...
    private CFRecordsAggregate(CFHeaderBase pHeader, CFRuleBase[] pRules) {
        ...
        if (pRules.length > MAX_97_2003_CONDTIONAL_FORMAT_RULES) {
            logger.log(
                    POILogger.WARN,
                    "Excel versions before 2007 require that "
                            + "No more than "
                            + MAX_97_2003_CONDTIONAL_FORMAT_RULES
    ...
    private static CFHeaderBase createHeader(CellRangeAddress[] regions,
                                             CFRuleBase[] rules) {
    ...
    public static CFRecordsAggregate createCFAggregate(RecordStream rs) {
        ...
        if (rec.getSid() != CFHeaderRecord.sid
            ...
            throw new IllegalStateException("next record sid was "
                    + rec.getSid() + " instead of " + CFHeaderRecord.sid
                    + " or " + CFHeader12Record.sid + " as expected");
        }
    ...
    private static CellRangeAddress shiftRange(FormulaShifter shifter,
                                               CellRangeAddress cra, int currentExternSheetIx) {
        // FormulaShifter works well in terms of Ptgs - so convert
        // CellRangeAddress to AreaPtg (and back) here
        AreaPtg aptg = new AreaPtg(cra.getFirstRow(), cra.getLastRow(),
                cra.getFirstColumn(), cra.getLastColumn(), false, false, false,
                false);
        Ptg[] ptgs = {aptg,};
        ...
        throw new IllegalStateException("Unexpected shifted ptg class ("
                + ptg0.getClass().getName() + ")");
    }
    ...
    private void checkRuleType(CFRuleBase r) {
        if (header instanceof CFHeaderRecord && r instanceof CFRuleRecord) {
            return;
        }
        if (header instanceof CFHeader12Record && r instanceof CFRule12Record) {
            return;
        }
        throw new IllegalArgumentException(
                "Header and Rule must both be CF or both be CF12, can't mix");
    }
    ...
    /**
     * @return <code>false</code> if this whole {@link CFHeaderRecord} /
     * {@link CFRuleRecord}s should be deleted
     */
    public boolean updateFormulasAfterCellShift(FormulaShifter shifter,
        ...
        for (int i = 0; i < rules.size(); i++) {
            ...
            if (ptgs != null
                    && shifter.adjustFormula(ptgs, currentExternSheetIx)) {
