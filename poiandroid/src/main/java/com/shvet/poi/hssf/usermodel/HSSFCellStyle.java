...
import com.shvet.poi.ss.usermodel.Font;
 ...
 *
 * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#createCellStyle()
 * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#getCellStyleAt(short)
 * @see com.shvet.poi.hssf.usermodel.HSSFCell#setCellStyle(HSSFCellStyle)
 */
public final class HSSFCellStyle implements CellStyle {
    ...
    /**
     * get the index within the HSSFWorkbook (sequence within the collection of
     * ExtnededFormat objects)
     *
     * @return unique index number of the underlying record this style
     * represents (probably you don't care unless you're comparing which
     * one is which)
     */
    ...
    /**
     * Return the parent style for this cell style. In most cases this will be
     * null, but in a few cases there'll be a fully defined parent.
     */
    public HSSFCellStyle getParentStyle() {
        short parentIndex = _format.getParentIndex();
        // parentIndex equal 0xFFF indicates no inheritance from a cell style XF
        // (See 2.4.353 XF)
        if (parentIndex == 0 || parentIndex == 0xFFF) {
            return null;
        }
        return new HSSFCellStyle(parentIndex,
                _workbook.getExFormatAt(parentIndex), _workbook);
    }

    /**
     * get the index of the format
     *
     * @see com.shvet.poi.hssf.usermodel.HSSFDataFormat
     */
    ...
    /**
     * Get the contents of the format string, by looking up the DataFormat
     * against the bound workbook
     *
    ...
    /**
     * Get the contents of the format string, by looking up the DataFormat
     * against the supplied workbook
     *
     * @return the format string or "General" if not found
     * @see com.shvet.poi.hssf.usermodel.HSSFDataFormat
     */
    public String getDataFormatString(
            com.shvet.poi.ss.usermodel.Workbook workbook) {
    ...
    /**
     * Get the contents of the format string, by looking up the DataFormat
     * against the supplied low level workbook
     *
     * @see com.shvet.poi.hssf.usermodel.HSSFDataFormat
     */
    public String getDataFormatString(
            com.shvet.poi.hssf.model.InternalWorkbook workbook) {
     ...
     *
     * @param font a font object created or retreived from the HSSFWorkbook
     *             object
     * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#createFont()
     * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#getFontAt(short)
     */
     ...
     * @see com.shvet.poi.hssf.usermodel.HSSFCellStyle#getFontIndex()
     * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#getFontAt(short)
     */
    public HSSFFont getFont(com.shvet.poi.ss.usermodel.Workbook parentWorkbook) {
     ...
     *
     * @return rotation degrees (between -90 and 90 degrees, or 0xff for
     * vertical)
     */
    ...
    public void setRotation(short rotation) {
        if (rotation == 0xff) {
            ...
            throw new IllegalArgumentException(
                    "The rotation must be between -90 and 90 degrees, or 0xff");
        }
     ...
     * @see com.shvet.poi.hssf.usermodel.HSSFPalette#getColor(short)
     */
    ...
    private void checkDefaultBackgroundFills() {
        if (_format.getFillForeground() == com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index) {
            // JMH: Why +1, hell why not. I guess it made some sense to someone
            // at the time. Doesnt
            // to me now.... But experience has shown that when the fore is set
            // to AUTOMATIC then the
            // background needs to be incremented......
            if (_format.getFillBackground() != (com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index + 1))
                setFillBackgroundColor((short) (com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index + 1));
        } else if (_format.getFillBackground() == com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index + 1)
            // Now if the forground changes to a non-AUTOMATIC color the
            // background resets itself!!!
            if (_format.getFillForeground() != com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index)
                setFillBackgroundColor(com.shvet.poi.hssf.util.HSSFColor.AUTOMATIC.index);
    }

    /**
     * Get the background fill color. Note - many cells are actually filled with
     * a foreground fill, not a background fill - see
     * {@link #getFillForegroundColor()}
     *
     * @return fill color
     * @see com.shvet.poi.hssf.usermodel.HSSFPalette#getColor(short)
     */
    ...
    /**
     * Get the foreground fill color. Many cells are filled with this, instead
     * of a background color ({@link #getFillBackgroundColor()})
     *
     * @return fill color
     * @see com.shvet.poi.hssf.usermodel.HSSFPalette#getColor(short)
     */
    ...
    /**
     * Gets the name of the user defined style. Returns null for built in
     * styles, and styles where no name has been defined
     */
    ...
    public void setUserStyleName(String styleName) {
        ...
        if (sr.isBuiltin() && _index <= 20) {
            throw new IllegalArgumentException(
                    "Unable to set user specified style names for built in styles!");
        }
     ...
     * <p>
     * 0 means Context (Default), 1 means Left To Right, and 2 means Right to
     * Left
     * </p>
     *
    ...
    /**
     * Verifies that this style belongs to the supplied Workbook. Will throw an
     * exception if it belongs to a different one. This is normally called when
     * trying to assign a style to a cell, to ensure the cell and the style are
     * from the same workbook (if they're not, it won't work)
     *
     ...
     * HSSFCellStyle will be lost!
     * <p/>
     * The source HSSFCellStyle could be from another HSSFWorkbook if you like.
     * This allows you to copy styles from one HSSFWorkbook to another.
     */
    ...
    public void cloneStyleFrom(HSSFCellStyle source) {
        ...
        if (_workbook != source._workbook) {
            ...
            short fmt = (short) _workbook.createFormat(source
                    .getDataFormatString());
            setDataFormat(fmt);
            ...
            fr.cloneStyleFrom(source._workbook.getFontRecordAt(source
                    .getFontIndex()));
