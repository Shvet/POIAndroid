...
public final class NoteRecord extends StandardRecord {
    ...
    /**
     * Construct a new <code>NoteRecord</code> and fill its data with the
     * default values
     */
    public NoteRecord() {
        field_6_author = "";
        field_3_flags = 0;
        field_7_padding = DEFAULT_PADDING; // seems to be always present
        // regardless of author text
    }
    ...
    protected int getDataSize() {
        return 11 // 5 shorts + 1 byte
                + field_6_author.length()
                * (field_5_hasMultibyte ? 2 : 1)
