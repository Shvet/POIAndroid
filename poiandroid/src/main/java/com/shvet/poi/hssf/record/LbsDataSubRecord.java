...
/**
 * This structure specifies the properties of a list or drop-down list embedded
 * object in a sheet.
 */
public class LbsDataSubRecord extends SubRecord {
     ...
     * <p/>
     * An unsigned integer that indirectly specifies whether some of the data in
     * this structure appear in a subsequent Continue record. If _cbFContinued
     * is 0x00, all of the fields in this structure except sid and _cbFContinued
     * MUST NOT exist. If this entire structure is contained within the same
     * record, then _cbFContinued MUST be greater than or equal to the size, in
     * bytes, of this structure, not including the four bytes for the ft and
     * _cbFContinued fields
     */
    private int _cbFContinued;

    /**
     * a formula that specifies the range of cell values that are the items in
     * this list.
     */
    ...
    /**
     * An unsigned integer that specifies the one-based index of the first
     * selected item in this list. A value of 0x00 specifies there is no
    ...
    /**
     * An ObjId that specifies the edit box associated with this list. A value
     * of 0x00 specifies that there is no edit box associated with this list.
     */
    private int _idEdit;

    /**
     * An optional LbsDropData that specifies properties for this dropdown
     * control. This field MUST exist if and only if the containing Obj?s cmo.ot
     * is equal to 0x14.
     */
    private LbsDropData _dropData;

    /**
     * An optional array of strings where each string specifies an item in the
     * list. The number of elements in this array, if it exists, MUST be
     * {@link #_cLines}
     */
    private String[] _rgLines;

    /**
     * An optional array of booleans that specifies which items in the list are
     ...
     * @param cmoOt        the containing Obj's
     *                     {@link CommonObjectDataSubRecord#field_1_objectType}
     */
    public LbsDataSubRecord(LittleEndianInput in, int cbFContinued, int cmoOt) {
        ...
        // From [MS-XLS].pdf 2.5.147 FtLbsData:
        // This field MUST exist if and only if the containing Obj?s cmo.ot is
        // equal to 0x14.
        if (cmoOt == 0x14) {
        ...
        // From [MS-XLS].pdf 2.5.147 FtLbsData:
        // This array MUST exist if and only if the wListType field is not equal
        // to 0.
        if (((_flags >> 4) & 0x2) != 0) {
     ...
     * @see com.shvet.poi.hssf.model.ComboboxShape#createObjRecord(org.apache.poi.hssf.usermodel.HSSFSimpleShape,
     * int)
     */
    public static LbsDataSubRecord newAutoFilterInstance() {
        LbsDataSubRecord lbs = new LbsDataSubRecord();
        lbs._cbFContinued = 0x1FEE; // autofilters seem to alway have this magic
        // number
        lbs._iSel = 0x000;
    ...
    public String toString() {
        ...
        if (_linkPtg != null)
            sb.append(_linkPtg.toString()).append(_linkPtg.getRVAType())
                    .append('\n');
        sb.append("    .nEntryCount   =").append(HexDump.shortToHex(_cLines))
                .append("\n");
        sb.append("    .selEntryIx    =").append(HexDump.shortToHex(_iSel))
                .append("\n");
        sb.append("    .style         =").append(HexDump.shortToHex(_flags))
                .append("\n");
        sb.append("    .unknownShort10=").append(HexDump.shortToHex(_idEdit))
                .append("\n");
        if (_dropData != null)
    ...
    /**
     * @return the formula that specifies the range of cell values that are the
     * items in this list.
     */
    ...
    public static class LbsDropData {
        ...
        /**
         * An unsigned integer that specifies the number of lines to be
         * displayed in the dropdown.
         */
        private int _cLine;

        /**
         * An unsigned integer that specifies the smallest width in pixels
         * allowed for the dropdown window
         */
        private int _dxMin;
        ...
        /**
         * Optional, undefined and MUST be ignored. This field MUST exist if and
         * only if the size of str in bytes is an odd number
         */
         ...
         * Possible values:
         * <p/>
         * 0 Combo dropdown control 1 Combo Edit dropdown control 2 Simple
         * dropdown control (just the dropdown button)
         */
