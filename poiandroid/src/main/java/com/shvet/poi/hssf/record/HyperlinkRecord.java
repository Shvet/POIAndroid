...
import com.shvet.poi.util.HexRead;
...
/**
 * The <code>HyperlinkRecord</code> (0x01B8) wraps an HLINK-record from the
 * Excel-97 format. Supports only external links for now (eg http://)
 */
public final class HyperlinkRecord extends StandardRecord {
    ...
    final static GUID STD_MONIKER = GUID
            .parse("79EAC9D0-BAF9-11CE-8C82-00AA004BA90B");
    final static GUID URL_MONIKER = GUID
            .parse("79EAC9E0-BAF9-11CE-8C82-00AA004BA90B");
    final static GUID FILE_MONIKER = GUID
            .parse("00000303-0000-0000-C000-000000000046");
    ...
    private static POILogger logger = POILogFactory
            .getLogger(HyperlinkRecord.class);
    /**
    ...
    /**
     * in 8:3 DOS format No Unicode string header, always 8-bit characters,
     * zero-terminated
     */
    ...
    public HyperlinkRecord(RecordInputStream in) {
        ...
        /**
         * streamVersion (4 bytes): An unsigned integer that specifies the
         * version number of the serialization implementation used to save this
         * structure. This value MUST equal 2.
         */
        int streamVersion = in.readInt();
        if (streamVersion != 0x00000002) {
            throw new RecordFormatException(
                    "Stream Version must be 0x2 but found " + streamVersion);
        }
        ...
        if ((_linkOpts & HLINK_URL) != 0 && (_linkOpts & HLINK_UNC_PATH) == 0) {
            ...
            if (URL_MONIKER.equals(_moniker)) {
                ...
                /**
                 * The value of
                 * <code>length<code> be either the byte size of the url field
                ...
                if (length == remaining) {
                    ...
                    _address = in.readUnicodeLEString(nChars);
                    /**
                     * TODO: make sense of the remaining bytes According to the
                     * spec they consist of: 1. 16-byte GUID: This field MUST
                     * equal {0xF4815879, 0x1D3B, 0x487F, 0xAF, 0x2C, 0x82,
                     * 0x5D, 0xC4, 0x85, 0x27, 0x63} 2. Serial version, this
                     * field MUST equal 0 if present. 3. URI Flags
                     */
    ...
    private static byte[] readTail(byte[] expectedTail, LittleEndianInput in) {
        ...
        in.readFully(result);
        // if (false) { // Quite a few examples in the unit tests which don't
        // have the exact expected tail
        // for (int i = 0; i < expectedTail.length; i++) {
        // if (expectedTail[i] != result[i]) {
        // logger.log( POILogger.ERROR, "Mismatch in tail byte [" + i + "]"
        // + "expected " + (expectedTail[i] & 0xFF) + " but got " + (result[i] &
        // 0xFF));
        // }
        // }
        // }
    ...
    /**
     * Set the first column (zero-based)of the range that contains this
     * hyperlink
     */
    ...
    /**
     * @return 16-byte guid identifier Seems to always equal
     * {@link #STD_MONIKER}
     */
    GUID getGuid() {
        return _guid;
    ...
    /**
     * Hyperlink address. Depending on the hyperlink type it can be URL, e-mail,
     * path to a file, etc.
     *
    ...
    /**
     * Link options. Must be a combination of HLINK_* constants. For testing
     * only
     */
    int getLinkOptions() {
        return _linkOpts;
    ...
    public String toString() {
        ...
        buffer.append("    .range   = ").append(_range.formatAsString())
                .append("\n");
        buffer.append("    .guid    = ").append(_guid.formatAsString())
                .append("\n");
        buffer.append("    .linkOpts= ").append(HexDump.intToHex(_linkOpts))
                .append("\n");
        buffer.append("    .label   = ").append(getLabel()).append("\n");
        if ((_linkOpts & HLINK_TARGET_FRAME) != 0) {
            buffer.append("    .targetFrame= ").append(getTargetFrame())
                    .append("\n");
        }
    ...
    static final class GUID {
        ...
        /**
         * 8 bytes - serialized as big endian, stored with inverted endianness
         * here
         */
        ...
        public static GUID parse(String rep) {
            ...
            if (cc.length != TEXT_FORMAT_LENGTH) {
                throw new RecordFormatException(
                        "supplied text is the wrong length for a GUID");
            }
