...
import com.shvet.poi.hssf.record.AutoFilterInfoRecord;
import com.shvet.poi.hssf.record.CellValueRecordInterface;
import com.shvet.poi.hssf.record.DVRecord;
import com.shvet.poi.hssf.record.DimensionsRecord;
import com.shvet.poi.hssf.record.DrawingRecord;
...
import com.shvet.poi.hssf.record.RowRecord;
import com.shvet.poi.hssf.record.SCLRecord;
import com.shvet.poi.hssf.record.WSBoolRecord;
import com.shvet.poi.hssf.record.WindowTwoRecord;
import com.shvet.poi.hssf.record.aggregates.DataValidityTable;
...
import com.shvet.poi.hssf.record.aggregates.WorksheetProtectionBlock;
import com.shvet.poi.hssf.util.PaneInformation;
...
import com.shvet.poi.ss.formula.ptg.MemFuncPtg;
import com.shvet.poi.ss.formula.ptg.Ptg;
import com.shvet.poi.ss.formula.ptg.UnionPtg;
import com.shvet.poi.ss.usermodel.Cell;
import com.shvet.poi.ss.usermodel.CellRange;
import com.shvet.poi.ss.usermodel.CellStyle;
import com.shvet.poi.ss.usermodel.DataValidation;
import com.shvet.poi.ss.usermodel.DataValidationHelper;
import com.shvet.poi.ss.usermodel.Row;
import com.shvet.poi.ss.util.CellRangeAddress;
import com.shvet.poi.ss.util.CellRangeAddressList;
import com.shvet.poi.ss.util.CellReference;
import com.shvet.poi.ss.util.SSCellRange;
import com.shvet.poi.ss.util.SheetUtil;
 ...
 */
public final class HSSFSheet implements com.shvet.poi.ss.usermodel.Sheet {
    /**
     * Used for compile-time optimization. This is the initial size for the
     * collection of rows. It is currently set to 20. If you generate larger
     * sheets you may benefit by setting this to a higher number and recompiling
     * a custom edition of HSSFSheet.
     */
    public final static int INITIAL_CAPACITY = Configurator.getIntValue(
            "HSSFSheet.RowInitialCapacity", 20);
    private static final POILogger log = POILogFactory
            .getLogger(HSSFSheet.class);
     ...
     * scratch. You should not be calling this from application code (its
     * protected anyhow).
     *
     * @param workbook - The HSSF Workbook object associated with the sheet.
     * @see com.shvet.poi.hssf.usermodel.HSSFWorkbook#createSheet()
     */
    ...
    HSSFSheet cloneSheet(HSSFWorkbook workbook) {
        ...
        DrawingRecord dr = (DrawingRecord) sheet._sheet
                .findFirstRecordBySid(DrawingRecord.sid);
        if (null != dr) {
            sheet._sheet.getRecords().remove(dr);
        }
        if (getDrawingPatriarch() != null) {
            HSSFPatriarch patr = HSSFPatriarch.createPatriarch(
                    this.getDrawingPatriarch(), sheet);
        ...
        while (iter.hasNext()) {
            ...
            if (log.check(POILogger.DEBUG)) {
                if (cval instanceof Record) {
                    log.log(DEBUG,
                            "record id = "
                                    + Integer.toHexString(((Record) cval)
                                    .getSid()));
                } else {
     ...
     * @see com.shvet.poi.hssf.usermodel.HSSFRow
     * @see #removeRow(com.shvet.poi.ss.usermodel.Row)
     */
    ...
    /**
     * Used internally to create a high level Row object from a low level row
     * object. USed when reading an existing file
     *
     * @param row low level record to represent as a high level Row and add to
     *            sheet
    ...
    /**
     * Remove a row from this sheet. All cells contained in the row are removed
     * as well
     *
    ...
    public void removeRow(Row row) {
        ...
        if (row.getSheet() != this) {
            throw new IllegalArgumentException(
                    "Specified row does not belong to this sheet");
        }
        for (Cell cell : row) {
     ...
     *
     * @param addLow whether to add the row to the low level model - false if its
     *               already there
     */
    ...
    /**
     * Returns the logical row (not physical) 0-based. If you ask for a row that
     * is not defined you get a null. This is to say row 4 represents the fifth
     * row on a sheet.
     *
     * @param rowIndex row to get
     * @return HSSFRow representing the row number or null if its not defined on
     * the sheet
     */
    ...
    /**
     * Returns the number of physically defined rows (NOT the number of rows in
     * the sheet)
     */
     ...
     * {@link #getPhysicalNumberOfRows()} to tell if there is a row at position
     * zero or not.
     *
    ...
    public List<HSSFDataValidation> getDataValidations() {
        ...
        RecordVisitor visitor = new RecordVisitor() {
            private HSSFEvaluationWorkbook book = HSSFEvaluationWorkbook
                    .create(getWorkbook());
            ...
            public void visitRecord(Record r) {
                ...
                CellRangeAddressList regions = dvRecord.getCellRangeAddress()
                        .copy();
                DVConstraint constraint = DVConstraint.createDVConstraint(
                        dvRecord, book);
                HSSFDataValidation hssfDataValidation = new HSSFDataValidation(
                        regions, constraint);
                hssfDataValidation.setErrorStyle(dvRecord.getErrorStyle());
                hssfDataValidation.setEmptyCellAllowed(dvRecord
                        .getEmptyCellAllowed());
                hssfDataValidation.setSuppressDropDownArrow(dvRecord
                        .getSuppressDropdownArrow());
                hssfDataValidation.createPromptBox(dvRecord.getPromptTitle(),
                        dvRecord.getPromptText());
                hssfDataValidation.setShowPromptBox(dvRecord
                        .getShowPromptOnCellSelected());
                hssfDataValidation.createErrorBox(dvRecord.getErrorTitle(),
                        dvRecord.getErrorText());
                hssfDataValidation.setShowErrorBox(dvRecord
     ...
     * <p/>
     * <p>
     * The maximum column width for an individual cell is 255 characters. This
     * value represents the number of characters that can be displayed in a cell
     * that is formatted with the standard font (first font in the workbook).
     * </p>
     * <p/>
     * <p>
     * Character width is defined as the maximum digit width of the numbers
     * <code>0, 1, 2, ... 9</code> as rendered using the default font (first
     * font in the workbook). <br/>
     * Unless you are using a very special font, the default character is '0'
     * (zero), this is true for Arial (default font font in HSSF) and Calibri
     * (default font in XSSF)
     * </p>
     * <p/>
     * <p>
     * Please note, that the width set by this method includes 4 pixels of
     * margin padding (two on each side), plus 1 pixel padding for the gridlines
     * (Section 3.3.1.12 of the OOXML spec). This results is a slightly less
     * value of visible characters than passed to this method (approx. 1/2 of a
     * character).
     * </p>
     * <p>
     ...
     * font size is 7 pixels (at 96 dpi). If you set a column width to be eight
     * characters wide, e.g. <code>setColumnWidth(columnIndex, 8*256)</code>,
     * then the actual value of visible characters (the value shown in Excel) is
     * derived from the following equation: <code>
     ...
     * @throws IllegalArgumentException if width > 255*256 (the maximum column width in Excel is 255
     *                                  characters)
     */
    ...
    /**
     * get the default row height for the sheet (if the rows do not define their
     * own height) in twips (1/20 of a point)
     *
    ...
    /**
     * set the default row height for the sheet (if the rows do not define their
     * own height) in twips (1/20 of a point)
     *
    ...
    /**
     * Returns the HSSFCellStyle that applies to the given (0 based) column, or
     * null if no style has been set for that column
     */
    ...
    /**
     * @deprecated (Aug-2008) use <tt>CellRangeAddress</tt> instead of
     * <tt>Region</tt>
     */
    public int addMergedRegion(com.shvet.poi.ss.util.Region region) {
    ...
    public int addMergedRegion(CellRangeAddress region) {
        ...
        return _sheet.addMergedRegion(region.getFirstRow(),
                region.getFirstColumn(), region.getLastRow(),
            ...
            for (int colIn = firstColumn; colIn <= lastColumn; colIn++) {
                ...
                if (cell.isPartOfArrayFormulaGroup()) {
                    ...
                    if (arrayRange.getNumberOfCells() > 1
                            ...
                            region.getFirstColumn()) || arrayRange
                            .isInRange(region.getFirstRow(),
                                    region.getFirstColumn()))) {
                        String msg = "The range "
     ...
     * <p/>
     * <p>
     * Calculating the formula values with
     * {@link com.shvet.poi.ss.usermodel.FormulaEvaluator} is the recommended
     * solution, but this may be used for certain cases where evaluation in POI
     * is not possible.
     * </p>
     * <p/>
     * <p>
     * It is recommended to force recalcuation of formulas on workbook level
     * using
     * {@link com.shvet.poi.ss.usermodel.Workbook#setForceFormulaRecalculation(boolean)}
     * to ensure that all cross-worksheet formuals and external dependencies are
     * updated.
     * </p>
     *
     * @param value true if the application will perform a full recalculation of
     *              this worksheet values when the workbook is opened
     * @see com.shvet.poi.ss.usermodel.Workbook#setForceFormulaRecalculation(boolean)
     */
    ...
    /**
     * Determine whether printed output for this sheet will be horizontally
     * centered.
     */
    ...
    /**
     * Sets whether the worksheet is displayed from right to left instead of
     * from left to right.
     *
     ...
     */
    public com.shvet.poi.hssf.util.Region getMergedRegionAt(int index) {
        CellRangeAddress cra = getMergedRegion(index);

        return new com.shvet.poi.hssf.util.Region(cra.getFirstRow(),
                (short) cra.getFirstColumn(), cra.getLastRow(),
    ...
    public Iterator<Row> rowIterator() {
        ...
        // can this clumsy generic syntax be improved?
                Iterator<Row> result = (Iterator<Row>) (Iterator<? extends Row>) _rows
                .values().iterator();
        return result;
    }
    ...
    /**
     * used internally in the API to get the low level Sheet record represented
     * by this Object.
     *
    ...
    public void setAlternativeExpression(boolean b) {
        WSBoolRecord record = (WSBoolRecord) _sheet
                .findFirstRecordBySid(WSBoolRecord.sid);
    ...
    /**
     * Gets the flag indicating whether the window should show 0 (zero) in cells
     * containing zero value. When false, cells with zero value appear blank
     * instead of showing the number zero.
     * <p>
     * In Excel 2003 this option can be changed in the Options dialog on the
     * View tab.
     * </p>
     *
    ...
    /**
     * Set whether the window should show 0 (zero) in cells containing zero
     * value. When false, cells with zero value appear blank instead of showing
     * the number zero.
     * <p>
     * In Excel 2003 this option can be set in the Options dialog on the View
     * tab.
     * </p>
     *
     ...
     *
     * @param newPrintGridlines boolean to turn on or off the printing of gridlines
     */
    ...
    public void protectSheet(String password) {
        getProtectionBlock().protectSheet(password, true, true); // protect
        // objs&scenarios(normal)
    }
    ...
    public void setZoom(int numerator, int denominator) {
        if (numerator < 1 || numerator > 65535)
            throw new IllegalArgumentException(
                    "Numerator must be greater than 0 and less than 65536");
        if (denominator < 1 || denominator > 65535)
            throw new IllegalArgumentException(
                    "Denominator must be greater than 0 and less than 65536");
    ...
    /**
     * The top row in the visible view when the sheet is first viewed after
    ...
    /**
     * The left col in the visible view when the sheet is first viewed after
     * opening it in a viewer
     *
     * @return short indicating the rownum (0 based) of the top row
     */
    ...
    public void showInPane(int toprow, int leftcol) {
        ...
        if (toprow > maxrow)
            throw new IllegalArgumentException("Maximum row number is "
                    + maxrow);
    ...
    protected void shiftMerged(int startRow, int endRow, int n, boolean isRow) {
        ...
        for (int i = 0; i < getNumMergedRegions(); i++) {
            ...
            boolean inStart = (merged.getFirstRow() >= startRow || merged
                    .getLastRow() >= startRow);
            boolean inEnd = (merged.getFirstRow() <= endRow || merged
                    .getLastRow() <= endRow);
            ...
            // only shift if the region outside the shifted rows is not merged
            // too
     ...
     * @param copyRowHeight          whether to copy the row height during the shift
     * @param resetOriginalRowHeight whether to set the original row's height to the default
     * @param moveComments           whether to move comments at the same time as the cells they
     *                               are attached to
     */
    public void shiftRows(int startRow, int endRow, int n,
                          boolean copyRowHeight, boolean resetOriginalRowHeight,
        ...
        for (int rowNum = s; rowNum >= startRow && rowNum <= endRow
            ...
            if (moveComments) {
                // This code would get simpler if NoteRecords could be organised
                // by HSSFRow.
     ...
     * @param topRow         Top row visible in bottom pane
     * @param leftmostColumn Left column visible in right pane.
     * @param activePane     Active pane. One of: PANE_LOWER_RIGHT, PANE_UPPER_RIGHT,
    ...
    public void createSplitPane(int xSplitPos, int ySplitPos,
        ...
        getSheet().createSplitPane(xSplitPos, ySplitPos, topRow,
                leftmostColumn, activePane);
    }

    /**
     * Returns the information regarding the currently configured pane (split or
     * freeze).
     *
    ...
    /**
     * Sets a page break at the indicated row Breaks occur above the specified
     * row and left of the specified column inclusive.
     * <p/>
     * For example, <code>sheet.setColumnBreak(2);</code> breaks the sheet into
     * two parts with columns A,B,C in the first and D,E,... in the second.
     * Simuilar, <code>sheet.setRowBreak(2);</code> breaks the sheet into two
     * parts with first three rows (rownum=1...3) in the first part and rows
     ...
     * specified row and left of the specified column inclusive.
     * <p/>
     * For example, <code>sheet.setColumnBreak(2);</code> breaks the sheet into
     * two parts with columns A,B,C in the first and D,E,... in the second.
     * Simuilar, <code>sheet.setRowBreak(2);</code> breaks the sheet into two
    ...
    protected void validateColumn(int column) {
        ...
        if (column > maxcol)
            throw new IllegalArgumentException("Maximum column number is "
                    + maxcol);
        if (column < 0)
    ...
    /**
     * Aggregates the drawing records and dumps the escher record hierarchy to
     * the standard output.
     */
    ...
    public EscherAggregate getDrawingEscherAggregate() {
        ...
        int found = _sheet.aggregateDrawingRecords(_book.getDrawingManager(),
                false);
        if (found == -1) {
     ...
     *
     * @return the top-level drawing patriarch, if there is one, else returns
     * null
     */
     ...
     * </p>
     * <p>
     * Note that this will normally have the effect of removing any existing
     * drawings on this sheet.
     * </p>
     *
    ...
    /**
     * Sets the default column style for a given column. POI will only apply
     * this style to new cells added to the sheet.
     *
     ...
     * normally only be called once per column, at the end of your processing.
     * <p/>
     * You can specify whether the content of merged cells should be considered
     * or ignored. Default is to ignore merged cells.
     *
     * @param column         the column index
     * @param useMergedCells whether to use the contents of merged cells when calculating
    ...
    private CellRange<HSSFCell> getCellRange(CellRangeAddress range) {
        ...
        return SSCellRange.create(firstRow, firstColumn, height, width, temp,
                HSSFCell.class);
    }
    ...
    public CellRange<HSSFCell> removeArrayFormula(Cell cell) {
        if (cell.getSheet() != this) {
            throw new IllegalArgumentException(
                    "Specified cell does not belong to this sheet.");
        }
        ...
        if (!(rec instanceof FormulaRecordAggregate)) {
            ...
            throw new IllegalArgumentException("Cell " + ref
    ...
    public HSSFAutoFilter setAutoFilter(CellRangeAddress range) {
        ...
        NameRecord name = workbook.getSpecificBuiltinRecord(
                NameRecord.BUILTIN_FILTER_DB, sheetIndex + 1);

        if (name == null) {
            name = workbook.createBuiltInName(NameRecord.BUILTIN_FILTER_DB,
                    sheetIndex + 1);
        }
        ...
        Area3DPtg ptg = new Area3DPtg(firstRow, range.getLastRow(),
                range.getFirstColumn(), range.getLastColumn(), false, false,
                false, false, sheetIndex);
        ...
        for (int col = range.getFirstColumn(); col <= range.getLastColumn(); col++) {
            p.createComboBox(new HSSFClientAnchor(0, 0, 0, 0, (short) col,
                    firstRow, (short) (col + 1), firstRow + 1));
        }
    ...
    private HSSFComment lookForComment(HSSFShapeContainer container, int row,
        ...
        for (Object object : container.getChildren()) {
            ...
            if (shape instanceof HSSFShapeGroup) {
                HSSFShape res = lookForComment((HSSFShapeContainer) shape, row,
                        column);
                if (null != res) {
    ...
    private void setRepeatingRowsAndColumns(CellRangeAddress rowDef,
                                            CellRangeAddress colDef) {
        int sheetIndex = _workbook.getSheetIndex(this);
        ...
        short externSheetIndex = _workbook.getWorkbook().checkExternSheet(
                sheetIndex);
        ...
        if (name == null) {
            name = _workbook.createBuiltInName(NameRecord.BUILTIN_PRINT_TITLE,
                    sheetIndex);
        }

        List<Ptg> ptgList = new ArrayList<Ptg>();
        if (setBoth) {
            final int exprsSize = 2 * 11 + 1; // 2 * Area3DPtg.SIZE +
            // UnionPtg.SIZE
                ...
                if (areaPtg.getFirstColumn() == 0
                    ...
                    if (rows) {
                        CellRangeAddress rowRange = new CellRangeAddress(
                                areaPtg.getFirstRow(), areaPtg.getLastRow(),
                                -1, -1);
                        return rowRange;
                    }
                    ...
                    if (!rows) {
                        CellRangeAddress columnRange = new CellRangeAddress(-1,
                                -1, areaPtg.getFirstColumn(),
                                areaPtg.getLastColumn());
                        return columnRange;
                    }
                }
    ...
    private NameRecord getBuiltinNameRecord(byte builtinCode) {
        int sheetIndex = _workbook.getSheetIndex(this);
        int recIndex = _workbook.findExistingBuiltinNameRecordIdx(sheetIndex,
                builtinCode);
        if (recIndex == -1) {
            return null;
        }
    ...
    /**
     * Returns the column outline level. Increased as you put it into more
     * groups (outlines), reduced as you take it out of them.
     */
