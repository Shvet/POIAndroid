...
public abstract class SubRecord {
     ...
     *              CommonObjectDataSubRecord, we need it to propagate to next
     *              sub-records as it defines what data follows
     * @return the created sub-record
     */
    public static SubRecord createSubRecord(LittleEndianInput in, int cmoOt) {
        int sid = in.readUShort();
        int secondUShort = in.readUShort(); // Often (but not always) the
        // datasize for the sub-record
    ...
    /**
     * @return the size of the data for this record (which is always 4 bytes
     * less than the total record size). Note however, that ushort
     * encoded after the record sid is usually but not always the data
     * size.
     */
    ...
    /**
     * Wether this record terminates the sub-record stream. There are two cases
     * when this method must be overridden and return <code>true</code> -
     * EndSubRecord (sid = 0x00) - LbsDataSubRecord (sid = 0x12)
     *
