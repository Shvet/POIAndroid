...
/**
 * Excel can get cranky if you give it files containing too many (especially
 * duplicate) objects, and this class can help to avoid those. In general, it's
 * much better to make sure you don't duplicate the objects in your code, as
 * this is likely to be much faster than creating lots and lots of excel
 * objects+records, only to optimise them down to many fewer at a later stage.
 * However, sometimes this is too hard / tricky to do, which is where the use of
 * this class comes in.
 */
public class HSSFOptimiser {
    /**
     * Goes through the Workbook, optimising the fonts by removing duplicate
     * ones. For now, only works on fonts used in {@link HSSFCellStyle} and
     * {@link HSSFRichTextString}. Any other font uses (eg charts, pictures) may
     * well end up broken! This can be a slow operation, especially if you have
    ...
    public static void optimiseFonts(HSSFWorkbook workbook) {
        ...
        short[] newPos = new short[workbook.getWorkbook()
                .getNumberOfFontRecords() + 1];
            ...
            for (Row row : s) {
                for (Cell cell : row) {
                    if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                        HSSFRichTextString rtr = (HSSFRichTextString) cell
    ...
    /**
     * Goes through the Wokrbook, optimising the cell styles by removing
     * duplicate ones, and ones that aren't used. For best results, optimise the
     * fonts via a call to {@link #optimiseFonts(HSSFWorkbook)} first.
     *
    ...
    public static void optimiseCellStyles(HSSFWorkbook workbook) {
        ...
        for (int i = 21; i < newPos.length; i++) {
            ...
            int earlierDuplicate = -1;
            for (int j = 0; j < i && earlierDuplicate == -1; j++) {
                ExtendedFormatRecord xfCheck = workbook.getWorkbook()
                        .getExFormatAt(j);
        ...
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            for (Row row : s) {
                for (Cell cellI : row) {
                    ...
                    HSSFCellStyle newStyle = workbook
                            .getCellStyleAt(newPos[oldXf]);
