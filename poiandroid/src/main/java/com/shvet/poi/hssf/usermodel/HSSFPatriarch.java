...
import com.shvet.poi.ddf.EscherProperty;
...
import com.shvet.poi.hssf.record.EmbeddedObjectRefSubRecord;
import com.shvet.poi.hssf.record.EndSubRecord;
import com.shvet.poi.hssf.record.EscherAggregate;
import com.shvet.poi.hssf.record.FtCfSubRecord;
import com.shvet.poi.hssf.record.FtPioGrbitSubRecord;
...
import com.shvet.poi.ss.usermodel.Chart;
import com.shvet.poi.ss.usermodel.ClientAnchor;
import com.shvet.poi.ss.usermodel.Drawing;
...
/**
 * The patriarch is the toplevel container for shapes in a sheet. It does little
 * other than act as a container for other shapes and groups.
 */
public final class HSSFPatriarch implements HSSFShapeContainer, Drawing {
    ...
    /**
     * The EscherAggregate we have been bound to. (This will handle writing us
     * out into records, and building up our shapes from the records)
     */
    ...
    HSSFPatriarch(HSSFSheet sheet, EscherAggregate boundAggregate) {
        ...
        _mainSpgrContainer = _boundAggregate.getEscherContainer()
                .getChildContainers().get(0);
        EscherContainerRecord spContainer = (EscherContainerRecord) _boundAggregate
                .getEscherContainer().getChildContainers().get(0).getChild(0);
     ...
     * patriarch
     */
    static HSSFPatriarch createPatriarch(HSSFPatriarch patriarch,
                                         HSSFSheet sheet) {
        HSSFPatriarch newPatriarch = new HSSFPatriarch(sheet,
                new EscherAggregate(true));
    ...
    /**
     * check if any shapes contain wrong data At now(13.08.2010) check if
     * patriarch contains 2 or more comments with same coordinates
     */
    protected void preSerialize() {
        ...
        for (NoteRecord rec : tailRecords.values()) {
            String noteRef = new CellReference(rec.getRow(), rec.getColumn())
                    .formatAsString(); // A1-style notation
            if (coordinates.contains(noteRef)) {
                throw new IllegalStateException(
                        "found multiple cell comments for cell " + noteRef);
            } else {
    ...
    public boolean removeShape(HSSFShape shape) {
        boolean isRemoved = _mainSpgrContainer.removeChildRecord(shape
                .getEscherContainer());
        if (isRemoved) {
    ...
    void afterCreate() {
        DrawingManager2 drawingManager = _sheet.getWorkbook().getWorkbook()
                .getDrawingManager();
     ...
     * and ovals.
     * <p/>
     * Note: Microsoft Excel seems to sometimes disallow higher y1 than y2 or
     * higher x1 than x2 in the anchor, you might need to reverse them and draw
     * shapes vertically or horizontally flipped!
     *
     * @param anchor the client anchor describes how this group is attached to the
     *               sheet.
     ...
     * @param anchor       the client anchor describes how this group is attached to the
     *                     sheet.
     * @param pictureIndex - pointer to the byte array saved inside workbook in escher
     *                     bse record
     * @return the newly created shape.
     */
     ...
     * @param anchor       the client anchor describes how this picture is attached to
     *                     the sheet.
     * @param storageId    the storageId returned by
     *                     {@link HSSFWorkbook#addOlePackage(POIFSFileSystem, String, String, String)}
     * @param pictureIndex the index of the picture (used as preview image) in the
     *                     workbook collection of pictures.
     * @return newly created shape
     */
    public HSSFObjectData createObjectData(HSSFClientAnchor anchor,
                                           int storageId, int pictureIndex) {
        ObjRecord obj = new ObjRecord();

        CommonObjectDataSubRecord ftCmo = new CommonObjectDataSubRecord();
        ftCmo.setObjectType(CommonObjectDataSubRecord.OBJECT_TYPE_PICTURE);
        // ftCmo.setObjectId(oleShape.getShapeId()); ... will be set by
        // onCreate(...)
        ...
        HSSFPictureData pictData = getSheet().getWorkbook().getAllPictures()
                .get(pictureIndex - 1);
        ...
        try {
            ...
            throw new IllegalStateException(
                    "trying to add ole shape without actually adding data first - use HSSFWorkbook.addOlePackage first",
                    e);
        }
        ...
        EscherSpRecord spRecord = spContainer
    ...
    private void onCreate(HSSFShape shape) {
        EscherContainerRecord spgrContainer = _boundAggregate
                .getEscherContainer().getChildContainers().get(0);
    ...
    public int countOfAllChildren() {
        ...
        for (Iterator<HSSFShape> iterator = _shapes.iterator(); iterator
                .hasNext(); ) {
    ...
    /**
     * Sets the coordinate space of this group. All children are constrained to
     * these coordinates.
     */
     ...
     */
    int newShapeId() {
        DrawingManager2 dm = _sheet.getWorkbook().getWorkbook()
                .getDrawingManager();
        EscherDgRecord dg = _boundAggregate.getEscherContainer().getChildById(
    ...
    /**
     * Does this HSSFPatriarch contain a chart? (Technically a reference to a
     * chart, since they get stored in a different block of records) FIXME -
     * detect chart in all cases (only seems to work on some charts so far)
     */
    public boolean containsChart() {
        // TODO - support charts properly in usermodel

        // We're looking for a EscherOptRecord
        EscherOptRecord optRecord = (EscherOptRecord) _boundAggregate
                .findFirstWithId(EscherOptRecord.RECORD_ID);
        if (optRecord == null) {
            // No opt record, can't have chart
            return false;
        }

        for (Iterator<EscherProperty> it = optRecord.getEscherProperties()
                .iterator(); it.hasNext(); ) {
     ...
     */
    public HSSFClientAnchor createAnchor(int dx1, int dy1, int dx2, int dy2,
     ...
     */
    void buildShapeTree() {
        EscherContainerRecord dgContainer = _boundAggregate
                .getEscherContainer();
        if (dgContainer == null) {
            return;
        }
        EscherContainerRecord spgrConrainer = dgContainer.getChildContainers()
                .get(0);
        List<EscherContainerRecord> spgrChildren = spgrConrainer
                .getChildContainers();
    ...
    private void setFlipFlags(HSSFShape shape) {
        EscherSpRecord sp = shape.getEscherContainer().getChildById(
