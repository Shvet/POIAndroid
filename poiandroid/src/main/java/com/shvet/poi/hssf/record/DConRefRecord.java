 ...
 * serves as a data source for pivot tables or data consolidation.
 * <p/>
 * Represents a <code>DConRef</code> Structure <a
 * href="http://msdn.microsoft.com/en-us/library/dd923854(office.12).aspx"
 * >[MS-XLS s. 2.4.86]</a>, and the contained <code>DConFile</code> structure <a
 * href="http://msdn.microsoft.com/en-us/library/dd950157(office.12).aspx">
 * [MS-XLS s. 2.5.69]</a>. This in turn contains a
 * <code>XLUnicodeStringNoCch</code> <a
 * href="http://msdn.microsoft.com/en-us/library/dd910585(office.12).aspx">
 * [MS-XLS s. 2.5.296]</a>.
 * <p/>
 * <pre>
 ...
 * Where
 * <ul>
 * <li><code>DConFile.h = 0x00</code> if the characters in<code>rgb</code> are
 * single byte, and <code>DConFile.h = 0x01</code> if they are double byte. <br/>
 * If they are double byte, then<br/>
 * <ul>
 * <li>If it exists, the length of <code>DConRef.un = 2</code>. Otherwise it is
 * 1.
 * <li>The length of <code>DConFile.rgb = (2 * DConRef.cch)</code>. Otherwise it
 * is equal to <code>DConRef.cch</code>.
 * </ul>
 * <li><code>DConRef.rgb</code> starts with <code>0x01</code> if it is an
 * external reference, and with <code>0x02</code> if it is a self-reference.
 * </ul>
 * <p/>
...
public class DConRefRecord extends StandardRecord {
    ...
    /**
     * A RefU structure specifying the range of cells if this record is part of
     * an SXTBL. <a
     * href="http://msdn.microsoft.com/en-us/library/dd920420(office.12).aspx">
     * [MS XLS s.2.5.211]</a>
     */
     ...
     * <code>XLUnicodeStringNoCch</code>. Therefore it will contain at least one
     * leading special character (0x01 or 0x02) and probably other ones.
     * <p/>
     *
     * @see <A
     * href="http://msdn.microsoft.com/en-us/library/dd923491(office.12).aspx">
     * DConFile [MS-XLS s. 2.5.77]</A> and <A
     * href="http://msdn.microsoft.com/en-us/library/dd950157(office.12).aspx"
    ...
    public DConRefRecord(byte[] data) {
        ...
        offset += LittleEndian.SHORT_SIZE;
        if (charCount < 2)
            throw new com.shvet.poi.hssf.record.RecordFormatException(
		...
		/*
         * bytelength is the length of the string in bytes, which depends on
		 * whether the string is made of single- or double-byte chars. This is
		 * given by charType, which equals 0 if single-byte, 1 if double-byte.
		 */
		 ...
		 * char type) are the unused field. Not sure If i need to bother with
		 * this...
		 */
        if (path[0] == 0x02)
    ...
    public DConRefRecord(RecordInputStream inStream) {
        ...
        // byteLength depends on whether we are using single- or double-byte
        // chars.
    ...
    /*
     * assuming this wants the number of bytes returned by {@link
     * serialize(LittleEndianOutput)}, that is, (length - 4).
     */
    @Override
    ...
    /**
     * @return the link's path, with the special characters stripped/replaced.
     * May be null. See MS-XLS 2.5.277 (VirtualPath)
     */
    public String getReadablePath() {
        if (path != null) {
            // all of the path strings start with either 0x02 or 0x01 followed
            // by zero or
            // more of 0x01..0x08
            int offset = 1;
    ...
    /**
     * Checks if the data source in this reference record is external to this
     * sheet or internal.
     *
