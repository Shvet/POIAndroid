...
public class Biff8RC4Key extends Biff8EncryptionKey {
    ...
    static byte[] createKeyDigest(String password, byte[] docIdData) {
        ...
        byte[] result = CryptoFunctions.getBlock0(md5.digest(),
                KEY_DIGEST_LENGTH);
        return result;
    }
    ...
    private static void check16Bytes(byte[] data, String argName) {
        if (data.length != 16) {
            throw new IllegalArgumentException("Expected 16 byte " + argName
                    + ", but got " + HexDump.toHex(data));
        }
    }

    /**
     * @return <code>true</code> if the keyDigest is compatible with the
     * specified saltData and saltHash
     */
    public boolean validate(byte[] verifier, byte[] verifierHash) {
        ...
        try {
            ...
            rc4.update(verifierHashPrime, 0, verifierHashPrime.length,
                    verifierHashPrime);
        ...
        if (log.check(POILogger.DEBUG)) {
            byte[] verifierHashThatWouldWork = xor(verifierHash,
                    xor(verifierHashPrime, finalVerifierResult));
            log.log(POILogger.DEBUG, "valid verifierHash value",
                    HexDump.toHex(verifierHashThatWouldWork));
        }
    ...
    Cipher getCipher() {
        CipherAlgorithm ca = CipherAlgorithm.rc4;
        Cipher rc4 = CryptoFunctions.getCipher(_secretKey, ca, null, null,
                Cipher.ENCRYPT_MODE);
        return rc4;
    }
    ...
    void initCipherForBlock(Cipher rc4, int keyBlockNo) {
        ...
        SecretKeySpec skeySpec = new SecretKeySpec(md5.digest(),
                _secretKey.getAlgorithm());
        try {
            rc4.init(Cipher.ENCRYPT_MODE, skeySpec);
        } catch (GeneralSecurityException e) {
            throw new EncryptedDocumentException("Can't rekey for next block",
                    e);
        }
    }
