...
/**
 * This class handles serialization of SST records. It utilizes the record
 * processor class write individual records. This has been refactored from the
 * SSTRecord class.
 *
...
final class SSTSerializer {
    ...
    public SSTSerializer(IntMapper<UnicodeString> strings, int numStrings,
        ...
        int infoRecs = ExtSSTRecord.getNumberOfInfoRecsForStrings(strings
                .size());
        this.bucketAbsoluteOffsets = new int[infoRecs];
        this.bucketRelativeOffsets = new int[infoRecs];
    }

    private static UnicodeString getUnicodeString(
            IntMapper<UnicodeString> strings, int index) {
