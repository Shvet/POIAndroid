...
public final class FontRecord extends StandardRecord {
    public final static short sid = 0x0031; // docs are wrong (0x231 Microsoft
    // Support site article Q184647)
    ...
    private static final BitField italic = BitFieldFactory.getInstance(0x02); // is
    // 2 0x04 - reserved bit must be 0
    private static final BitField strikeout = BitFieldFactory.getInstance(0x08); // is
    // this
    // font
    // has
    // a
    ...
    // the
    // center
    private static final BitField macoutline = BitFieldFactory
            .getInstance(0x10); // some weird macintosh thing....but who
    // this
    // font
    // in
    // italics
    // understands those mac people anyhow
    private static final BitField macshadow = BitFieldFactory.getInstance(0x20); // some
    ...
    private byte field_8_charset; // ?? defined by windows api logfont
    // structure?
    ...
    public FontRecord(RecordInputStream in) {
        ...
        int unicodeFlags = in.readUByte(); // options byte present always (even
        // if no character data)

        if (field_10_font_name_len > 0) {
            if (unicodeFlags == 0) { // is compressed unicode
                field_11_font_name = in
                        .readCompressedUnicode(field_10_font_name_len);
            } else { // is not compressed unicode
                field_11_font_name = in
                        .readUnicodeLEString(field_10_font_name_len);
            }
        } else {
            field_11_font_name = "";
        }
    ...
    public void setStrikeout(boolean strike) {
        field_2_attributes = strikeout.setShortBoolean(field_2_attributes,
                strike);
    }
    ...
    /**
     * whether to use the mac outline font style thing (mac only) - Some mac
     * person should comment this instead of me doing it (since I have no idea)
     *
     * @param mac - whether to do that mac font outline thing or not
     * @see #setAttributes(short)
     */
    public void setMacoutline(boolean mac) {
        field_2_attributes = macoutline
                .setShortBoolean(field_2_attributes, mac);
    }

    /**
     * whether to use the mac shado font style thing (mac only) - Some mac
     * person should comment this instead of me doing it (since I have no idea)
     *
    ...
    /**
     * get the font attributes (see individual bit getters that reference this
     * method)
     *
    ...
    /**
     * set the font attributes (see individual bit setters that reference this
     * method)
     *
    ...
    public void setItalic(boolean italics) {
        field_2_attributes = italic
                .setShortBoolean(field_2_attributes, italics);
    }
    ...
    public String toString() {
        ...
        sb.append("    .fontheight    = ")
                .append(HexDump.shortToHex(getFontHeight())).append("\n");
        sb.append("    .attributes    = ")
                .append(HexDump.shortToHex(getAttributes())).append("\n");
        ...
        sb.append("    .colorpalette  = ")
                .append(HexDump.shortToHex(getColorPaletteIndex()))
                .append("\n");
        sb.append("    .boldweight    = ")
                .append(HexDump.shortToHex(getBoldWeight())).append("\n");
        sb.append("    .supersubscript= ")
                .append(HexDump.shortToHex(getSuperSubScript())).append("\n");
        sb.append("    .underline     = ")
                .append(HexDump.byteToHex(getUnderline())).append("\n");
        sb.append("    .family        = ")
                .append(HexDump.byteToHex(getFamily())).append("\n");
        sb.append("    .charset       = ")
                .append(HexDump.byteToHex(getCharset())).append("\n");
    ...
    /**
     * Clones all the font style information from another FontRecord, onto this
     * one. This will then hold all the same font style options.
     */
    ...
    /**
     * Does this FontRecord have all the same font properties as the supplied
     * FontRecord? Note that {@link #equals(Object)} will check for exact
     * objects, while this will check for exact contents, because normally the
     * font record's position makes a big difference too.
     */
    public boolean sameProperties(FontRecord other) {
        return field_1_font_height == other.field_1_font_height
                ...
                && stringEquals(this.field_11_font_name,
                other.field_11_font_name);
    }
