...
import com.shvet.poi.hssf.usermodel.HSSFDataValidation;
 ...
 * <p/>
 * Description: This record stores data validation settings and a list of cell
 * ranges which contain these settings. The data validation settings of a sheet
 * are stored in a sequential list of DV records. This list is followed by DVAL
 * record(s)
 *
...
public final class DVRecord extends StandardRecord {
    ...
    private static final BitField opt_string_list_formula = new BitField(
            0x00000080);
    private static final BitField opt_empty_cell_allowed = new BitField(
            0x00000100);
    private static final BitField opt_suppress_dropdown_arrow = new BitField(
            0x00000200);
    private static final BitField opt_show_prompt_on_cell_selected = new BitField(
            0x00040000);
    private static final BitField opt_show_error_on_invalid_value = new BitField(
            0x00080000);
    private static final BitField opt_condition_operator = new BitField(
            0x00700000);
    /**
     * Option flags
     */
    ...
    public DVRecord(int validationType, int operator, int errorStyle,
                    boolean emptyCellAllowed, boolean suppressDropDownArrow,
                    boolean isExplicitList, boolean showPromptBox, String promptTitle,
                    String promptText, boolean showErrorBox, String errorTitle,
                    String errorText, Ptg[] formula1, Ptg[] formula2,
        ...
        flags = opt_suppress_dropdown_arrow.setBoolean(flags,
                suppressDropDownArrow);
        flags = opt_string_list_formula.setBoolean(flags, isExplicitList);
        flags = opt_show_prompt_on_cell_selected.setBoolean(flags,
                showPromptBox);
    ...
    /**
     * When entered via the UI, Excel translates empty string into "\0" While it
     * is possible to encode the title/text as empty string (Excel doesn't
     * exactly crash), the resulting tool-tip text / message box looks wrong. It
     * is best to do the same as the Excel UI and encode 'not present' as "\0".
     */
    ...
    private static void serializeUnicodeString(UnicodeString us,
    ...
    /**
     * @return <code>true</code> if in list validations the string list is
     * explicitly given in the formula, <code>false</code> otherwise
     */
    ...
    /**
     * @return <code>true</code> if empty values are allowed in cells,
    ...
    /**
     * @return <code>true</code> if drop down arrow should be suppressed when
     * list validation is used, <code>false</code> otherwise
     */
    ...
    /**
     * @return <code>true</code> if a prompt window should appear when cell is
    ...
    /**
     * @return <code>true</code> if an error window should appear when an
     * invalid value is entered in the cell, <code>false</code>
     * otherwise
     */
    ...
    public String toString() {
        ...
        for (int i = 0; i < nRegions; i++) {
            ...
            sb.append('(').append(addr.getFirstRow()).append(',')
                    .append(addr.getLastRow());
            sb.append(',').append(addr.getFirstColumn()).append(',')
                    .append(addr.getLastColumn()).append(')');
        }
        sb.append("\n");
        sb.append("[/DV]");
    ...
    /**
     * Clones the object. Uses serialisation, as the contents are somewhat
     * complex
     */
    public Object clone() {
