...
import com.shvet.poi.util.LittleEndianInputStream;
...
public final class EmbeddedObjectRefSubRecord extends SubRecord {
    ...
    private static POILogger logger = POILogFactory
            .getLogger(EmbeddedObjectRefSubRecord.class);
    ...
    /**
     * note- this byte is not present in the encoding if the string length is
     * zero
     */
    private boolean field_3_unicode_flag; // Flags whether the string is
    // Unicode.
    private String field_4_ole_classname; // Classname of the embedded OLE
    // document (e.g. Word.Document.8)
    /**
     * Formulas often have a single non-zero trailing byte. This is in a similar
     * position to he pre-streamId padding It is unknown if the value is
     * important (it seems to mirror a value a few bytes earlier)
     */
    private Byte field_4_unknownByte;
    private Integer field_5_stream_id; // ID of the OLE stream containing the
    // actual data.
    private byte[] field_6_unknown;

    // currently for testing only - needs review
    public EmbeddedObjectRefSubRecord() {
        field_2_unknownFormulaData = new byte[]{0x02, 0x6C, 0x6A, 0x16, 0x01,}; // just
        // some
        // sample
        // data.
        // These
    ...
    public EmbeddedObjectRefSubRecord(LittleEndianInput in, int size) {
        ...
        // 1223 void XclImpOleObj::ReadPictFmla( XclImpStream& rStrm, sal_uInt16
        // nRecSize )
        ...
        if (field_2_refPtg == null) {
            ...
            // The formula almost looks like tTbl but the row/column values seem
            // like garbage.
        ...
        if (remaining >= dataLenAfterFormula + 3) {
            ...
            if (nChars > 0) {
                // OOO: the 4th way Xcl stores a unicode string: not even a
                // Grbit byte present if length 0
                field_3_unicode_flag = (in.readByte() & 0x01) != 0;
                stringByteCount += LittleEndian.BYTE_SIZE;
                if (field_3_unicode_flag) {
                    field_4_ole_classname = StringUtil
                            .readUnicodeLE(in, nChars);
                    stringByteCount += nChars * 2;
                } else {
                    field_4_ole_classname = StringUtil.readCompressedUnicode(
                            in, nChars);
        ...
        if (nUnexpectedPadding > 0) {
            logger.log(POILogger.ERROR, "Discarding " + nUnexpectedPadding
    ...
    private static Ptg readRefPtg(byte[] formulaRawBytes) {
        LittleEndianInput in = new LittleEndianInputStream(
                new ByteArrayInputStream(formulaRawBytes));
    ...
    protected int getDataSize() {
        int formulaSize = field_2_refPtg == null ? field_2_unknownFormulaData.length
                : field_2_refPtg.getSize();
    ...
    public void serialize(LittleEndianOutput out) {
        ...
        switch (idOffset - (pos - 6)) { // 6 for 3 shorts: sid, dataSize,
            // idOffset
            case 1:
                out.writeByte(field_4_unknownByte == null ? 0x00
                        : field_4_unknownByte.intValue());
                pos++;
            case 0:
                break;
            default:
                throw new IllegalStateException("Bad padding calculation ("
                        + idOffset + ", " + pos + ")");
        }
    ...
    /**
     * Gets the stream ID containing the actual data. The data itself can be
     * found under a top-level directory entry in the OLE2 filesystem under the
     * name "MBD<var>xxxxxxxx</var>" where <var>xxxxxxxx</var> is this ID
     * converted into hex (in big endian order, funnily enough.)
     *
    ...
    public String toString() {
        ...
        if (field_2_refPtg == null) {
            ...
            sb.append("    .formula       = ")
                    .append(field_2_refPtg.toString()).append("\n");
        }
        if (field_4_ole_classname != null) {
            sb.append("    .unicodeFlag   = ").append(field_3_unicode_flag)
                    .append("\n");
            sb.append("    .oleClassname  = ").append(field_4_ole_classname)
                    .append("\n");
        }
