...
import com.shvet.poi.ddf.EscherRecordFactory;
import com.shvet.poi.ddf.EscherSerializationListener;
import com.shvet.poi.ddf.EscherSpRecord;
import com.shvet.poi.ddf.EscherSpgrRecord;
 ...
 * -> MSODRAWING -> OBJ records but the escher records are serialized _across_
 * the MSODRAWING records.
 * <p/>
 ...
 * <p/>
 * DrawingRecord ContinueRecord ... ContinueRecord ObjRecord | TextObjectRecord
 * ..... ContinueRecord ... ContinueRecord ObjRecord | TextObjectRecord
 * NoteRecord ... NoteRecord
 * <p/>
 * To work with shapes we have to read data from Drawing and Continue records
 * into single array of bytes and build escher(office art) records tree from
 * this array. Each shape in drawing layer matches corresponding ObjRecord Each
 * textbox matches corresponding TextObjectRecord
 * <p/>
 * ObjRecord contains information about shape. Thus each ObjRecord corresponds
 * EscherContainerRecord(SPGR)
 * <p/>
...
public final class EscherAggregate extends AbstractEscherHolderRecord {
    ...
    private static POILogger log = POILogFactory
            .getLogger(EscherAggregate.class);
    /**
    ...
    /**
     * list of "tail" records that need to be serialized after all drawing group
     * records
     */
     ...
     *
     * @param createDefaultTree if true creates base tree of the escher records, see
     *                          EscherAggregate.buildBaseTree() else return empty escher
     *                          aggregate
     */
     ...
     * @return true if record is instance of DrawingRecord or ContinueRecord or
     * ObjRecord or TextObjRecord
     */
    private static boolean isDrawingLayerRecord(final short sid) {
        return sid == DrawingRecord.sid || sid == ContinueRecord.sid
                || sid == ObjRecord.sid || sid == TextObjectRecord.sid;
    }

    /**
     * Collapses the drawing records into an aggregate. read Drawing, Obj,
     * TxtObj, Note and Continue records into single byte array, create Escher
     * tree from byte array, create map <EscherRecord, Record>
     *
    ...
    public static EscherAggregate createAggregate(List<RecordBase> records,
        ...
        while (loc + 1 < records.size()
            ...
            try {
                if (!(sid(records, loc) == DrawingRecord.sid || sid(records,
                        loc) == ContinueRecord.sid)) {
                    loc++;
                    continue;
                }
                if (sid(records, loc) == DrawingRecord.sid) {
                    buffer.write(((DrawingRecord) records.get(loc))
                            .getRecordData());
                } else {
                ...
                throw new RuntimeException(
                        "Couldn't get data from drawing/continue records", e);
            }
            loc++;
        }
        ...
        while (pos < buffer.size()) {
            EscherRecord r = recordFactory.createRecord(buffer.toByteArray(),
                    pos);
            int bytesRead = r.fillFields(buffer.toByteArray(), pos,
                    recordFactory);
    ...
    private static boolean isObjectRecord(List<RecordBase> records, int loc) {
        return sid(records, loc) == ObjRecord.sid
                || sid(records, loc) == TextObjectRecord.sid;
    }
    ...
    /**
     * Calculates the xml representation of this record. This is simply a dump
     * of all the records.
     *
    ...
    public int serialize(int offset, byte[] data) {
        ...
        for (Object record : records) {
            ...
            pos += e.serialize(pos, buffer, new EscherSerializationListener() {
                ...
                public void afterRecordSerialize(int offset, short recordId,
                                                 int size, EscherRecord record) {
        ...
        for (i = 1; i < shapes.size(); i++) {
            ...
            if (i == shapes.size() - 1 && endOffset < buffer.length - 1) {
                ...
                System.arraycopy(buffer, endOffset + 1, drawingData, 0,
                        drawingData.length);
                pos += writeDataIntoDrawingRecord(drawingData,
                        writtenEscherBytes, pos, data, i);
            }
        }
        if ((pos - offset) < buffer.length - 1) {
            byte[] drawingData = new byte[buffer.length - (pos - offset)];
            System.arraycopy(buffer, (pos - offset), drawingData, 0,
                    drawingData.length);
            pos += writeDataIntoDrawingRecord(drawingData, writtenEscherBytes,
                    pos, data, i);
        }
        ...
        if (bytesWritten != getRecordSize())
            throw new RecordFormatException(bytesWritten
     ...
     * @param writtenEscherBytes - count of bytes already saved into drawing records (we should
     *                           know it to decide create drawing or continue record)
     ...
     */
    private int writeDataIntoDrawingRecord(byte[] drawingData,
                                           int writtenEscherBytes, int pos, byte[] data, int i) {
        int temp = 0;
        // First record in drawing layer MUST be DrawingRecord
        if (writtenEscherBytes + drawingData.length > RecordInputStream.MAX_RECORD_DATA_SIZE
                && i != 1) {
            for (int j = 0; j < drawingData.length; j += RecordInputStream.MAX_RECORD_DATA_SIZE) {
                byte[] buf = new byte[Math.min(
                        RecordInputStream.MAX_RECORD_DATA_SIZE,
                        drawingData.length - j)];
                System.arraycopy(drawingData, j, buf, 0, Math.min(
                        RecordInputStream.MAX_RECORD_DATA_SIZE,
                        drawingData.length - j));
    ...
    /**
     * @return record size, including header size of obj, text, note, drawing,
     * continue records
     */
    public int getRecordSize() {
        // To determine size of aggregate record we have to know size of each
        // DrawingRecord because if DrawingRecord
        // is split into several continue records we have to add header size to
        // total EscherAggregate size
        ...
        for (int i = 1; i < spEndingOffsets.size(); i++) {
            ...
            continueRecordsHeadersSize += ((spEndingOffsets.get(i) - spEndingOffsets
                    .get(i - 1)) / RecordInputStream.MAX_RECORD_DATA_SIZE) * 4;
        }
     ...
     * -EscherSpgrContainer --EscherSpContainer ---EscherSpRecord
     * ---EscherSpgrRecord ---EscherSpRecord -EscherDgRecord
     * <p/>
    ...
    /**
     * EscherDgContainer -EscherSpgrContainer -EscherDgRecord - set id for this
     * record set id for DgRecord of DgContainer
     *
    ...
    /**
     * EscherDgContainer -EscherSpgrContainer --EscherSpContainer
     * ---EscherSpRecord -set id for this record ---*** --*** -EscherDgRecord
    ...
    public void setMainSpRecordId(int shapeId) {
        EscherContainerRecord dgContainer = getEscherContainer();
        EscherContainerRecord spgrConatiner = dgContainer
                .getChildById(EscherContainerRecord.SPGR_CONTAINER);
        EscherContainerRecord spContainer = (EscherContainerRecord) spgrConatiner
                .getChild(0);
        EscherSpRecord sp = spContainer
                .getChildById(EscherSpRecord.RECORD_ID);
        sp.setShapeId(shapeId);
    }

    /**
     * @return unmodifiable copy of the mapping of
     * {@link EscherClientDataRecord} and {@link EscherTextboxRecord} to
     * their {@link TextObjectRecord} or {@link ObjRecord} .
     * <p/>
     * We need to access it outside of EscherAggregate when building
     * shapes
     */
    ...
    /**
     * @return unmodifiable copy of tail records. We need to access them when
     * building shapes. Every HSSFComment shape has a link to a
     * NoteRecord from the tailRec collection.
     */
     ...
     * @return null if note record is not found else returns note record with id
     * == obj.id
     */
    public NoteRecord getNoteRecordByObj(ObjRecord obj) {
        CommonObjectDataSubRecord cod = (CommonObjectDataSubRecord) obj
                .getSubRecords().get(0);
