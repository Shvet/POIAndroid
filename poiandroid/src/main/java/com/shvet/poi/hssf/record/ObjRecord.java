...
public final class ObjRecord extends Record {
    ...
    public ObjRecord() {
        ...
        // TODO - ensure 2 sub-records (ftCmo 15h, and ftEnd 00h) are always
        // created
    ...
    public ObjRecord(RecordInputStream in) {
        ...
        if (LittleEndian.getUShort(subRecordData, 0) != CommonObjectDataSubRecord.sid) {
            // seems to occur in just one junit on "OddStyleRecord.xls" (file
            // created by CrystalReports)
            // Excel tolerates the funny ObjRecord, and replaces it with a
            // corrected version
        ...
        /*
         * if (subRecordData.length % 2 != 0) { String msg =
		 * "Unexpected length of subRecordData : " +
		 * HexDump.toHex(subRecordData); throw new RecordFormatException(msg); }
		 */

        subrecords = new ArrayList<SubRecord>();
        ByteArrayInputStream bais = new ByteArrayInputStream(subRecordData);
        LittleEndianInputStream subRecStream = new LittleEndianInputStream(bais);
        CommonObjectDataSubRecord cmo = (CommonObjectDataSubRecord) SubRecord
                .createSubRecord(subRecStream, 0);
        subrecords.add(cmo);
        while (true) {
        ...
        if (nRemainingBytes > 0) {
            // At present (Oct-2008), most unit test samples have
            // (subRecordData.length % 2 == 0)
            _isPaddedToQuadByteMultiple = subRecordData.length
                    % MAX_PAD_ALIGNMENT == 0;
            if (nRemainingBytes >= (_isPaddedToQuadByteMultiple ? MAX_PAD_ALIGNMENT
    ...
    /**
     * Some XLS files have ObjRecords with nearly 8Kb of excessive padding.
     * These were probably written by a version of POI (around 3.1) which
     * incorrectly interpreted the second short of the ftLbs subrecord (0x1FEE)
     * as a length, and read that many bytes as padding (other bugs helped allow
     * this to occur).
     * <p/>
     * Excel reads files with this excessive padding OK, truncating the
     * over-sized ObjRecord back to the its proper size. POI does the same.
     */
    private static boolean canPaddingBeDiscarded(byte[] data,
                                                 int nRemainingBytes) {
    ...
    public String toString() {
        ...
        sb.append("[OBJ]\n");
        if (subrecords != null) { // there are special cases where this can be,
    ...
    public int serialize(int offset, byte[] data) {
        ...
        LittleEndianByteArrayOutputStream out = new LittleEndianByteArrayOutputStream(
                data, offset, recSize);
