...
import com.shvet.poi.hssf.record.cont.ContinuableRecord;
import com.shvet.poi.hssf.record.cont.ContinuableRecordOutput;
...
import com.shvet.poi.util.LittleEndianByteArrayInputStream;
...
public final class NameRecord extends ContinuableRecord {
    ...
    /**
     * One-based extern index of sheet (resolved via LinkTable). Zero if this is
     * a global name
     */
     ...
     *
     * @param builtin Built-in byte representation for the name record, use the
     *                public constants
     */
    public NameRecord(byte builtin, int sheetNumber) {
        ...
        field_6_sheetNumber = sheetNumber; // the extern sheets are set through
        // references
    }
    ...
    public NameRecord(RecordInputStream ris) {
        ...
        if (isBuiltInName()) {
            ...
            if (field_11_nameIsMultibyte) {
                field_12_name_text = StringUtil.readUnicodeLE(in,
                        field_3_length_name_text);
            } else {
                field_12_name_text = StringUtil.readCompressedUnicode(in,
                        field_3_length_name_text);
            }
        }

        int nBytesAvailable = in.available()
                - (f7_customMenuLen + f8_descriptionTextLen
                + f9_helpTopicTextLen + f10_statusBarTextLen);
        field_13_name_definition = Formula.read(field_4_length_name_definition,
                in, nBytesAvailable);

        // Who says that this can only ever be compressed unicode???
        field_14_custom_menu_text = StringUtil.readCompressedUnicode(in,
                f7_customMenuLen);
        field_15_description_text = StringUtil.readCompressedUnicode(in,
                f8_descriptionTextLen);
        field_16_help_topic_text = StringUtil.readCompressedUnicode(in,
                f9_helpTopicTextLen);
        field_17_status_bar_text = StringUtil.readCompressedUnicode(in,
                f10_statusBarTextLen);
    }
    ...
    /**
     * Indicates that the defined name refers to a user-defined function. This
     * attribute is used when there is an add-in or other code project
    ...
    /**
     * @return <code>true</code> if name has a formula (named range or defined
     * value)
     */
    public boolean hasFormula() {
        return Option.isFormula(field_1_option_flag)
                && field_13_name_definition.getEncodedTokenSize() > 0;
    }
    ...
    public String getNameText() {
        ...
        return isBuiltInName() ? translateBuiltInName(getBuiltInName())
                : field_12_name_text;
    }
    ...
    public void serialize(ContinuableRecordOutput out) {
        ...
        int field_8_length_description_text = field_15_description_text
                .length();
        ...
        // Note - formula size is not immediately before encoded formula, and
        // does not include any array constant data
	...
	/*
     * 20 00 00 01 1A 00 // sz = 0x1A = 26 00 00 01 00 00 00 00 00 00 // unicode
	 * flag 07 // name
	 * 
	 * 29 17 00 3B 00 00 00 00 FF FF 00 00 02 00 3B 00 //{ 26 00 07 00 07 00 00
	 * 00 FF 00 10 // }
	 * 
	 * 
	 * 
	 * 20 00 00 01 0B 00 // sz = 0xB = 11 00 00 01 00 00 00 00 00 00 // unicode
	 * flag 07 // name
	 * 
	 ...
	 * 18, 00, 1B, 00,
	 * 
	 * 20, 00, 00, 01, 0B, 00, 00, 00, 00, 00, 00, 07, 3B 00 00 07 00 07 00 00
	 * 00 FF 00 ]
	 */
    ...
    public String toString() {
        ...
        sb.append("    .keyboard shortcut      = ")
                .append(HexDump.byteToHex(field_2_keyboard_shortcut))
                .append("\n");
        sb.append("    .length of the name     = ").append(getNameTextLength())
                .append("\n");
        sb.append("    .extSheetIx(1-based, 0=Global)= ")
                .append(field_5_externSheetIndex_plus1).append("\n");
        sb.append("    .sheetTabIx             = ").append(field_6_sheetNumber)
                .append("\n");
        ...
        sb.append("    .Name (Unicode text)    = ").append(getNameText())
                .append("\n");
        Ptg[] ptgs = field_13_name_definition.getTokens();
        sb.append("    .Formula (nTokens=").append(ptgs.length).append("):")
                .append("\n");
        ...
        sb.append("    .Menu text       = ").append(field_14_custom_menu_text)
                .append("\n");
        sb.append("    .Description text= ").append(field_15_description_text)
                .append("\n");
        sb.append("    .Help topic text = ").append(field_16_help_topic_text)
                .append("\n");
        sb.append("    .Status bar text = ").append(field_17_status_bar_text)
                .append("\n");
        sb.append("[/NAME]\n");
