...
import com.shvet.poi.hssf.record.CRNCountRecord;
import com.shvet.poi.hssf.record.CRNRecord;
import com.shvet.poi.hssf.record.CountryRecord;
import com.shvet.poi.hssf.record.ExternSheetRecord;
import com.shvet.poi.hssf.record.ExternalNameRecord;
 ...
 * the Workbook Globals Substream (4.2.5). The Link Table itself is optional and
 * occurs only if there are any references in the document.
 * <p/>
 * <p/>
 * In BIFF8 the Link Table consists of
 * <ul>
 * <li>zero or more EXTERNALBOOK Blocks
 * <p/>
 * each consisting of
 * <ul>
 * <li>exactly one EXTERNALBOOK (0x01AE) record</li>
 * <li>zero or more EXTERNALNAME (0x0023) records</li>
 * <li>zero or more CRN Blocks
 * <p/>
 * each consisting of
 * <ul>
...
final class LinkTable {
    ...
    private final WorkbookRecordList _workbookRecordList; // TODO - would be
    ...
    public LinkTable(List<Record> inputList, int startIndex,
                     ...
                     Map<String, NameCommentRecord> commentRecords) {
        ...
        if (_externalBookBlocks.length > 0) {
            // If any ExternalBookBlock present, there is always 1 of
            // ExternSheetRecord
        ...
        _workbookRecordList.getRecords().addAll(
    ...
    public LinkTable(int numberOfSheets, WorkbookRecordList workbookRecordList) {
        _workbookRecordList = workbookRecordList;
        _definedNames = new ArrayList<NameRecord>();
        _externalBookBlocks = new ExternalBookBlock[]{new ExternalBookBlock(
                numberOfSheets),};
    ...
    private static boolean isDuplicatedNames(NameRecord firstName,
                                             NameRecord lastName) {
    ...
    public NameRecord getSpecificBuiltinRecord(byte builtInCode, int sheetNumber) {
        ...
        while (iterator.hasNext()) {
            ...
            if (record.getBuiltInName() == builtInCode
                    && record.getSheetNumber() == sheetNumber) {
                return record;
            }
        }
    ...
    public void removeBuiltinRecord(byte name, int sheetIndex) {
        ...
        // TODO - do we need "Workbook.records.remove(...);" similar to that in
        // Workbook.removeName(int namenum) {}?
    }
    ...
    public void addName(NameRecord name) {
        ...
        // TODO - this is messy
        // Not the most efficient way but the other way was causing too many
        // bugs
    ...
    public String[] getExternalBookAndSheetName(int extRefIndex) {
        ...
        int shIx1 = _externSheetRecord
                .getFirstSheetIndexFromRefIndex(extRefIndex);
        int shIx2 = _externSheetRecord
                .getLastSheetIndexFromRefIndex(extRefIndex);
    ...
    private int getExternalWorkbookIndex(String workbookName) {
        for (int i = 0; i < _externalBookBlocks.length; i++) {
            ...
            if (workbookName.equals(ebr.getURL())) { // not sure if 'equals()'
                // works when url has a
                // directory
                return i;
            }
        }
    ...
    public int getExternalSheetIndex(String workbookName,
        ...
        SupBookRecord ebrTarget = _externalBookBlocks[externalBookIndex]
    ...
    /**
     * @deprecated Was prevously used for removing sheets, which we now do
     * differently
     */
    @Deprecated
    ...
    public int checkExternSheet(int firstSheetIndex, int lastSheetIndex) {
        ...
        int i = _externSheetRecord.getRefIxForSheet(thisWbIndex,
                firstSheetIndex, lastSheetIndex);
        if (i >= 0) {
            return i;
        }
        // We haven't found reference to this sheet
        return _externSheetRecord.addRef(thisWbIndex, firstSheetIndex,
                lastSheetIndex);
    }
    ...
    private int findFirstRecordLocBySid(short sid) {
        int index = 0;
        for (Iterator<Record> iterator = _workbookRecordList.iterator(); iterator
                .hasNext(); ) {
    ...
    public String resolveNameXText(int refIndex, int definedNameIndex,
        ...
        int extBookIndex = _externSheetRecord
                .getExtbookIndexFromRefIndex(refIndex);
        int firstTabIndex = _externSheetRecord
                .getFirstSheetIndexFromRefIndex(refIndex);
        if (firstTabIndex == -1) {
        ...
        if (externalBook._externalNameRecords.length > definedNameIndex) {
            return _externalBookBlocks[extBookIndex]
                    .getNameText(definedNameIndex);
    ...
    /**
     * Finds the external name definition for the given name, optionally
     * restricted by externsheet index, and returns (if found) as a NameXPtg.
     *
    ...
    private static final class ExternalBookBlock {
         ...
         *
         * @see com.shvet.poi.hssf.model.LinkTable#LinkTable(int,
         * WorkbookRecordList)
         */
        public ExternalBookBlock(int numberOfSheets) {
            _externalBookRecord = SupBookRecord
                    .createInternalReferences((short) numberOfSheets);
         ...
         *
         * @see com.shvet.poi.hssf.model.LinkTable#addNameXPtg(String)
         */
