...
import com.shvet.poi.hssf.record.cf.FontFormatting;
import com.shvet.poi.hssf.record.cf.PatternFormatting;
 ...
 * <p/>
 * <p/>
 * This is for the older-style Excel conditional formattings, new-style (Excel
 * 2007+) also make use of {@link CFRule12Record} and {@link CFExRuleRecord} for
 * their rules.
 */
public abstract class CFRuleBase extends StandardRecord {
    ...
    protected static final POILogger logger = POILogFactory
            .getLogger(CFRuleBase.class);
    static final BitField modificationBits = bf(0x003FFFFF); // Bits:
    // font,align,bord,patt,prot
    static final BitField alignHor = bf(0x00000001); // 0 = Horizontal alignment
    // modified
    static final BitField alignVer = bf(0x00000002); // 0 = Vertical alignment
    // modified
    static final BitField alignWrap = bf(0x00000004); // 0 = Text wrapped flag
    // modified
    static final BitField alignRot = bf(0x00000008); // 0 = Text rotation
    // modified
    static final BitField alignJustLast = bf(0x00000010); // 0 = Justify last
    // line flag
    // modified
    static final BitField alignIndent = bf(0x00000020); // 0 = Indentation
    // modified
    static final BitField alignShrin = bf(0x00000040); // 0 = Shrink to fit flag
    // modified
    static final BitField mergeCell = bf(0x00000080); // Normally 1, 0 = Merge
    // Cell flag modified
    static final BitField protLocked = bf(0x00000100); // 0 = Cell locked flag
    // modified
    static final BitField protHidden = bf(0x00000200); // 0 = Cell hidden flag
    // modified
    static final BitField bordLeft = bf(0x00000400); // 0 = Left border style
    // and colour modified
    static final BitField bordRight = bf(0x00000800); // 0 = Right border style
    // and colour modified
    static final BitField bordTop = bf(0x00001000); // 0 = Top border style and
    // colour modified
    static final BitField bordBot = bf(0x00002000); // 0 = Bottom border style
    // and colour modified
    static final BitField bordTlBr = bf(0x00004000); // 0 = Top-left to
    // bottom-right border
    // flag modified
    static final BitField bordBlTr = bf(0x00008000); // 0 = Bottom-left to
    // top-right border flag
    // modified
    static final BitField pattStyle = bf(0x00010000); // 0 = Pattern style
    // modified
    static final BitField pattCol = bf(0x00020000); // 0 = Pattern colour
    // modified
    static final BitField pattBgCol = bf(0x00040000); // 0 = Pattern background
    // colour modified
    static final BitField notUsed2 = bf(0x00380000); // Always 111 (ifmt / ifnt
    // / 1)
    static final BitField undocumented = bf(0x03C00000); // Undocumented bits
    static final BitField fmtBlockBits = bf(0x7C000000); // Bits:
    // font,align,bord,patt,prot
    static final BitField font = bf(0x04000000); // 1 = Record contains font
    // formatting block
    static final BitField align = bf(0x08000000); // 1 = Record contains
    // alignment formatting
    // block
    static final BitField bord = bf(0x10000000); // 1 = Record contains border
    // formatting block
    static final BitField patt = bf(0x20000000); // 1 = Record contains pattern
    // formatting block
    static final BitField prot = bf(0x40000000); // 1 = Record contains
    // protection formatting
    // block
    static final BitField alignTextDir = bf(0x80000000); // 0 = Text direction
     ...
     * @return encoded size of the formula tokens (does not include 2 bytes for
     * ushort length)
     */
    ...
    /**
     * TODO - parse conditional format formulas properly i.e. produce tRefN and
     * tAreaN instead of tRef and tArea this call will produce the wrong results
     * if the formula contains any cell references One approach might be to
     * apply the inverse of SharedFormulaRecord.convertSharedFormulas(Stack,
     * int, int) Note - two extra parameters (rowIx & colIx) will be required.
     * They probably come from one of the Region objects.
     *
    ...
    public static Ptg[] parseFormula(String formula, HSSFSheet sheet) {
        ...
        return HSSFFormulaParser.parse(formula, sheet.getWorkbook(),
        ...
        if ((this instanceof CFRuleRecord)) {
            if (condition_type == CONDITION_TYPE_CELL_VALUE_IS
                ...
                throw new IllegalArgumentException(
                        "CFRuleRecord only accepts Value-Is and Formula types");
            }
        }
    ...
    public void setComparisonOperation(byte operation) {
        if (operation < 0 || operation > ComparisonOperator.max_operator)
            throw new IllegalArgumentException(
                    "Valid operators are only in the range 0 to "
                            + ComparisonOperator.max_operator);
    ...
    protected int getFormattingBlockSize() {
        return 6
                + (containsFontFormattingBlock() ? _fontFormatting
                .getRawRecord().length : 0)
    ...
    protected void copyTo(CFRuleBase rec) {
        ...
        if (containsBorderFormattingBlock()) {
            rec._borderFormatting = (BorderFormatting) _borderFormatting
                    .clone();
        }
        if (containsPatternFormattingBlock()) {
            rec._patternFormatting = (PatternFormatting) _patternFormatting
                    .clone();
        }
