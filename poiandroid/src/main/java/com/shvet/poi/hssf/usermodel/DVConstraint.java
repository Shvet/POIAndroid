...
import com.shvet.poi.ss.formula.FormulaRenderer;
...
import com.shvet.poi.ss.usermodel.DataValidationConstraint;
...
public class DVConstraint implements DataValidationConstraint {
    ...
    private DVConstraint(int validationType, int comparisonOperator,
                         String formulaA, String formulaB, Double value1, Double value2,
                         String[] excplicitListValues) {
    ...
    private DVConstraint(String listFormula, String[] excplicitListValues) {
        this(ValidationType.LIST, OperatorType.IGNORED, listFormula, null,
                null, null, excplicitListValues);
    }

    /**
     * Creates a number based data validation constraint. The text values
     * entered for expr1 and expr2 can be either standard Excel formulas or
     * formatted number values. If the expression starts with '=' it is parsed
     * as a formula, otherwise it is parsed as a formatted number.
     *
     * @param validationType     one of
     *                           {@link org.apache.poi.ss.usermodel.DataValidationConstraint.ValidationType#ANY}
     *                           ,
     ...
     * @param comparisonOperator any constant from
     *                           {@link org.apache.poi.ss.usermodel.DataValidationConstraint.OperatorType}
     *                           enum
     * @param expr1              date formula (when first char is '=') or formatted number
     *                           value
     * @param expr2              date formula (when first char is '=') or formatted number
     *                           value
     */
    public static DVConstraint createNumericConstraint(int validationType,
                                                       int comparisonOperator, String expr1, String expr2) {
        ...
        return new DVConstraint(validationType, comparisonOperator, formula1,
                formula2, value1, value2, null);
    }
    ...
    public static DVConstraint createExplicitListConstraint(
            String[] explicitListValues) {
        return new DVConstraint(null, explicitListValues);
    }

    /**
     * Creates a time based data validation constraint. The text values entered
     * for expr1 and expr2 can be either standard Excel formulas or formatted
     * time values. If the expression starts with '=' it is parsed as a formula,
     * otherwise it is parsed as a formatted time. To parse formatted times, two
     * formats are supported: "HH:MM" or "HH:MM:SS". This is contrary to Excel
     * which uses the default time format from the OS.
     *
     * @param comparisonOperator constant from
     *                           {@link org.apache.poi.ss.usermodel.DataValidationConstraint.OperatorType}
     *                           enum
    ...
    public static DVConstraint createTimeConstraint(int comparisonOperator,
        ...
        return new DVConstraint(ValidationType.TIME, comparisonOperator,
                formula1, formula2, value1, value2, null);
    }

    /**
     * Creates a date based data validation constraint. The text values entered
     * for expr1 and expr2 can be either standard Excel formulas or formatted
     * date values. If the expression starts with '=' it is parsed as a formula,
     * otherwise it is parsed as a formatted date (Excel uses the same
     * convention). To parse formatted dates, a date format needs to be
     * specified. This is contrary to Excel which uses the default short date
     * format from the OS.
     *
     * @param comparisonOperator constant from
     *                           {@link org.apache.poi.ss.usermodel.DataValidationConstraint.OperatorType}
     ...
     * @param dateFormat         ignored if both expr1 and expr2 are formulas. Default value is
     *                           "YYYY/MM/DD" otherwise any other valid argument for
     *                           <tt>SimpleDateFormat</tt> can be used
     * @see <a href=
     * 'http://java.sun.com/j2se/1.5.0/docs/api/java/text/DateFormat.html'>SimpleDateFormat</
     * a >
     */
    public static DVConstraint createDateConstraint(int comparisonOperator,
                                                    String expr1, String expr2, String dateFormat) {
        if (expr1 == null) {
            throw new IllegalArgumentException("expr1 must be supplied");
        }
        OperatorType.validateSecondArg(comparisonOperator, expr2);
        ...
        return new DVConstraint(ValidationType.DATE, comparisonOperator,
                formula1, formula2, value1, value2, null);
    }

    /**
     * Distinguishes formula expressions from simple value expressions. This
     * logic is only required by a few factory methods in this class that create
     * data validation constraints from more or less the same parameters that
     * would have been entered in the Excel UI. The data validation dialog box
     * uses the convention that formulas begin with '='. Other methods in this
     * class follow the POI convention (formulas and values are distinct), so
     * the '=' convention is not used there.
     *
     * @param textExpr a formula or value expression
     * @return all text after '=' if textExpr begins with '='. Otherwise
     * <code>null</code> if textExpr does not begin with '='
     */
    private static String getFormulaFromTextExpression(String textExpr) {
        ...
        if (textExpr.length() < 1) {
            throw new IllegalArgumentException(
                    "Empty string is not a valid formula/value expression");
        }
     ...
     * @return <code>null</code> if timeStr is <code>null</code>
     */
    private static Double convertDate(String dateStr,
    ...
    public static DVConstraint createCustomFormulaConstraint(String formula) {
        ...
        return new DVConstraint(ValidationType.FORMULA, OperatorType.IGNORED,
                formula, null, null, null, null);
    }

    /**
     * @return The parsed token array representing the formula or value
     * specified. Empty array if both formula and value are
     * <code>null</code>
     */
    @SuppressWarnings("resource")
    private static Ptg[] convertDoubleFormula(String formula, Double value,
                                              HSSFSheet sheet) {
        if (formula == null) {
            if (value == null) {
        ...
        if (value != null) {
            throw new IllegalStateException(
                    "Both formula and value cannot be present");
        }
        HSSFWorkbook wb = sheet.getWorkbook();
        return HSSFFormulaParser.parse(formula, wb, FormulaType.CELL,
                wb.getSheetIndex(sheet));
    }

    static DVConstraint createDVConstraint(DVRecord dvRecord,
                                           FormulaRenderingWorkbook book) {
        switch (dvRecord.getDataType()) {
            case ValidationType.ANY:
                return new DVConstraint(ValidationType.ANY,
                        dvRecord.getConditionOperator(), null, null, null, null,
                        null);
            case ValidationType.INTEGER:
            case ValidationType.DECIMAL:
            ...
            case ValidationType.TEXT_LENGTH:
                FormulaValuePair pair1 = toFormulaString(dvRecord.getFormula1(),
                        book);
                FormulaValuePair pair2 = toFormulaString(dvRecord.getFormula2(),
                        book);
                return new DVConstraint(dvRecord.getDataType(),
                        dvRecord.getConditionOperator(), pair1.formula(),
                        pair2.formula(), pair1.value(), pair2.value(), null);
            case ValidationType.LIST:
                if (dvRecord.getListExplicitFormula()) {
                    String values = toFormulaString(dvRecord.getFormula1(), book)
                            .string();
                    ...
                    String listFormula = toFormulaString(dvRecord.getFormula1(),
                            book).string();
                    return createFormulaListConstraint(listFormula);
                }
            case ValidationType.FORMULA:
                return createCustomFormulaConstraint(toFormulaString(
                        dvRecord.getFormula1(), book).string());
            default:
                throw new UnsupportedOperationException("validationType="
                        + dvRecord.getDataType());
        }
    }

    private static FormulaValuePair toFormulaString(Ptg[] ptgs,
                                                    FormulaRenderingWorkbook book) {
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#getValidationType
     * ()
     */
     ...
     * Convenience method
     *
     * @return <code>true</code> if this constraint is a 'list' validation with
     * explicit values
     */
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#setOperator(int)
     */
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#getExplicitListValues
     * ()
     */
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#setExplicitListValues
     * (java.lang.String[])
     */
    public void setExplicitListValues(String[] explicitListValues) {
        if (_validationType != ValidationType.LIST) {
            throw new RuntimeException(
                    "Cannot setExplicitListValues on non-list constraint");
        }
        _formula1 = null;
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#setFormula1(java
     * .lang.String)
     */
     ...
     *
     * @see
     * org.apache.poi.hssf.usermodel.DataValidationConstraint#setFormula2(java
     * .lang.String)
     */
    ...
    private Ptg[] createListFormula(HSSFSheet sheet) {
        ...
        if (_explicitListValues == null) {
            HSSFWorkbook wb = sheet.getWorkbook();
            // formula is parsed with slightly different RVA rules: (root node
            // type must be 'reference')
            return HSSFFormulaParser.parse(_formula1, wb,
                    FormulaType.DATAVALIDATION_LIST, wb.getSheetIndex(sheet));
            // To do: Excel places restrictions on the available operations
            // within a list formula.
