...
/**
 * <tt>RecordAggregate</tt>s are groups of of BIFF <tt>Record</tt>s that are
 * typically stored together and/or updated together. Workbook / Sheet records
 * are typically stored in a sequential list, which does not provide much
 * structure to coordinate updates.
 *
 * @author Josh Micich
 */
public abstract class RecordAggregate extends RecordBase {
     ...
     * {@link RecordAggregate} in the order that they should be written to file.
     * Implementors may or may not return the actual {@link Record}s being used
     * to manage POI's internal implementation. Callers should not assume either
     * way, and therefore only attempt to modify those {@link Record}s after
     * cloning
     */
    public abstract void visitContainedRecords(RecordVisitor rv);

    public final int serialize(int offset, byte[] data) {
        SerializingRecordVisitor srv = new SerializingRecordVisitor(data,
                offset);
    ...
    private static final class SerializingRecordVisitor implements
            RecordVisitor {
