...
public class ExternSheetRecord extends StandardRecord {
        ...
        for (int i = 0; i < nItems; i++) {
            ...
            if (refSubRecord.getFirstSheetIndex() == sheetIdx
                ...
                toRemove = i;
                    ...
                    && refSubRecord.getLastSheetIndex() > sheetIdx) {
                _list.set(i,
                        new RefSubRecord(refSubRecord.getExtBookIndex(),
                                refSubRecord.getFirstSheetIndex() - 1,
                                refSubRecord.getLastSheetIndex() - 1));
            }
        }
    ...
    /**
     * Returns the first sheet that the reference applies to, or -1 if the
     * referenced sheet can't be found, or -2 if the reference is workbook
     * scoped.
     */
    ...
    /**
     * Returns the last sheet that the reference applies to, or -1 if the
     * referenced sheet can't be found, or -2 if the reference is workbook
     * scoped. For a single sheet reference, the first and last should be the
     * same.
     */
     ...
     * {@link org.apache.poi.hssf.record.SupBookRecord}.
     * <p>
     ...
     * <ol>
     * <li><code>-2</code> Workbook-level reference that applies to the entire
     * workbook.</li>
     * <li><code>-1</code> Sheet-level reference.</li>
     * <li><code>&gt;=0</code> Sheet-level reference. This specifies the first
     * sheet in the reference.
     * <p>
     * If the SupBook type is unused or external workbook referencing, then this
     * value specifies the zero-based index of an external sheet name, see
     * {@link org.apache.poi.hssf.record.SupBookRecord#getSheetNames()}. This
     * referenced string specifies the name of the first sheet within the
     * external workbook that is in scope. This sheet MUST be a worksheet or
     * macro sheet.
     * <p>
     * <p>
     ...
     * workbook stream that specifies the first sheet within the scope of this
     * reference. This sheet MUST be a worksheet or a macro sheet.
     * </p>
     * </li>
     * </ol>
     *
    ...
    public int addRef(int extBookIndex, int firstSheetIndex, int lastSheetIndex) {
        _list.add(new RefSubRecord(extBookIndex, firstSheetIndex,
                lastSheetIndex));
        return _list.size() - 1;
    }

    public int getRefIxForSheet(int externalBookIndex, int firstSheetIndex,
                                int lastSheetIndex) {
        int nItems = _list.size();
        for (int i = 0; i < nItems; i++) {
    ...
    private static final class RefSubRecord {
        ...
        /**
         * index to External Book Block (which starts with a EXTERNALBOOK
         * record)
         */
         ...
         */
        public RefSubRecord(int extBookIndex, int firstSheetIndex,
                            int lastSheetIndex) {
