...
public final class NameCommentRecord extends StandardRecord {
    ...
    public NameCommentRecord(final RecordInputStream ris) {
        ...
        if (in.readByte() == 0) {
            ...
            field_6_name_text = StringUtil.readUnicodeLE(in,
                    field_4_name_length);
        }
        if (in.readByte() == 0) {
    ...
    public void serialize(final LittleEndianOutput out) {
        ...
        boolean isCommentMultiByte = StringUtil
                .hasMultibyte(field_7_comment_text);
    ...
    protected int getDataSize() {
        return 18 // 4 shorts + 1 long + 2 spurious 'nul's
                + (StringUtil.hasMultibyte(field_6_name_text) ? field_6_name_text
                .length() * 2 : field_6_name_text.length())
                + (StringUtil.hasMultibyte(field_7_comment_text) ? field_7_comment_text
                .length() * 2 : field_7_comment_text.length());
    }
    ...
    public String toString() {
        ...
        sb.append("    .frt cell ref flag      = ")
                .append(HexDump.byteToHex(field_2_frt_cell_ref_flag))
                .append("\n");
        sb.append("    .reserved               = ").append(field_3_reserved)
                .append("\n");
        sb.append("    .name length            = ")
                .append(field_6_name_text.length()).append("\n");
        sb.append("    .comment length         = ")
                .append(field_7_comment_text.length()).append("\n");
        sb.append("    .name                   = ").append(field_6_name_text)
                .append("\n");
        sb.append("    .comment                = ")
    ...
    /**
     * Updates the name we're associated with, normally used when renaming that
     * Name
     */
