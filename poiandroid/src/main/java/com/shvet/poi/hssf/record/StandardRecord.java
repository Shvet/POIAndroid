...
public abstract class StandardRecord extends Record {
    ...
    public final int serialize(int offset, byte[] data) {
        ...
        if (out.getWriteIndex() - offset != recSize) {
            throw new IllegalStateException("Error in serialization of ("
                    + getClass().getName() + "): "
                    + "Incorrect number of bytes written - expected " + recSize
                    + " but got " + (out.getWriteIndex() - offset));
        }
        return recSize;
    }
     ...
     * size' header fields have already been written by the superclass.<br/>
     * <p/>
     * The number of bytes written must equal the record size reported by
     * {@link org.apache.poi.hssf.record.Record#getRecordSize()} minus four (
     * record header consiting of a 'ushort sid' and 'ushort reclength' has
     * already been written by thye superclass).
     */
