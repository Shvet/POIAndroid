...
import com.shvet.poi.hpsf.HPSFException;
import com.shvet.poi.hpsf.MutablePropertySet;
import com.shvet.poi.hpsf.PropertySet;
import com.shvet.poi.hpsf.PropertySetFactory;
import com.shvet.poi.hpsf.SummaryInformation;
import com.shvet.poi.hpsf.WritingNotSupportedException;
import com.shvet.poi.poifs.crypt.EncryptionInfo;
import com.shvet.poi.poifs.filesystem.DirectoryNode;
import com.shvet.poi.poifs.filesystem.DocumentInputStream;
...
/**
 * This holds the common functionality for all POI Document classes. Currently,
 * this relates to Document Information Properties
 */
public abstract class POIDocument {
     ...
     *
     * @return The Document Summary Information or null if it could not be read
     * for this document.
     */
     ...
     *
     * @return The Summary information for the document or null if it could not
     * be read for this document.
     */
    ...
    /**
     * Will create whichever of SummaryInformation and
     * DocumentSummaryInformation (HPSF) properties are not already part of your
     * document. This is normally useful when creating a new document from
     * scratch. If the information properties are already there, then nothing
     * will happen.
     */
     ...
     * Properties (HPSF). If a given property set is missing or corrupt, it will
     * remain null;
     */
    protected void readProperties() {
        ...
        if (ps instanceof SummaryInformation) {
            ...
            logger.log(
                    POILogger.WARN,
                    "SummaryInformation property set came back with wrong class - ",
                    ps.getClass());
        }
    ...
    /**
     * For a given named property entry, either return it or null if if it
     * wasn't found
     *
    ...
    protected PropertySet getPropertySet(String setName,
        ...
        if (encryptionInfo != null) {
            try {
                InputStream is = encryptionInfo.getDecryptor().getDataStream(
                        directory);
                ...
                logger.log(POILogger.ERROR,
                        "Error getting encrypted property set with name "
                                + setName, e);
                return null;
            }
        }
        ...
        try {
            ...
            logger.log(POILogger.WARN, "Error getting property set with name "
                    + setName + "\n" + ie);
            return null;
        }
        ...
        try {
            ...
            logger.log(POILogger.WARN, "Error creating property set with name "
                    + setName + "\n" + ie);
     ...
     * into the currently open NPOIFSFileSystem TODO Implement in-place update
     *
     * @throws IOException if an error when writing to the open {@link NPOIFSFileSystem}
     *                     occurs TODO throws exception if open from stream not file
     */
     ...
     * @throws IOException if an error when writing to the {@link NPOIFSFileSystem}
     *                     occurs
     */
    ...
    protected void writeProperties(NPOIFSFileSystem outFS,
                                   List<String> writtenEntries) throws IOException {
        ...
        if (dsi != null) {
            ...
            if (writtenEntries != null) {
                writtenEntries
                        .add(DocumentSummaryInformation.DEFAULT_STREAM_NAME);
            }
        }
    }
    ...
    protected void writePropertySet(String name, PropertySet set,
        ...
        try {
            ...
            logger.log(POILogger.INFO, "Wrote property set " + name
                    + " of size " + data.length);
        } catch (WritingNotSupportedException wnse) {
            logger.log(POILogger.ERROR,
                    "Couldn't write property set with name " + name
                            + " as not supported by HPSF yet");
        }
    }

    /**
     * Writes the document out to the specified output stream. The stream is not
