...
public abstract class PackagePart implements RelationshipSource,
     ...
     * @param partName    The part name, relative to the parent Package root.
     * @param contentType The content type.
     * @throws InvalidFormatException If the specified URI is not valid.
     */
     ...
     * @param pack        Parent package.
     * @param partName    The part name, relative to the parent Package root.
     * @param contentType The Multipurpose Internet Mail Extensions (MIME) content type
    ...
    public PackageRelationship addExternalRelationship(String target,
        ...
        if (target == null) {
            throw new IllegalArgumentException("target is null for type "
                    + relationshipType);
        }
     ...
     *
     * @param targetPartName   Name of the target part. This one must be relative to the
     *                         source root directory of the part.
     * @param targetMode       Mode [Internal|External].
     * @param relationshipType Type of relationship.
     * @return The newly created and added relationship
     ...
     * @param id               Relationship unique id.
     * @return The newly created and added relationship
     * @throws InvalidOperationException If a writing operation is done on a read only package or
     *                                   invalid nested relations are created.
     * @throws InvalidFormatException    If the URI point to a relationship part URI.
     * @throws IllegalArgumentException  if targetPartName, targetMode or relationshipType are passed
     *                                   as null
     * @see org.apache.poi.openxml4j.opc.RelationshipSource#addRelationship(org.apache.poi.openxml4j.opc.PackagePartName,
     * org.apache.poi.openxml4j.opc.TargetMode, java.lang.String,
     * java.lang.String)
     ...
     * Add a relationship to a part (except relationships part).
     *
     * @param targetURI        URI the target part. Must be relative to the source root
     ...
     * </p>
     *
     * @param targetURI        URI of the target part. Must be relative to the source root
     *                         directory of the part.
     * @param targetMode       Mode [Internal|External].
     * @param relationshipType Type of relationship.
     * @param id               Relationship unique id.
    ...
    public PackageRelationship addRelationship(URI targetURI,
        ...
        return _relationships.addRelationship(targetURI, targetMode,
                relationshipType, id);
    }
     ...
     *
     * @param id The ID identified the part to delete.
     ...
     * type.
     *
     * @param relationshipType Relationship type filter.
     * @return All relationships from this part that have the specified type.
     * @throws InvalidFormatException    If an error occurs while parsing the part.
     * @throws InvalidOperationException If the package is open in write only mode.
     ...
     *
     * @param filter Relationship type filter. If <i>null</i> then the filter is
     *               disabled and return all the relationships.
     * @return All relationships from this part that have the specified type.
     * @throws InvalidFormatException    Throws if an error occurs during parsing the relationships
     *                                   part.
     * @throws InvalidOperationException Throws if the package is open en write only mode.
    ...
    public PackagePart getRelatedPart(PackageRelationship rel)
        ...
        if (!isRelationshipExists(rel)) {
            throw new IllegalArgumentException("Relationship " + rel
                    + " doesn't start with this part " + _partName);
        }
        ...
        if (target.getFragment() != null) {
            ...
            try {
                ...
                throw new InvalidFormatException("Invalid target URI: "
                        + target);
            }
        }
        ...
        if (part == null) {
            throw new IllegalArgumentException(
                    "No part found for relationship " + rel);
        }
        return part;
    }
     ...
     * Zip package, it'll be transform intot a <i>MemoryPackagePart</i> in order
     * to write inside (the standard Java API doesn't allow to write in the
     * file)
     *
     * @see com.shvet.poi.openxml4j.opc.internal.MemoryPackagePart
     */
     ...
     *
     * @throws InvalidOperationException If this part is a relationship part.
     */
     ...
     *
     * @throws InvalidFormatException Throws if
     */
     ...
     * @throws InvalidFormatException    Throws if the content type is not valid.
     * @throws InvalidOperationException Throws if you try to change the content type whereas this
     ...
     *
     * @throws IOException Throws if an IO Exception occur in the implementation
     *                     method.
     */
     ...
     * stream.
     *
     * @param zos Output stream to save this part.
     * @throws OpenXML4JException If any exception occur.
     */
     ...
     *
     * @param ios The input stream of the content to load.
     * @return <b>true</b> if the content has been successfully loaded, else
     * <b>false</b>.
     * @throws InvalidFormatException Throws if the content format is invalid.
     */
