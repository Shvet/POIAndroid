...
package com.shvet.poi.openxml4j.opc.internal.unmarshallers;

import com.shvet.poi.openxml4j.exceptions.InvalidFormatException;
import com.shvet.poi.openxml4j.opc.PackageNamespaces;
import com.shvet.poi.openxml4j.opc.PackagePart;
import com.shvet.poi.openxml4j.opc.PackageProperties;
import com.shvet.poi.openxml4j.opc.ZipPackage;
...
import com.shvet.poi.openxml4j.opc.internal.ZipHelper;
import com.shvet.poi.util.DocumentHelper;
...
public final class PackagePropertiesUnmarshaller implements PartUnmarshaller {
    ...
    public PackagePart unmarshall(UnmarshallContext context, InputStream in)
        ...
        PackagePropertiesPart coreProps = new PackagePropertiesPart(
                context.getPackage(), context.getPartName());
    ...
    private String readElement(Document xmlDoc, String localName,
                               String namespaceURI) {
    ...
    public void checkElementForOPCCompliance(Element el)
        ...
        for (int i = 0; i < namedNodeCount; i++) {
            ...
            if (attr.getNamespaceURI().equals(
        ...
        if (el.getNamespaceURI().equals(PackageProperties.NAMESPACE_DCTERMS)) {
            ...
            Attr typeAtt = el.getAttributeNodeNS(
            ...
            if (!typeAtt.getValue().equals(el.getPrefix() + ":W3CDTF"))
                throw new InvalidFormatException(
                        "The element '"
                                + elName
                                + "' must have the 'xsi:type' attribute with the value '"
                                + el.getPrefix() + ":W3CDTF', but had '"
                                + typeAtt.getValue() + "' !");
        }
