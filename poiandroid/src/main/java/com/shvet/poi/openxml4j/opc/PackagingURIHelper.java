...
public final class PackagingURIHelper {
    ...
    private final static POILogger _logger = POILogFactory
            .getLogger(PackagingURIHelper.class);
    private static final Pattern missingAuthPattern = Pattern
            .compile("\\w+://");
     ...
     *                     This flag violates [M1.4]: A part name shall start with a
     *                     forward slash ('/') character, but allows generating URIs
    ...
    public static URI relativizeURI(URI sourceURI, URI targetURI,
        ...
        // If we didn't have a good match or at least except a first empty
        // element
        ...
        if (segmentsTheSame == segmentsSource.length
            ...
            if (sourceURI.equals(targetURI)) {
                // if source and target are the same they should be resolved to
                // the last segment,
                // Example: if a slide references itself, e.g. the source URI is
                // "/ppt/slides/slide1.xml" and the targetURI is "slide1.xml"
                // then
                // this it should be relativized as "slide1.xml", i.e. the last
                // segment.
    ...
    public static String decodeURI(URI uri) {
        ...
        for (int i = 0; i < uriStr.length(); ++i) {
            ...
            if (c == '%') {
                ...
                char decodedChar = (char) Integer.parseInt(
                        uriStr.substring(i + 1, i + 3), 16);
     ...
     * <p>
     * 1. Percent-encode each open bracket ([) and close bracket (]).</li> 2.
     * Percent-encode each percent (%) character that is not followed by a
     * hexadecimal notation of an octet value.</li> 3. Un-percent-encode each
     * percent-encoded unreserved character. 4. Un-percent-encode each forward
     * slash (/) and back slash (\). 5. Convert all back slashes to forward
     * slashes. 6. If present in a segment containing non-dot (?.?) characters,
     * remove trailing dot (?.?) characters from each segment. 7. Replace each
     * occurrence of multiple consecutive forward slashes (/) with a single
     * forward slash. 8. If a single trailing forward slash (/) is present,
     * remove that trailing forward slash. 9. Remove complete segments that
     * consist of three or more dots. 10. Resolve the relative reference against
     * the base URI of the part holding the Unicode string, as it is defined in
     * ?5.2 of RFC 3986. The path component of the resulting absolute URI is the
     * part name.
     * </p>
     *
        ...
        if (value.length() > 0) {
            ...
            if (b.length() > 0) {
                value = value.substring(0, idx + 1)
                        + encode(b.reverse().toString());
            }
        }

        // MS Office can insert URIs with missing authority, e.g. "http://" or
        // "javascript://"
     ...
     * <p/>
     * Examples: 'Apache POI' --> 'Apache%20POI' 'Apache\u0410POI' -->
     * 'Apache%04%10POI'
     *
