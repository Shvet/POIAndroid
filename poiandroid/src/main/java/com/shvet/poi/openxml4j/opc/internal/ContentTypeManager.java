...
package com.shvet.poi.openxml4j.opc.internal;
...
public abstract class ContentTypeManager {
    ...
    public ContentTypeManager(InputStream in, OPCPackage pkg)
        ...
        if (in != null) {
            try {
                ...
                // here it is useful to add the cause to not loose the original
                // stack-trace
                ex.initCause(e);
    ...
    public void addContentType(PackagePartName partName, String contentType) {
        boolean defaultCTExists = this.defaultContentType
    ...
    private void parseContentTypesFile(InputStream in)
        ...
        try {
            ...
            NodeList defaultTypes = xmlContentTypetDoc.getDocumentElement()
            ...
            for (int i = 0; i < defaultTypeCount; i++) {
                ...
                String extension = element
                        .getAttribute(EXTENSION_ATTRIBUTE_NAME);
                String contentType = element
                        .getAttribute(CONTENT_TYPE_ATTRIBUTE_NAME);
                addDefaultContentType(extension, contentType);
            }

            // Overriden content types
            NodeList overrideTypes = xmlContentTypetDoc.getDocumentElement()
            ...
            for (int i = 0; i < overrideTypeCount; i++) {
                ...
                URI uri = new URI(
                        element.getAttribute(PART_NAME_ATTRIBUTE_NAME));
    ...
    public boolean save(OutputStream outStream) {
        ...
        // Building namespace
        Element typesElem = xmlOutDoc.createElementNS(TYPES_NAMESPACE_URI,
                TYPES_TAG_NAME);
    ...
    private void appendSpecificTypes(Element root,
        ...
        Element specificType = root.getOwnerDocument().createElementNS(
                TYPES_NAMESPACE_URI, OVERRIDE_TAG_NAME);
        specificType.setAttribute(PART_NAME_ATTRIBUTE_NAME, entry.getKey()
                .getName());
        specificType
                .setAttribute(CONTENT_TYPE_ATTRIBUTE_NAME, entry.getValue());
    ...
    private void appendDefaultType(Element root, Entry<String, String> entry) {
        Element defaultType = root.getOwnerDocument().createElementNS(
                TYPES_NAMESPACE_URI, DEFAULT_TAG_NAME);
