...
/**
 * This class holds all of the section formatting properties from Old (Word 6 /
 * Word 95) documents. Unlike with Word 97+, it all gets held in the same
 * stream. In common with the rest of the old support, it is read only
 */
@Internal
    ...
    public OldSectionTable(byte[] documentStream, int offset, int size) {
        ...
        for (int x = 0; x < length; x++) {
            ...
            if (fileOffset == 0xffffffff) {
                ...
                int sepxSize = LittleEndian
                        .getShort(documentStream, fileOffset);
                // Because we don't properly know about all the details of the
                // old
                // section properties, and we're trying to decode them as if
                // they
                // were the new ones, we sometimes "need" more data than we
                // have.
