 ...
 * Paragraph's or Character run's text. The ending offset is the next value in
 * the array. For example, if an fkp has X number of Paragraph's stored in it
 * then there are (x + 1) 4 byte ints in the beginning array. The number X is
 * determined by the last byte in a 512 byte fkp.
 * <p/>
...
public final class PAPFormattedDiskPage extends FormattedDiskPage {
     ...
     * that is necessary because there is no easy and efficient way to determine
     * the number PAPX's that will fit into one FKP. THe sequence is as follows:
     * <p/>
     * fill() toByteArray() getOverflow()
     *
