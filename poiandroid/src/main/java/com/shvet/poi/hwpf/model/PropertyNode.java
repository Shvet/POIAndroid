 ...
 * This only ever works in characters. For the few odd cases when the start and
 * end aren't in characters (eg PAPX and CHPX), use {@link BytePropertyNode}
...
public abstract class PropertyNode<T extends PropertyNode<T>> implements
     ...
     * @param buf     FIXME: Old documentation is:
    ...
    protected PropertyNode(int fcStart, int fcEnd, Object buf) {
        ...
        if (_cpStart < 0) {
            _logger.log(POILogger.WARN,
                    "A property claimed to start before zero, at " + _cpStart
