...
public final class CharacterSprmCompressor {
    ...
    public static byte[] compressCharacterProperty(CharacterProperties newCHP,
        ...
        if (newCHP.isFSpec() != oldCHP.isFSpec()
                || newCHP.getFcPic() != oldCHP.getFcPic()) {
            size += SprmUtils.addSprm((short) 0x6a03, newCHP.getFcPic(), null,
                    sprmList);
        }
        if (newCHP.getIbstRMark() != oldCHP.getIbstRMark()) {
            size += SprmUtils.addSprm((short) 0x4804, newCHP.getIbstRMark(),
                    null, sprmList);
        }
        if (!newCHP.getDttmRMark().equals(oldCHP.getDttmRMark())) {
            ...
            size += SprmUtils.addSprm((short) 0x6805, LittleEndian.getInt(buf),
                    null, sprmList);
        }
        ...
        if (newCHP.getIcoHighlight() != oldCHP.getIcoHighlight()) {
            size += SprmUtils.addSprm((short) 0x2a0c, newCHP.getIcoHighlight(),
                    null, sprmList);
        }
        if (newCHP.getFcObj() != oldCHP.getFcObj()) {
            size += SprmUtils.addSprm((short) 0x680e, newCHP.getFcObj(), null,
                    sprmList);
        }
        if (newCHP.getIstd() != oldCHP.getIstd()) {
            size += SprmUtils.addSprm((short) 0x4a30, newCHP.getIstd(), null,
                    sprmList);
        }
        ...
        if (newCHP.getKul() != oldCHP.getKul()) {
            size += SprmUtils.addSprm((short) 0x2a3e, newCHP.getKul(), null,
                    sprmList);
        }
        if (newCHP.getDxaSpace() != oldCHP.getDxaSpace()) {
            size += SprmUtils.addSprm((short) 0x8840, newCHP.getDxaSpace(),
                    null, sprmList);
        }
        if (newCHP.getIco() != oldCHP.getIco()) {
            size += SprmUtils.addSprm((short) 0x2a42, newCHP.getIco(), null,
                    sprmList);
        }
        if (newCHP.getHps() != oldCHP.getHps()) {
            size += SprmUtils.addSprm((short) 0x4a43, newCHP.getHps(), null,
                    sprmList);
        }
        if (newCHP.getHpsPos() != oldCHP.getHpsPos()) {
            size += SprmUtils.addSprm((short) 0x4845, newCHP.getHpsPos(), null,
                    sprmList);
        }
        if (newCHP.getHpsKern() != oldCHP.getHpsKern()) {
            size += SprmUtils.addSprm((short) 0x484b, newCHP.getHpsKern(),
                    null, sprmList);
        }
        if (newCHP.getHresi().equals(oldCHP.getHresi())) {
            size += SprmUtils.addSprm((short) 0x484e, newCHP.getHresi()
                    .getValue(), null, sprmList);
        }
        if (newCHP.getFtcAscii() != oldCHP.getFtcAscii()) {
            size += SprmUtils.addSprm((short) 0x4a4f, newCHP.getFtcAscii(),
                    null, sprmList);
        }
        if (newCHP.getFtcFE() != oldCHP.getFtcFE()) {
            size += SprmUtils.addSprm((short) 0x4a50, newCHP.getFtcFE(), null,
                    sprmList);
        }
        if (newCHP.getFtcOther() != oldCHP.getFtcOther()) {
            size += SprmUtils.addSprm((short) 0x4a51, newCHP.getFtcOther(),
                    null, sprmList);
        }
        ...
        if (newCHP.getSfxtText() != oldCHP.getSfxtText()) {
            size += SprmUtils.addSprm((short) 0x2859, newCHP.getSfxtText(),
                    null, sprmList);
        }
