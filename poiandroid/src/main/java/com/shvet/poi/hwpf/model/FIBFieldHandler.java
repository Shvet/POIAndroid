...
public final class FIBFieldHandler {
    ...
    private static POILogger log = POILogFactory
            .getLogger(FIBFieldHandler.class);
    private Map<Integer, UnhandledDataStructure> _unknownMap = new HashMap<Integer, UnhandledDataStructure>();
    private int[] _fields;

    FIBFieldHandler(byte[] mainStream, int offset, int cbRgFcLcb,
                    byte[] tableStream, HashSet<Integer> offsetList, boolean areKnown) {
                    ...
                    if (dsOffset + dsSize > tableStream.length) {
                        if (log.check(POILogger.WARN)) {
                            log.log(POILogger.WARN,
                                    "Unhandled data structure points to outside the buffer. "
                                            + "offset = " + dsOffset
                                            + ", length = " + dsSize
                                            + ", buffer length = "
                                            + tableStream.length);
    ...
    void writeTo(byte[] mainStream, int offset, HWPFOutputStream tableStream)
        ...
        for (int x = 0; x < _fields.length / 2; x++) {
            ...
            if (ds != null) {
                ...
                LittleEndian
                        .putInt(mainStream, offset, tableStream.getOffset());
