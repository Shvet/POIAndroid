...
import com.shvet.poi.hwpf.sprm.CharacterSprmUncompressor;
...
public final class StyleSheet implements HDFType {
     ...
     *
     * @param tableStream A byte array containing a document's raw stylesheet info.
     *                    Found by using FileInformationBlock.getFcStshf() and
    ...
    public void writeTo(HWPFOutputStream out) throws IOException {
        ...
        for (int x = 0; x < _styleDescriptions.length; x++) {
            if (_styleDescriptions[x] != null) {
                ...
                LittleEndian.putShort(sizeHolder, 0,
                        (short) ((std.length) + (std.length % 2)));
     ...
     * CharacterProperties object is placed in the StyleDescription at istd
     * after its been created. Not every StyleDescription will contain a chpx.
