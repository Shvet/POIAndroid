...
import com.shvet.poi.hwpf.sprm.SprmOperation;
 ...
 * <p/>
 * Make sure you call getStart() / getEnd() when you want characters (normal
 * use), but getStartByte() / getEndByte() when you're reading in / writing out!
 *
 * @author Ryan Ackley
 */
@Internal
...
public final class PAPX extends BytePropertyNode<PAPX> {
    ...
    public PAPX(int fcStart, int fcEnd, CharIndexTranslator translator,
                byte[] papx, ParagraphHeight phe, byte[] dataStream) {
    ...
    @Deprecated
    public PAPX(int fcStart, int fcEnd, CharIndexTranslator translator,
                SprmBuffer buf, byte[] dataStream) {
    ...
    private SprmBuffer findHuge(SprmBuffer buf, byte[] datastream) {
        ...
        if (grpprl.length == 8 && datastream != null) // then check for
        // sprmPHugePapx
        {
            ...
            if ((sprm.getOperation() == 0x45 || sprm.getOperation() == 0x46)
                ...
                if (hugeGrpprlOffset + 1 < datastream.length) {
                    int grpprlSize = LittleEndian.getShort(datastream,
                            hugeGrpprlOffset);
                    if (hugeGrpprlOffset + grpprlSize < datastream.length) {
                        ...
                        System.arraycopy(datastream, hugeGrpprlOffset + 2,
                                hugeGrpprl, 2, grpprlSize);
    ...
    public ParagraphProperties getParagraphProperties(StyleSheet ss) {
        ...
        ParagraphProperties props = ParagraphSprmUncompressor.uncompressPAP(
                baseStyle, getGrpprl(), 2);
        return props;
    }
