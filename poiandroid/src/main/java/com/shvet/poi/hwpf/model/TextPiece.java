...
/**
 * Lightweight representation of a text piece. Works in the character domain,
 * not the byte domain, so you need to have turned byte references into
 * character references before getting here.
 *
 * @author Ryan Ackley
 */
@Internal
public class TextPiece extends PropertyNode<TextPiece> {
    ...
    public TextPiece(int start, int end, byte[] text, PieceDescriptor pd) {
        ...
        if (end - start != textLength) {
            throw new IllegalStateException("Told we're for characters "
                    + start + " -> " + end + ", but actually covers "
                    + textLength + " characters!");
        }
        if (end < start) {
            throw new IllegalStateException(
                    "Told we're of negative size! start=" + start + " end="
                            + end);
        }
    }
    ...
    private static StringBuilder buildInitSB(byte[] text, PieceDescriptor pd) {
        String str = new String(text,
                Charset.forName(pd.isUnicode() ? "UTF-16LE" : "Cp1252"));
    ...
    public boolean equals(Object o) {
        ...
        return (limitsAreEqual(o) && tp._usesUnicode == this._usesUnicode
                && tp._buf.toString().equals(this._buf.toString()) && tp._pd
                .equals(this._pd));
    }
