...
public abstract class LSTFAbstractType {
    ...
    public String toString() {
        ...
        builder.append(" (").append(Arrays.toString(getRgistdPara()))
                .append(" )\n");
        builder.append("    .flags                = ");
        builder.append(" (").append(getFlags()).append(" )\n");
        builder.append("         .fSimpleList              = ")
                .append(isFSimpleList()).append('\n');
        builder.append("         .unused1                  = ")
                .append(isUnused1()).append('\n');
        builder.append("         .fAutoNum                 = ")
                .append(isFAutoNum()).append('\n');
        builder.append("         .unused2                  = ")
                .append(isUnused2()).append('\n');
        builder.append("         .fHybrid                  = ")
                .append(isFHybrid()).append('\n');
    ...
    /**
     * A signed integer that specifies the list identifier. This MUST be unique
     * for each LSTF. This value MUST not be 0xFFFFFFFF.
     */
    @Internal
    public int getLsid() {
        return field_1_lsid;
    ...
    /**
     * A Tplc that specifies a unique identifier for this LSTF that MAY be used
     * for user interface purposes. If fHybrid is nonzero, this MUST be ignored.
     */
    @Internal
    public int getTplc() {
        return field_2_tplc;
    ...
    /**
     * An array of nine 16-bit signed integers. Each element of rgistdPara
     * specifies the ISTD of the style that is linked to the corresponding level
     * in the list. If no style is linked to a given level, the value of the
     * corresponding element of rgistdPara MUST be 0x0FFF.
     */
    @Internal
    ...
    /**
     * A bit that, when set to 0x1, specifies that this LSTF represents a simple
     * (one-level) list that has one corresponding LVL (see the fcPlfLst field
     * of FibRgFcLcb97). Otherwise, this LSTF represents a multi-level list that
     * has nine corresponding LVLs
     *
    ...
    /**
     * Sets the fSimpleList field value. A bit that, when set to 0x1, specifies
     * that this LSTF represents a simple (one-level) list that has one
     * corresponding LVL (see the fcPlfLst field of FibRgFcLcb97). Otherwise,
     * this LSTF represents a multi-level list that has nine corresponding LVLs
     */
    @Internal
    ...
    /**
     * A bit that specifies whether the list that this LSTF represents is used
     * for the AUTONUMOUT, AUTONUMLGL, and AUTONUM fields (see AUTONUMOUT,
     * AUTONUMLGL, and AUTONUM in flt)
     *
    ...
    /**
     * Sets the fAutoNum field value. A bit that specifies whether the list that
     * this LSTF represents is used for the AUTONUMOUT, AUTONUMLGL, and AUTONUM
     * fields (see AUTONUMOUT, AUTONUMLGL, and AUTONUM in flt)
     */
    @Internal
    ...
    /**
     * Sets the fHybrid field value. A bit that specifies whether the list this
     * LSTF defines is a hybrid list
     */
    @Internal
