...
import com.shvet.poi.hwpf.HWPFDocument;
import com.shvet.poi.hwpf.usermodel.CharacterRun;
import com.shvet.poi.hwpf.usermodel.Picture;
 ...
 * objects) provided by HWPFDocument.
 * <p/>
 ...
 * stream. For example, binary data describing in-line pictures and/or
 * formfields an also embedded objects-native data. Word picture structures are
 * concatenated one after the other in the data stream if the document contains
 * pictures. Data stream is easily reachable via HWPFDocument._dataStream
 * property. A picture is represented in the document text stream as a special
 * character, an Unicode \u0001 whose CharacterRun.isSpecial() returns true. The
 * file location of the picture in the Word binary file is accessed via
 * CharacterRun.getPicOffset(). The CharacterRun.getPicOffset() is a byte offset
 * into the data stream. Beginning at the position recorded in picOffset, a
 * header data structure, will be stored.
 *
...
public final class PicturesTable {
    ...
    public PicturesTable(HWPFDocument _document, byte[] _dataStream,
                         byte[] _mainStream, FSPATable fspa, EscherRecordHolder dgg) {
    ...
    private static short getBlockType(byte[] dataStream, int pictOffset) {
        return LittleEndian
                .getShort(dataStream, pictOffset + BLOCK_TYPE_OFFSET);
    }

    private static short getMmMode(byte[] dataStream, int pictOffset) {
        return LittleEndian.getShort(dataStream, pictOffset
                + MM_MODE_TYPE_OFFSET);
    }
    ...
    public boolean hasPicture(CharacterRun run) {
        ...
        if (run.isSpecialCharacter() && !run.isObj() && !run.isOle2()
                && !run.isData()) {
            // Image should be in it's own run, or in a run with the
            // end-of-special marker
            if ("\u0001".equals(run.text())
                    || "\u0001\u0015".equals(run.text())) {
    ...
    public boolean hasEscherPicture(CharacterRun run) {
        return run.isSpecialCharacter() && !run.isObj() && !run.isOle2()
                && !run.isData() && run.text().startsWith("\u0008");
    }
    ...
    private boolean isPictureRecognized(short blockType,
                                        short mappingModeOfMETAFILEPICT) {
        return (blockType == TYPE_IMAGE
                || blockType == TYPE_IMAGE_PASTED_FROM_CLIPBOARD
                || (blockType == TYPE_IMAGE_WORD2000 && mappingModeOfMETAFILEPICT == 0x64) || (blockType == TYPE_IMAGE_PASTED_FROM_CLIPBOARD_WORD2000 && mappingModeOfMETAFILEPICT == 0x64));
    }
     ...
     *                  represent picture's contents. If you don't want to have that
     *                  byte array in memory but only write picture's contents to
     ...
     * null otherwise. PicturesTable.hasPicture is used to determine
     * this.
    ...
    private boolean isBlockContainsImage(int i) {
        return isPictureRecognized(getBlockType(_dataStream, i),
                getMmMode(_dataStream, i));
    }
