...
public final class TableSprmCompressor {
    ...
    public static byte[] compressTableProperty(TableProperties newTAP) {
        ...
        if (newTAP.getJc() != 0) {
            size += SprmUtils.addSprm((short) 0x5400, newTAP.getJc(), null,
                    sprmList);
        }
        ...
        if (newTAP.getDyaRowHeight() != 0) {
            size += SprmUtils.addSprm((short) 0x9407, newTAP.getDyaRowHeight(),
                    null, sprmList);
        }
        if (newTAP.getItcMac() > 0) {
            ...
            for (int x = 0; x < cellDescriptors.length; x++) {
                cellDescriptors[x].serialize(buf, 1
                        + ((itcMac + 1) * LittleEndian.SHORT_SIZE)
