...
public final class CHPX extends BytePropertyNode<CHPX> {
    ...
    public CHPX(int fcStart, int fcEnd, CharIndexTranslator translator,
                byte[] grpprl) {
        super(fcStart, translator.lookIndexBackward(fcEnd), translator,
                new SprmBuffer(grpprl, 0));
    }
    ...
    public CharacterProperties getCharacterProperties(StyleSheet ss, short istd) {
        ...
        CharacterProperties props = CharacterSprmUncompressor.uncompressCHP(ss,
                baseStyle, getGrpprl(), 0);
        return props;
    }

    public String toString() {
        return "CHPX from " + getStart() + " to " + getEnd() + " (in bytes "
