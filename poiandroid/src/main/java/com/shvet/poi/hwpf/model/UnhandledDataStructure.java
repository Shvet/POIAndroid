...
/**
 * A data structure used to hold some data we don't understand / can't handle,
 * so we have it available for when we come to write back out again
 */
@Internal
public final class UnhandledDataStructure {
    ...
    public UnhandledDataStructure(byte[] buf, int offset, int length) {
        ...
        if (offsetEnd > buf.length || offsetEnd < 0) {
            throw new IndexOutOfBoundsException("Buffer Length is "
                    + buf.length + " " + "but code is tried to read " + length
                    + " " + "from offset " + offset + " to " + offsetEnd);
        }
        if (offset < 0 || length < 0) {
            throw new IndexOutOfBoundsException(
                    "Offset and Length must both be >= 0, negative "
