...
import com.shvet.poi.hwpf.model.LFO;
import com.shvet.poi.hwpf.model.ListLevel;
import com.shvet.poi.hwpf.model.ListTables;
import com.shvet.poi.hwpf.model.PAPX;
import com.shvet.poi.hwpf.model.StyleSheet;
import com.shvet.poi.hwpf.sprm.ParagraphSprmUncompressor;
import com.shvet.poi.hwpf.sprm.SprmBuffer;
import com.shvet.poi.hwpf.sprm.TableSprmCompressor;
...
public class Paragraph extends Range implements Cloneable {
    ...
    protected Paragraph(PAPX papx, Range parent) {
        super(Math.max(parent._start, papx.getStart()), Math.min(parent._end,
                papx.getEnd()), parent);
        _props = papx.getParagraphProperties(_doc.getStyleSheet());
        _papx = papx.getSprmBuf();
        _istd = papx.getIstd();
    ...
    /**
     * Returns the index of the style which applies to this Paragraph. Details
     * of the style can be looked up from the {@link StyleSheet}, via
    ...
    /**
     * Returns the ilfo, an index to the document's hpllfo, which describes the
     * automatic number formatting of the paragraph. A value of zero means it
     * isn't numbered.
     */
    public int getIlfo() {
    ...
    /**
     * Returns the multi-level indent for the paragraph. Will be zero for
     * non-list paragraphs, and the first level of any list. Subsequent levels
     * in hold values 1-8.
     */
    public int getIlvl() {
    ...
    /**
     * Returns the heading level (1-8), or 9 if the paragraph isn't in a heading
     * style.
     */
    public int getLvl() {
