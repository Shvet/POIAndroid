...
import com.shvet.poi.hwpf.usermodel.TableProperties;
...
public final class TableSprmUncompressor extends SprmUncompressor {
    private static final POILogger logger = POILogFactory
            .getLogger(TableSprmUncompressor.class);
    ...
    /**
     * Used to uncompress a table property. Performs an operation defined by a
     * sprm stored in a tapx.
     *
     ...
     */
    static void unCompressTAPOperation(TableProperties newTAP,
                                       SprmOperation sprm) {
        switch (sprm.getOperation()) {
            ...
            case 0x01: {
                ...
                int adjust = sprm.getOperand()
                        - (rgdxaCenter[0] + newTAP.getDxaGapHalf());
            ...
            case 0x08: {
                ...
                for (int x = 0; x < itcMac; x++) {
                    rgdxaCenter[x] = LittleEndian.getShort(grpprl, offset
                            + (1 + (x * 2)));
                }

                // only try to get the TC entries if they exist...
                int endOfSprm = offset + sprm.size() - 6; // -2 bytes for sprm - 2
                // for size short - 2 to
                // correct offsets being
                // 0 based
                ...
                for (int x = 0; x < itcMac; x++) {
                    // Sometimes, the grpprl does not contain data at every offset.
                    // I have no idea why this happens.
                    if (hasTCs
                            && offset + (1 + ((itcMac + 1) * 2) + (x * 20)) < grpprl.length)
                ...
                rgdxaCenter[itcMac] = LittleEndian.getShort(grpprl, offset
                        + (1 + (itcMac * 2)));
                break;
            }
            case 0x09:
            ...
            case 0x21: {
                ...
                if (index >= itcMac) {
                    ...
                    System.arraycopy(newTAP.getRgdxaCenter(), 0, rgdxaCenter, 0,
                            index + 1);
                    System.arraycopy(newTAP.getRgdxaCenter(), index + 1,
                            rgdxaCenter, index + count, itcMac - (index));
                    // copy rgtc
