...
import com.shvet.poi.hwpf.model.PICF;
import com.shvet.poi.hwpf.model.PICFAndOfficeArtData;
import com.shvet.poi.util.POILogFactory;
import com.shvet.poi.util.POILogger;
import com.shvet.poi.util.PngUtils;
...
public final class Picture {
    ...
    /**
     * Builds a Picture object for a Picture stored as Escher. TODO We need to
     * pass in the PICF data too somehow!
     */
    public Picture(EscherBlipRecord blipRecord) {
        this._blipRecords = Arrays
                .asList(blipRecord);
    }
    ...
    private void fillImageContent() {
        ...
        if (matchSignature(rawContent, COMPRESSED1, 32)
            ...
            // PNG created on MAC may have a 16-byte prefix which prevents
            // successful reading.
            // Just cut it off!.
    ...
    public String getDescription() {
        for (EscherRecord escherRecord : _picfAndOfficeArtData.getShape()
            ...
            if (escherRecord instanceof EscherOptRecord) {
                ...
                for (EscherProperty property : escherOptRecord
                        .getEscherProperties()) {
                    if (EscherProperties.GROUPSHAPE__DESCRIPTION == property
                            .getPropertyNumber()) {
                        byte[] complexData = ((EscherComplexProperty) property)
                                .getComplexData();
                        return StringUtil.getFromUnicodeLE(complexData, 0,
