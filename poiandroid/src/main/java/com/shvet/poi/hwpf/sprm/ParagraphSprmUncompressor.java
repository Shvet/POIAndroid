...
public final class ParagraphSprmUncompressor extends SprmUncompressor {
    ...
    public static ParagraphProperties uncompressPAP(ParagraphProperties parent,
        ...
        try {
            ...
            throw new RuntimeException(
                    "There is no way this exception should happen!!");
        }
    ...
    static void unCompressPAPOperation(ParagraphProperties newPAP,
        ...
        switch (sprm.getOperation()) {
            ...
            case 0x11:
                ...
                break;
            case 0x12:
                newPAP.setLspd(new LineSpacingDescriptor(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x13:
            ...
            case 0x23:
                ...
                break;
            case 0x24:
                newPAP.setBrcTop(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x25:
                newPAP.setBrcLeft(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x26:
                newPAP.setBrcBottom(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x27:
                newPAP.setBrcRight(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x28:
                newPAP.setBrcBetween(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x29:
                newPAP.setBrcBar(new BorderCode(sprm.getGrpprl(), sprm
                        .getGrpprlOffset()));
                break;
            case 0x2a:
    ...
    private static void handleTabs(ParagraphProperties pap, SprmOperation sprm) {
        ...
        for (int x = 0; x < addSize; x++) {
            Integer key = Integer
                    .valueOf(LittleEndian.getShort(grpprl, offset));
    ...
    // private static void handleTabsAgain(ParagraphProperties pap,
    // SprmOperation sprm)
    // {
    // byte[] grpprl = sprm.getGrpprl();
    // int offset = sprm.getGrpprlOffset();
    ...
    // for (int x = 0; x < tabPositions.length; x++)
    // {
    // tabMap.put(Integer.valueOf(tabPositions[x]),
    // Byte.valueOf(tabDescriptors[x]));
    // }
    //
    // for (int x = 0; x < delSize; x++)
    // {
    ...
    // Byte val = Byte.valueOf(grpprl[(LittleEndian.INT_SIZE * (addSize - x)) +
    // x]);
    // tabMap.put(key, val);
    // offset += LittleEndian.INT_SIZE;
    // }
