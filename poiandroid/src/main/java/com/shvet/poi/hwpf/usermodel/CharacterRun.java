...
import com.shvet.poi.hwpf.model.FFData;
import com.shvet.poi.hwpf.model.Ffn;
import com.shvet.poi.hwpf.model.NilPICFAndBinData;
...
public final class CharacterRun extends Range implements Cloneable,
        com.shvet.poi.wp.usermodel.CharacterRun {
    ...
    CharacterRun(CHPX chpx, StyleSheet ss, short istd, Range parent) {
        super(Math.max(parent._start, chpx.getStart()), Math.min(parent._end,
                chpx.getEnd()), parent);
    ...
    /**
     * Does the picture offset represent picture or binary data? If it's set,
     * then the picture offset refers to a NilPICFAndBinData structure,
     * otherwise to a PICFAndOfficeArtData
     */
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        cp._props.setDttmRMarkDel((DateAndTime) _props.getDttmRMarkDel()
                .clone());
        cp._props.setDttmPropRMark((DateAndTime) _props.getDttmPropRMark()
                .clone());
        cp._props.setDttmDispFldRMark((DateAndTime) _props
                .getDttmDispFldRMark().clone());
    ...
    /**
     * Returns true, if the CharacterRun is a special character run containing a
     * symbol, otherwise false.
     * <p/>
     * <p/>
     * In case of a symbol, the {@link #text()} method always returns a single
     * character 0x0028, but word actually stores the character in a different
     * field. Use {@link #getSymbolCharacter()} to get that character and
     * {@link #getSymbolFont()} to determine its font.
     */
     ...
     *
     * @throws IllegalStateException If this is not a symbol character run: call
     *                               {@link #isSymbol()} first.
     * @see #isSymbol()
     */
    ...
    /**
     * Returns the symbol font, if this is a symbol character run. Might return
     * null, if the font index is not found in the font table.
     *
     * @throws IllegalStateException If this is not a symbol character run: call
     *                               {@link #isSymbol()} first.
     * @see #isSymbol()
    ...
    /**
     * <p>
     * Returns the index of the base style which applies to this Run. Details of
     * the style can be looked up from the {@link StyleSheet}, via
     * {@link StyleSheet#getStyleDescription(int)}.
     * </p>
     * <p>
     * Note that runs typically override some of the style properties from the
     * base, so normally style information should be fetched directly from the
     * {@link CharacterRun} itself.
     * </p>
     */
