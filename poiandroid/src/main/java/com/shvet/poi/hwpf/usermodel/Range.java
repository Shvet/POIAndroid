...
import com.shvet.poi.hwpf.HWPFDocumentCore;
import com.shvet.poi.hwpf.model.CHPX;
import com.shvet.poi.hwpf.model.FileInformationBlock;
import com.shvet.poi.hwpf.model.ListTables;
import com.shvet.poi.hwpf.model.PAPX;
import com.shvet.poi.hwpf.model.PropertyNode;
import com.shvet.poi.hwpf.model.SEPX;
...
import com.shvet.poi.hwpf.sprm.CharacterSprmCompressor;
import com.shvet.poi.hwpf.sprm.ParagraphSprmCompressor;
import com.shvet.poi.hwpf.sprm.SprmBuffer;
...
public class Range { // TODO -instantiable superclass
     ...
     * lists.
     *
     * @param startIdx The starting index in the list, inclusive
     * @param endIdx   The ending index in the list, exclusive
    ...
    private void sanityCheckStartEnd() {
        if (_start < 0) {
            throw new IllegalArgumentException(
                    "Range start must not be negative. Given " + _start);
        }
        if (_end < _start) {
    ...
    public CharacterRun insertBefore(String text) {
        ...
        if (_doc instanceof HWPFDocument) {
            ((BookmarksImpl) ((HWPFDocument) _doc).getBookmarks()).afterInsert(
                    _start, text.length());
        }
    ...
    public CharacterRun insertBefore(String text, CharacterProperties props) {
        ...
        byte[] grpprl = CharacterSprmCompressor.compressCharacterProperty(
                props, baseStyle);
     ...
     */
    @Deprecated
    protected Paragraph insertBefore(ParagraphProperties props, int styleIndex,
                                     String text) {
        initAll();
        StyleSheet ss = _doc.getStyleSheet();
        ...
        byte[] grpprl = ParagraphSprmCompressor.compressParagraphProperty(
                props, baseStyle);
        byte[] withIndex = new byte[grpprl.length + LittleEndian.SHORT_SIZE];
        LittleEndian.putShort(withIndex, (short) styleIndex);
        System.arraycopy(grpprl, 0, withIndex, LittleEndian.SHORT_SIZE,
                grpprl.length);
    ...
    public void delete() {
        ...
        if (_doc instanceof HWPFDocument) {
            ((BookmarksImpl) ((HWPFDocument) _doc).getBookmarks()).afterDelete(
                    _start, (_end - _start));
        }
     ...
     */
    @Deprecated
    public ListEntry insertBefore(ParagraphProperties props, int listID,
                                  int level, int styleIndex) {
        ListTables lt = _doc.getListTables();
        if (lt.getLevel(listID, level) == null) {
            throw new NoSuchElementException(
                    "The specified list and level do not exist");
        }
     ...
     */
    @Deprecated
    public ListEntry insertAfter(ParagraphProperties props, int listID,
                                 int level, int styleIndex) {
        ListTables lt = _doc.getListTables();
        if (lt.getLevel(listID, level) == null) {
            throw new NoSuchElementException(
     ...
     * @param addAfter if <tt>true</tt> the text will be added at the end of current
    ...
    public Table getTable(Paragraph paragraph) {
        if (!paragraph.isInTable()) {
            throw new IllegalArgumentException(
                    "This paragraph doesn't belong to a table");
        }
        ...
        return new Table(paragraph.getStartOffset(), endOffsetExclusive, this,
    ...
    private int[] findRange(List<? extends PropertyNode<?>> rpl, int start,
        ...
        if (startIndex < 0 || startIndex >= rpl.size() || startIndex > endIndex
                || endIndex < 0 || endIndex >= rpl.size())
     ...
     *
     * @param length the length to adjust for (expected to be a count of
