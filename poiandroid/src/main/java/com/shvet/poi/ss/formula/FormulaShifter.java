...
import com.shvet.poi.ss.formula.ptg.Area2DPtgBase;
...
import com.shvet.poi.ss.formula.ptg.AreaPtgBase;
import com.shvet.poi.ss.formula.ptg.Deleted3DPxg;
...
import com.shvet.poi.ss.formula.ptg.RefPtgBase;
...
public final class FormulaShifter {
    ...
    /**
     * Extern sheet index of sheet where moving is occurring, used for updating
     * HSSF style 3D references
     */
    private final int _externSheetIndex;
    /**
     * Sheet name of the sheet where moving is occurring, used for updating XSSF
     * style 3D references on row shifts.
     */
     ...
     * <p/>
     * For example, this will be called on
     * {@link org.apache.poi.hssf.usermodel.HSSFSheet#shiftRows(int, int, int)}
     */
    private FormulaShifter(int externSheetIndex, String sheetName,
                           int firstMovedIndex, int lastMovedIndex, int amountToMove) {
        if (amountToMove == 0) {
     ...
     * <p/>
     * For example, this will be called on
     * {@link org.apache.poi.hssf.usermodel.HSSFWorkbook#setSheetOrder(String, int)}
     */
    ...
    public static FormulaShifter createForRowShift(int externSheetIndex,
                                                   String sheetName, int firstMovedRowIndex, int lastMovedRowIndex,
                                                   int numberOfRowsToMove) {
        return new FormulaShifter(externSheetIndex, sheetName,
                firstMovedRowIndex, lastMovedRowIndex, numberOfRowsToMove);
    }
    ...
    private static Ptg createDeletedRef(Ptg ptg) {
        ...
        throw new IllegalArgumentException("Unexpected ref ptg class ("
     ...
     * @param currentExternSheetIx - the extern sheet index of the sheet that contains the
     *                             formula being adjusted
    ...
    private Ptg rowMoveRefPtg(RefPtgBase rptg) {
        ...
        // else rules for adjusting area may also depend on the destination of
        // the moved rows
        ...
        throw new IllegalStateException("Situation not covered: ("
                + _firstMovedIndex + ", " + _lastMovedIndex + ", "
                + _amountToMove + ", " + refRow + ", " + refRow + ")");
    }
    ...
    private Ptg rowMoveAreaPtg(AreaPtgBase aptg) {
        ...
        if (_firstMovedIndex <= aFirstRow && aFirstRow <= _lastMovedIndex) {
            // Rows moved include the first row of the area ref, but not the
            // last row
        ...
        if (_firstMovedIndex <= aLastRow && aLastRow <= _lastMovedIndex) {
            ...
            // else - top area row has been replaced - both area top and bottom
            // may move now
            int areaRemainingBottomRowIx = _firstMovedIndex - 1;
            if (destLastRowIndex < areaRemainingBottomRowIx) {
                // old bottom row of area has moved up deep within the area, and
                // exposed a new bottom row
        ...
        if (aFirstRow <= destFirstRowIndex && destLastRowIndex <= aLastRow) {
            // destination rows are within area ref (possibly exact on top or
            // bottom, but not both)
        ...
        throw new IllegalStateException("Situation not covered: ("
                + _firstMovedIndex + ", " + _lastMovedIndex + ", "
                + _amountToMove + ", " + aFirstRow + ", " + aLastRow + ")");
    }
