...
import com.shvet.poi.ss.formula.eval.AreaEvalBase;
import com.shvet.poi.ss.formula.eval.ValueEval;
import com.shvet.poi.ss.formula.ptg.AreaI;
import com.shvet.poi.ss.formula.ptg.AreaI.OffsetArea;
...
final class LazyAreaEval extends AreaEvalBase {
    ...
    public LazyAreaEval(int firstRowIndex, int firstColumnIndex,
                        int lastRowIndex, int lastColumnIndex, SheetRangeEvaluator evaluator) {
    ...
    public LazyAreaEval getColumn(int columnIndex) {
        if (columnIndex >= getWidth()) {
            throw new IllegalArgumentException("Invalid columnIndex "
                    + columnIndex + ".  Allowable range is (0.." + getWidth()
                    + ").");
        }
    ...
    public boolean isSubTotal(int rowIndex, int columnIndex) {
        ...
        // internal ptgs
        SheetRefEvaluator _sre = _evaluator.getSheetEvaluator(_evaluator
                .getFirstSheetIndex());
