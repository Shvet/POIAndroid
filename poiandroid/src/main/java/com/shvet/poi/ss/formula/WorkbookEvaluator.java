...
import com.shvet.poi.ss.formula.CollaboratingWorkbooksEnvironment.WorkbookNotFoundException;
import com.shvet.poi.ss.formula.atp.AnalysisToolPak;
import com.shvet.poi.ss.formula.eval.BlankEval;
import com.shvet.poi.ss.formula.eval.BoolEval;
import com.shvet.poi.ss.formula.eval.ErrorEval;
import com.shvet.poi.ss.formula.eval.EvaluationException;
import com.shvet.poi.ss.formula.eval.ExternalNameEval;
import com.shvet.poi.ss.formula.eval.FunctionEval;
import com.shvet.poi.ss.formula.eval.FunctionNameEval;
import com.shvet.poi.ss.formula.eval.MissingArgEval;
import com.shvet.poi.ss.formula.eval.NotImplementedException;
import com.shvet.poi.ss.formula.eval.NumberEval;
import com.shvet.poi.ss.formula.eval.OperandResolver;
import com.shvet.poi.ss.formula.eval.StringEval;
import com.shvet.poi.ss.formula.eval.ValueEval;
import com.shvet.poi.ss.formula.function.FunctionMetadataRegistry;
import com.shvet.poi.ss.formula.functions.Choose;
import com.shvet.poi.ss.formula.functions.FreeRefFunction;
import com.shvet.poi.ss.formula.functions.Function;
import com.shvet.poi.ss.formula.functions.IfFunc;
import com.shvet.poi.ss.formula.ptg.Area3DPtg;
import com.shvet.poi.ss.formula.ptg.Area3DPxg;
import com.shvet.poi.ss.formula.ptg.AreaErrPtg;
...
import com.shvet.poi.ss.formula.ptg.ControlPtg;
import com.shvet.poi.ss.formula.ptg.DeletedArea3DPtg;
import com.shvet.poi.ss.formula.ptg.DeletedRef3DPtg;
...
import com.shvet.poi.ss.formula.ptg.MemErrPtg;
...
import com.shvet.poi.ss.formula.ptg.NameXPxg;
...
import com.shvet.poi.ss.formula.ptg.Ref3DPxg;
import com.shvet.poi.ss.formula.ptg.RefErrorPtg;
...
import com.shvet.poi.ss.formula.ptg.UnknownPtg;
 ...
 * calculated intermediate cell values. Be sure to call
 * {@link #clearAllCachedResultValues()} if any workbook cells are changed
 * between calls to evaluate~ methods on this class.<br/>
 * <p/>
...
public final class WorkbookEvaluator {
    ...
    private static final POILogger LOG = POILogFactory
            .getLogger(WorkbookEvaluator.class);
    ...
    // large output)
    private final POILogger EVAL_LOG = POILogFactory
            .getLogger("POI.FormulaEval");
    ...
    /* package */WorkbookEvaluator(EvaluationWorkbook workbook,
        ...
        AggregatingUDFFinder defaultToolkit = // workbook can be null in unit
                // tests
                workbook == null ? null : (AggregatingUDFFinder) workbook
                        .getUDFFinder();
     ...
     * @return {@link BlankEval} if cell is <code>null</code> or blank, never
     * <code>null</code>
     */
    /* package */
    ...
    /**
     * Calculates the number of tokens that the evaluator should skip upon
     * reaching a tAttrSkip.
     *
     * @return the number of tokens (starting from <tt>startIndex+1</tt>) that
     * need to be skipped to achieve the specified <tt>distInBytes</tt>
     * skip distance.
     */
    private static int countTokensToBeSkipped(Ptg[] ptgs, int startIndex,
                                              int distInBytes) {
        ...
        while (remBytes != 0) {
            ...
            if (remBytes < 0) {
                throw new RuntimeException(
                        "Bad skip distance (wrong token size calculation).");
            }
            if (index >= ptgs.length) {
                throw new RuntimeException(
                        "Skip distance too far (ran out of formula tokens).");
            }
        }
    ...
    public static ValueEval dereferenceResult(ValueEval evaluationResult,
                                              int srcRowNum, int srcColNum) {
        ValueEval value;
        try {
            value = OperandResolver.getSingleValue(evaluationResult, srcRowNum,
                    srcColNum);
        } catch (EvaluationException e) {
            return e.getErrorEval();
        }
        if (value == BlankEval.instance) {
            // Note Excel behaviour here. A blank final final value is converted
            // to zero.
            return NumberEval.ZERO;
            // Formulas _never_ evaluate to blank. If a formula appears to have
            // evaluated to
    ...
    /* package */void attachToEnvironment(
            CollaboratingWorkbooksEnvironment collaboratingWorkbooksEnvironment,
            EvaluationCache cache, int workbookIx) {
    ...
    /**
     * Discards the current workbook environment and attaches to the default
     * 'empty' environment. Also resets evaluation cache.
     */
    ...
    /**
     * @return the evaluator for another workbook which is part of the same
     * {@link CollaboratingWorkbooksEnvironment}
     */
	/* package */WorkbookEvaluator getOtherWorkbookEvaluator(String workbookName)
            throws WorkbookNotFoundException {
        return _collaboratingWorkbookEnvironment
                .getWorkbookEvaluator(workbookName);
    }
    ...
    /**
     * Should be called whenever there are changes to input cells in the
     * evaluated workbook. Failure to call this method after changing cell
     * values will cause incorrect behaviour of the evaluate~ methods of this
     * class
     */
    ...
    /**
     * Should be called to tell the cell value cache that the specified (value
     * or formula) cell has changed.
     */
    ...
    /**
     * Should be called to tell the cell value cache that the specified cell has
     * just been deleted.
     */
    ...
    private int getSheetIndex(EvaluationSheet sheet) {
        ...
        if (result == null) {
            ...
            if (sheetIndex < 0) {
                throw new RuntimeException(
                        "Specified sheet from a different book");
            }
    ...
    public ValueEval evaluate(EvaluationCell srcCell) {
        ...
        return evaluateAny(srcCell, sheetIndex, srcCell.getRowIndex(),
                srcCell.getColumnIndex(), new EvaluationTracker(_cache));
    }
    ...
    private ValueEval evaluateAny(EvaluationCell srcCell, int sheetIndex,
        ...
        boolean shouldCellDependencyBeRecorded = _stabilityClassifier == null ? true
                : !_stabilityClassifier.isCellFinal(sheetIndex, rowIndex,
                columnIndex);
        ...
        if (cce.getValue() == null) {
            ...
            try {
                ...
                if (re.getCause() instanceof WorkbookNotFoundException
                    ...
                    logInfo(re.getCause().getMessage()
                            + " - Continuing with cached value!");
                    switch (srcCell.getCachedFormulaResultType()) {
                        ...
                        case Cell.CELL_TYPE_BOOLEAN:
                            result = BoolEval
                                    .valueOf(srcCell.getBooleanCellValue());
                            break;
                        case Cell.CELL_TYPE_ERROR:
            ...
            if (evalListener != null) {
                evalListener.onCacheHit(sheetIndex, rowIndex, columnIndex,
                        cce.getValue());
            }
            return cce.getValue();
        }
        if (isDebugLogEnabled()) {
            ...
            logDebug("Evaluated " + sheetName + "!" + cr.formatAsString()
                    + " to " + result.toString());
        }
        // Usually (result === cce.getValue())
        // But sometimes: (result==ErrorEval.CIRCULAR_REF_ERROR,
        // cce.getValue()==null)
        // When circular references are detected, the cache entry is only
        // updated for
    ...
    private NotImplementedException addExceptionInfo(
            NotImplementedException inner, int sheetIndex, int rowIndex,
            int columnIndex) {

        try {
            String sheetName = _workbook.getSheetName(sheetIndex);
            CellReference cr = new CellReference(sheetName, rowIndex,
	...
	/* package */ValueEval evaluateFormula(OperationEvaluationContext ec,
        ...
        String dbgIndentStr = ""; // always init. to non-null just for defensive
        // avoiding NPE
        if (dbgEvaluationOutputForNextEval) {
            // first evaluation call when ouput is desired, so iit. this
            // evaluator instance
            dbgEvaluationOutputIndent = 1;
            dbgEvaluationOutputForNextEval = false;
        }
        if (dbgEvaluationOutputIndent > 0) {
            // init. indent string to needed spaces (create as substring vom
            // very long space-only string;
            // limit indendation for deep recursions)
            dbgIndentStr = "                                                                                                    ";
            dbgIndentStr = dbgIndentStr.substring(0, Math.min(
                    dbgIndentStr.length(), dbgEvaluationOutputIndent * 2));
            EVAL_LOG.log(
                    POILogger.WARN,
                    dbgIndentStr
                            + "- evaluateFormula('"
                            + ec.getRefEvaluatorForCurrentSheet()
                            .getSheetNameRange()
                            + "'/"
                            + new CellReference(ec.getRowIndex(), ec
                            .getColumnIndex()).formatAsString()
                            + "): "
                            + Arrays.toString(ptgs).replaceAll(
                            "\\Qorg.apache.poi.ss.formula.ptg.\\E", ""));
        ...
        for (int i = 0, iSize = ptgs.length; i < iSize; i++) {
            ...
            if (ptg instanceof AttrPtg) {
                ...
                if (attrPtg.isOptimizedChoose()) {
                    ...
                    try {
                        int switchIndex = Choose.evaluateFirstArg(arg0,
                                ec.getRowIndex(), ec.getColumnIndex());
                        if (switchIndex < 1 || switchIndex > nChoices) {
                            stack.push(ErrorEval.VALUE_INVALID);
                            dist = attrPtg.getChooseFuncOffset() + 4; // +4 for
                            // tFuncFar(CHOOSE)
                        } else {
                    ...
                    // Encoded dist for tAttrChoose includes size of jump table,
                    // but
                    // countTokensToBeSkipped() does not (it counts whole
                    // tokens).
                ...
                if (attrPtg.isOptimizedIf()) {
                    ...
                    if (evaluatedPredicate) {
                        ...
                        if (ptgs[i] instanceof AttrPtg
                                ...
                                // FuncVarPtg as third param, e.g.
                                // ROW()/COLUMN()!
                                ((FuncVarPtg) nextPtg).getFunctionIndex() == FunctionMetadataRegistry.FUNCTION_INDEX_IF) {
                            // this is an if statement without a false param (as
                            // opposed to MissingArgPtg as the false param)
                            i++;
            ...
            if (ptg instanceof MemFuncPtg || ptg instanceof MemAreaPtg) {
                // can ignore, rest of tokens for this expression are in OK RPN
                // order
                continue;
            }
     ...
     * StringPtg, BoolPtg <br/>
     * special Note: OperationPtg subtypes cannot be passed here!
     */
    private ValueEval getEvalForPtg(Ptg ptg, OperationEvaluationContext ec) {
        // consider converting all these (ptg instanceof XxxPtg) expressions to
        // (ptg.getClass() == XxxPtg.class)
        ...
        if (ptg instanceof UnknownPtg) {
            // POI uses UnknownPtg when the encoded Ptg array seems to be
            // corrupted.
            // This seems to occur in very rare cases (e.g. unused name formulas
            // in bug 44774, attachment 21790)
            // In any case, formulas are re-parsed before execution, so
            // UnknownPtg should not get here
            throw new RuntimeException("UnknownPtg not allowed");
        }
        if (ptg instanceof ExpPtg) {
            // ExpPtg is used for array formulas and shared formulas.
            // it is currently unsupported, and may not even get implemented
            // here
            throw new RuntimeException("ExpPtg currently not supported");
        }

        throw new RuntimeException("Unexpected ptg class ("
                + ptg.getClass().getName() + ")");
    }

    private ValueEval processNameEval(ValueEval eval,
    ...
    private ValueEval getEvalForNameRecord(EvaluationName nameRecord,
                                           OperationEvaluationContext ec) {
     ...
     */
	/* package */ValueEval evaluateNameFormula(Ptg[] ptgs,
                                               OperationEvaluationContext ec) {
        if (ptgs.length == 1) {
     ...
     * contained cell.
     */
	/* package */ValueEval evaluateReference(EvaluationSheet sheet,
    ...
    /**
     * Whether to ignore missing references to external workbooks and use cached
     * formula results in the main workbook instead.
     * <p/>
     * In some cases exetrnal workbooks referenced by formulas in the main
     * workbook are not avaiable. With this method you can control how POI
     * handles such missing references:
     * <ul>
     * <li>by default ignoreMissingWorkbooks=false and POI throws
     * {@link WorkbookNotFoundException} if an external reference cannot be
     * resolved</li>
     ...
     * @see <a
     * href="https://issues.apache.org/bugzilla/show_bug.cgi?id=52575">Bug
     * 52575 for details</a>
     */
