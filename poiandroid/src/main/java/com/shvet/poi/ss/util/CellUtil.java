 ...
 * various methods that deal with style's allow you to create your CellStyles as
 * you need them. When you apply a style change to a cell, the code will attempt
 * to see if a style already exists that meets your needs. If not, then it will
 * create a new style. This is to prevent creating too many styles. there is an
 * upper limit in Excel on the number of styles that can be supported.
 *
...
public final class CellUtil {
    ...
    static {
        unicodeMappings = new UnicodeMapping[]{um("alpha", "\u03B1"),
                um("beta", "\u03B2"), um("gamma", "\u03B3"),
                um("delta", "\u03B4"), um("epsilon", "\u03B5"),
                um("zeta", "\u03B6"), um("eta", "\u03B7"),
                um("theta", "\u03B8"), um("iota", "\u03B9"),
                um("kappa", "\u03BA"), um("lambda", "\u03BB"),
                um("mu", "\u03BC"), um("nu", "\u03BD"), um("xi", "\u03BE"),
                um("omicron", "\u03BF"),};
    }
    ...
    public static Cell createCell(Row row, int column, String value,
        ...
        cell.setCellValue(cell.getRow().getSheet().getWorkbook()
                .getCreationHelper().createRichTextString(value));
        if (style != null) {
    ...
    public static Cell translateUnicodeValues(Cell cell) {
        ...
        if (foundUnicode) {
            cell.setCellValue(cell.getRow().getSheet().getWorkbook()
                    .getCreationHelper().createRichTextString(s));
        }
        return cell;
    }
