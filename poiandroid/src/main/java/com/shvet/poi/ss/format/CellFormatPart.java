...
import static com.shvet.poi.ss.format.CellFormatter.logger;
import static com.shvet.poi.ss.format.CellFormatter.quote;
...
public class CellFormatPart {
        ...
        for (HSSFColor color : colors.values()) {
            ...
            if (name.equals(name.toUpperCase(Locale.ROOT))) {
                ...
                if (name.indexOf("_PERCENT") > 0)
                    NAMED_COLORS.put(
                            name.replace("_PERCENT", "%").replace('_', ' '), c);
            }
        }
    }
    ...
    private static int findGroup(Pattern pat, String str, String marker) {
        ...
        throw new IllegalArgumentException("\"" + marker + "\" not found in \""
                + pat.pattern() + "\"");
    }
