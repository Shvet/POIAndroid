...
import com.shvet.poi.ss.formula.eval.AreaEval;
...
public final class OperationEvaluationContext {
    ...
    private static NameType classifyCellReference(String str,
    ...
    SheetRangeEvaluator createExternSheetRefEvaluator(
    ...
    SheetRangeEvaluator createExternSheetRefEvaluator(String firstSheetName,
        ...
        ExternalSheet externalSheet = _workbook.getExternalSheet(
                firstSheetName, lastSheetName, externalWorkbookNumber);
        return createExternSheetRefEvaluator(externalSheet);
    }

    SheetRangeEvaluator createExternSheetRefEvaluator(int externSheetIndex) {
        ExternalSheet externalSheet = _workbook
                .getExternalSheet(externSheetIndex);
        return createExternSheetRefEvaluator(externalSheet);
    }
    ...
    SheetRangeEvaluator createExternSheetRefEvaluator(
        ...
        if (externalSheet == null || externalSheet.getWorkbookName() == null) {
            ...
            if (externalSheet == null) {
                ...
                otherFirstSheetIndex = _workbook.getSheetIndex(externalSheet
                        .getSheetName());
            }

            if (externalSheet instanceof ExternalSheetRange) {
                String lastSheetName = ((ExternalSheetRange) externalSheet)
                        .getLastSheetName();
            ...
            try {
                targetEvaluator = _bookEvaluator
                        .getOtherWorkbookEvaluator(workbookName);
            } catch (WorkbookNotFoundException e) {
                throw new RuntimeException(e.getMessage(), e);
            }

            otherFirstSheetIndex = targetEvaluator.getSheetIndex(externalSheet
                    .getSheetName());
            if (externalSheet instanceof ExternalSheetRange) {
                String lastSheetName = ((ExternalSheetRange) externalSheet)
                        .getLastSheetName();
                otherLastSheetIndex = targetEvaluator
                        .getSheetIndex(lastSheetName);
            }

            if (otherFirstSheetIndex < 0) {
                throw new RuntimeException("Invalid sheet name '"
                        + externalSheet.getSheetName() + "' in bool '"
                        + workbookName + "'.");
            }
        }
     ...
     */
    private SheetRefEvaluator createExternSheetRefEvaluator(
        ...
        if (workbookName == null) {
            ...
            if (sheetName == null) {
                throw new IllegalArgumentException(
                        "sheetName must not be null if workbookName is provided");
            }
            try {
                targetEvaluator = _bookEvaluator
                        .getOtherWorkbookEvaluator(workbookName);
        ...
        int otherSheetIndex = sheetName == null ? _sheetIndex : targetEvaluator
    ...
    public SheetRangeEvaluator getRefEvaluatorForCurrentSheet() {
        SheetRefEvaluator sre = new SheetRefEvaluator(_bookEvaluator, _tracker,
                _sheetIndex);
     ...
     *
     * @param workbookName the name of the workbook containing the reference. If
     *                     <code>null</code> the current workbook is assumed. Note - to
     *                     evaluate formulas which use multiple workbooks, a
     *                     {@link CollaboratingWorkbooksEnvironment} must be set up.
     * @param sheetName    the name of the sheet containing the reference. May be
     *                     <code>null</code> (when <tt>workbookName</tt> is also null) in
     *                     which case the current workbook and sheet is assumed.
     * @param refStrPart1  the single cell reference or first part of the area reference.
     *                     Must not be <code>null</code>.
     * @param refStrPart2  the second part of the area reference. For single cell
     *                     references this parameter must be <code>null</code>
     * @param isA1Style    specifies the format for <tt>refStrPart1</tt> and
     *                     <tt>refStrPart2</tt>. Pass <code>true</code> for 'A1' style
     *                     and <code>false</code> for 'R1C1' style. TODO - currently POI
     *                     only supports 'A1' reference style
     * @return a {@link RefEval} or {@link AreaEval}
     */
    public ValueEval getDynamicReference(String workbookName, String sheetName,
                                         String refStrPart1, String refStrPart2, boolean isA1Style) {
        if (!isA1Style) {
        ...
        // accessible
        SpreadsheetVersion ssVersion = ((FormulaParsingWorkbook) _workbook)
        ...
        switch (part1refType) {
            case BAD_CELL_OR_NAMED_RANGE:
                return ErrorEval.REF_INVALID;
            case NAMED_RANGE:
                EvaluationName nm = _workbook.getName(
                        refStrPart1, _sheetIndex);
                if (!nm.isRange()) {
                    throw new RuntimeException("Specified name '" + refStrPart1
                            + "' is not a range as expected.");
                }
                return _bookEvaluator.evaluateNameFormula(nm.getNameDefinition(),
                        this);
        }
        if (refStrPart2 == null) {
            // no ':'
            ...
            throw new IllegalStateException(
                    "Unexpected reference classification of '" + refStrPart1
                            + "'.");
        }
    ...
    public ValueEval getRef3DEval(Ref3DPtg rptg) {
        SheetRangeEvaluator sre = createExternSheetRefEvaluator(rptg
                .getExternSheetIndex());
    ...
    public ValueEval getArea3DEval(Area3DPtg aptg) {
        SheetRangeEvaluator sre = createExternSheetRefEvaluator(aptg
                .getExternSheetIndex());
    ...
    public ValueEval getNameXEval(NameXPtg nameXPtg) {
        ...
        ExternalSheet externSheet = _workbook.getExternalSheet(nameXPtg
                .getSheetRefIndex());
        ...
        ExternalName externName = _workbook.getExternalName(
                nameXPtg.getSheetRefIndex(), nameXPtg.getNameIndex());
        return getExternalNameXEval(externName, workbookName);
    }

    public ValueEval getNameXEval(NameXPxg nameXPxg) {
        ExternalSheet externSheet = _workbook.getExternalSheet(
                nameXPxg.getSheetName(), null,
        ...
        String workbookName = externSheet.getWorkbookName();
        ExternalName externName = _workbook.getExternalName(
                nameXPxg.getNameName(), nameXPxg.getSheetName(),
                nameXPxg.getExternalWorkbookNumber());
        return getExternalNameXEval(externName, workbookName);
    }
    ...
    private ValueEval getExternalNameXEval(ExternalName externName,
        ...
        try {
            ...
            // that
            WorkbookEvaluator refWorkbookEvaluator = _bookEvaluator
                    .getOtherWorkbookEvaluator(workbookName);
            EvaluationName evaluationName = refWorkbookEvaluator.getName(
                    externName.getName(), externName.getIx() - 1);
            if (evaluationName != null && evaluationName.hasFormula()) {
                if (evaluationName.getNameDefinition().length > 1) {
                    throw new RuntimeException(
                            "Complex name formulas not supported yet");
                }
                ...
                OperationEvaluationContext refWorkbookContext = new OperationEvaluationContext(
                        ...
                        refWorkbookEvaluator.getWorkbook(), -1, -1, -1,
                        _tracker);
