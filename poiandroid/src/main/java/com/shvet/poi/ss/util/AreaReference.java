...
public class AreaReference {
    ...
    /**
     * The character (:) that separates the two cell references in a multi-cell
     * area reference
     */
    ...
    /**
     * Create an area ref from a string representation. Sheet names containing
     * special characters should be delimited and escaped as per normal syntax
     * rules for formulas.<br/>
     * The area reference must be contiguous (i.e. represent a single rectangle,
     * not a union of rectangles)
     */
    public AreaReference(String reference, SpreadsheetVersion version) {
        ...
        if (parts.length == 1) {
            // TODO - probably shouldn't initialize area ref when text is really
            // a cell ref
    ...
    public AreaReference(CellReference topLeft, CellReference botRight) {
        ...
        if (swapRows || swapCols) {
            ...
            _firstCell = new CellReference(firstRow, firstColumn, firstRowAbs,
                    firstColAbs);
            _lastCell = new CellReference(lastRow, lastColumn, lastRowAbs,
                    lastColAbs);
        } else {
            _firstCell = topLeft;
            _lastCell = botRight;
    ...
    /**
     * Is the reference for a contiguous (i.e. unbroken) area, or is it made up
     * of several different parts? (If it is, you will need to call
    ...
    public static AreaReference getWholeColumn(SpreadsheetVersion version,
                                               String start, String end) {
        return new AreaReference(start + "$1:" + end + "$"
                + version.getMaxRows(), version);
    }
    ...
    public static boolean isWholeColumnReference(SpreadsheetVersion version,
                                                 CellReference topLeft, CellReference botRight) {
        if (null == version) {
            version = SpreadsheetVersion.EXCEL97; // how the code used to
            // behave.
        }
        ...
        return topLeft.getRow() == 0 && topLeft.isRowAbsolute()
                && botRight.getRow() == version.getLastRowIndex()
                && botRight.isRowAbsolute();
    }

    /**
     * Takes a non-contiguous area reference, and returns an array of contiguous
     * area references.
     */
    ...
    /**
     * Separates Area refs in two parts and returns them as separate elements in
     * a String array, each qualified with the sheet name (if present)
     *
    ...
    private static String[] separateAreaRefs(String reference) {
        ...
        for (int i = 0; i < len; i++) {
            switch (reference.charAt(i)) {
                case CELL_DELIMITER:
                    if (!insideDelimitedName) {
                        if (delimiterPos >= 0) {
                            throw new IllegalArgumentException(
                                    "More than one cell delimiter '"
                                            + CELL_DELIMITER
                                            + "' appears in area reference '"
                                            + reference + "'");
                        }
                        delimiterPos = i;
                    }
            ...
            if (i >= len - 1) {
                ...
                throw new IllegalArgumentException("Area reference '"
                        + reference + "' ends with special name delimiter '"
                        + SPECIAL_NAME_DELIMITER + "'");
            }
            if (reference.charAt(i + 1) == SPECIAL_NAME_DELIMITER) {
                // two consecutive quotes is the escape sequence for a single
                // one
        ...
        String sheetName = partA.substring(0, plingPos + 1); // +1 to include
        // delimiter
    ...
    /**
     * @return <code>false</code> if this area reference involves more than one
     * cell
     */
    ...
    /**
     * @return the first cell reference which defines this area. Usually this
     * cell is in the upper left corner of the area (but this is not a
     * requirement).
     */
    ...
    /**
     * Note - if this area reference refers to a single cell, the return value
     * of this method will be identical to that of <tt>getFirstCell()</tt>
     *
     * @return the second cell reference which defines this area. For multi-cell
     * areas, this is cell diagonally opposite the 'first cell'. Usually
     * this cell is in the lower right corner of the area (but this is
     * not a requirement).
     */
    ...
    public CellReference[] getAllReferencedCells() {
        ...
        for (int row = minRow; row <= maxRow; row++) {
            for (int col = minCol; col <= maxCol; col++) {
                CellReference ref = new CellReference(sheetName, row, col,
                        _firstCell.isRowAbsolute(), _firstCell.isColAbsolute());
                refs.add(ref);
            }
        }
     ...
     * <tr>
     * <td>A1:A1</td>
     * <td>Single cell area reference without sheet</td>
     * </tr>
     * <tr>
     * <td>A1:$C$1</td>
     * <td>Multi-cell area reference without sheet</td>
     * </tr>
     * <tr>
     * <td>Sheet1!A$1:B4</td>
     ...
     * </tr>
     * <tr>
     * <td>'O''Brien''s Sales'!B5:C6'&nbsp;</td>
     * <td>Sheet name with special characters</td>
     * </tr>
     * </table>
     *
     * @return the text representation of this area reference as it would appear
     * in a formula.
     */
    public String formatAsString() {
        // Special handling for whole-column references
        if (isWholeColumnReference()) {
            return CellReference.convertNumToColString(_firstCell.getCol())
                    + ":"
