...
/**
 * High level representation of a Excel workbook. This is the first object most
 * users will construct whether they are reading or writing a workbook. It is
 * also the top level object for creating new sheets/etc.
 */
public interface Workbook extends Closeable, Iterable<Sheet> {
    ...
    /**
     * Indicates the book window is hidden, but can be shown by the user via the
     * user interface.
     *
     * @see #setSheetHidden(int, int)
     */
    ...
    /**
     * Sets the tab whose data is actually seen when the sheet is opened. This
    ...
    /**
     * Sreate an Sheet for this Workbook, adds it to the sheets and returns the
     * high level representation. Use this to create new sheets.
     *
     ...
     * @return Sheet with the name provided or <code>null</code> if it does not
     * exist
     */
     ...
     * because it creates/modifies name records which are stored at the workbook
     * level.
     * <p/>
     ...
     * @deprecated use {@link Sheet#setRepeatingRows(CellRangeAddress)} or
     * {@link Sheet#setRepeatingColumns(CellRangeAddress)}
     */
    @Deprecated
    void setRepeatingRowsAndColumns(int sheetIndex, int startColumn,
     ...
     */
    Font findFont(short boldWeight, short color, short fontHeight, String name,
     ...
     * Workbook was read. After closing, the Workbook should no longer be used.
     * <p/>
     ...
     * <i>Note:</i> Excel defined names are case-insensitive and this method
     ...
     * external workbook to be added to this one.
     * <p/>
     * In order for formulas such as "[MyOtherWorkbook]Sheet3!$A$5" to be added
     * to the file, some linking information must first be recorded. Once a
     * given external workbook has been linked, then formulas using it can
     * added. Each workbook needs linking only once.
     * <p/>
     * This linking only applies for writing formulas. To link things for
     * evaluation, see
     * {@link FormulaEvaluator#setupReferencedWorkbooks(java.util.Map)}
     *
     ...
     */
    void setPrintArea(int sheetIndex, int startColumn, int endColumn,
     ...
     * row.
     * <p/>
     * This will then apply to all calls to {@link Row#getCell(int)} . See
    ...
    /**
     * @param hiddenFlag pass <code>false</code> to make the workbook visible in the
     *                   GUI
     */
     ...
     * <p/>
     * Please note that the sheet currently set as active sheet (sheet 0 in a
     * newly created workbook or the one set via setActiveSheet()) cannot be
     * hidden.
     *
