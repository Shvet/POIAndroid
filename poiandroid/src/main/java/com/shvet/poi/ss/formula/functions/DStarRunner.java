...
import com.shvet.poi.ss.util.NumberComparer;

/**
 * This class performs a D* calculation. It takes an {@link IDStarAlgorithm}
 * object and uses it for calculating the result value. Iterating a database and
 * checking the entries against the set of conditions is done here.
 * <p/>
 * TODO: - wildcards ? and * in string conditions - functions as conditions
 */
public final class DStarRunner implements Function3Arg {
    ...
    private static ValueEval solveReference(ValueEval field)
        ...
        if (field instanceof RefEval) {
            ...
            return solveReference(refEval.getInnerValueEval(refEval
                    .getFirstSheetIndex()));
        } else {
            return field;
        }
    ...
    /**
     * Returns the first column index that matches the given name. The name can
     * either be a string or an integer, when it's an integer, then the
     * respective column (1 based index) is returned.
     *
     * @param nameValueEval
     * @param db
    ...
    private static int getColumnForTag(ValueEval nameValueEval, TwoDEval db)
        ...
        if (nameValueEval instanceof NumericValueEval) {
            double doubleResultColumn = ((NumericValueEval) nameValueEval)
                    .getNumberValue();
     ...
     * @throws EvaluationException If references could not be resolved or comparison operators
     *                             and operands didn't match.
     */
    private static boolean fullfillsConditions(TwoDEval db, int row,
                                               TwoDEval cdb) throws EvaluationException {
        ...
        for (int conditionRow = 1; conditionRow < cdb.getHeight(); ++conditionRow) {
            ...
            for (int column = 0; column < cdb.getWidth(); ++column) { // columns
                // are
                // ANDed
                // Whether the condition column matches a database column, if
                // not it's a
                ...
                if (!(targetHeader instanceof StringValueEval))
                    ...
                    // No column found, it's again a special column that accepts
                    // formulas.
                    columnCondition = false;

                if (columnCondition == true) { // normal column condition
                    // Should not throw, checked above.
                    ValueEval target = db.getValue(row,
    ...
    private static boolean testNormalCondition(ValueEval value, String condition)
        ...
        if (condition.startsWith("<")) { // It's a </<= condition.
            ...
            if (number.startsWith("=")) {
                ...
                return testNumericCondition(value, operator.smallerEqualThan,
                        number);
            } else {
            ...
            if (number.startsWith("=")) {
                number = number.substring(1);
                return testNumericCondition(value, operator.largerEqualThan,
                        number);
            } else {
     ...
     */
    private static boolean testNumericCondition(ValueEval valueEval,
                                                operator op, String condition) throws EvaluationException {
    ...
    /**
     * Takes a ValueEval and tries to retrieve a String value from it. It tries
     * to resolve references if there are any.
     *
    ...
    public final ValueEval evaluate(ValueEval[] args, int srcRowIndex,
                                    int srcColumnIndex) {
        if (args.length == 3) {
    ...
    public ValueEval evaluate(int srcRowIndex, int srcColumnIndex,
                              ValueEval database, ValueEval filterColumn,
                              ValueEval conditionDatabase) {
        // Input processing and error checks.
        if (!(database instanceof TwoDEval)
                || !(conditionDatabase instanceof TwoDEval)) {
            return ErrorEval.VALUE_INVALID;
        }
        ...
        for (int row = 1; row < db.getHeight(); ++row) {
            ...
            if (matches) {
                try {
                    ValueEval currentValueEval = solveReference(db.getValue(
                            row, fc));
                    // Pass the match to the algorithm and conditionally abort
                    // the search.
                    boolean shouldContinue = algorithm
                            .processMatch(currentValueEval);
                    if (!shouldContinue) {
                        break;
                    }
                } catch (EvaluationException e) {
    ...
    private enum operator {
        largerThan, largerEqualThan, smallerThan, smallerEqualThan, equal
    }
}
