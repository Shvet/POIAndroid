...
package com.shvet.poi.ss.formula;

import com.shvet.poi.ss.SpreadsheetVersion;
import com.shvet.poi.ss.formula.constant.ErrorConstant;
import com.shvet.poi.ss.formula.function.FunctionMetadata;
import com.shvet.poi.ss.formula.function.FunctionMetadataRegistry;
import com.shvet.poi.ss.formula.ptg.AbstractFunctionPtg;
import com.shvet.poi.ss.formula.ptg.AddPtg;
import com.shvet.poi.ss.formula.ptg.AreaPtg;
import com.shvet.poi.ss.formula.ptg.ArrayPtg;
import com.shvet.poi.ss.formula.ptg.AttrPtg;
import com.shvet.poi.ss.formula.ptg.BoolPtg;
import com.shvet.poi.ss.formula.ptg.ConcatPtg;
import com.shvet.poi.ss.formula.ptg.DividePtg;
import com.shvet.poi.ss.formula.ptg.EqualPtg;
import com.shvet.poi.ss.formula.ptg.ErrPtg;
import com.shvet.poi.ss.formula.ptg.FuncPtg;
import com.shvet.poi.ss.formula.ptg.FuncVarPtg;
import com.shvet.poi.ss.formula.ptg.GreaterEqualPtg;
import com.shvet.poi.ss.formula.ptg.GreaterThanPtg;
import com.shvet.poi.ss.formula.ptg.IntPtg;
import com.shvet.poi.ss.formula.ptg.IntersectionPtg;
import com.shvet.poi.ss.formula.ptg.LessEqualPtg;
import com.shvet.poi.ss.formula.ptg.LessThanPtg;
import com.shvet.poi.ss.formula.ptg.MemAreaPtg;
import com.shvet.poi.ss.formula.ptg.MemFuncPtg;
import com.shvet.poi.ss.formula.ptg.MissingArgPtg;
import com.shvet.poi.ss.formula.ptg.MultiplyPtg;
import com.shvet.poi.ss.formula.ptg.NamePtg;
import com.shvet.poi.ss.formula.ptg.NameXPtg;
import com.shvet.poi.ss.formula.ptg.NotEqualPtg;
import com.shvet.poi.ss.formula.ptg.NumberPtg;
import com.shvet.poi.ss.formula.ptg.OperandPtg;
import com.shvet.poi.ss.formula.ptg.OperationPtg;
import com.shvet.poi.ss.formula.ptg.ParenthesisPtg;
import com.shvet.poi.ss.formula.ptg.PercentPtg;
import com.shvet.poi.ss.formula.ptg.PowerPtg;
import com.shvet.poi.ss.formula.ptg.Ptg;
import com.shvet.poi.ss.formula.ptg.RangePtg;
import com.shvet.poi.ss.formula.ptg.RefPtg;
import com.shvet.poi.ss.formula.ptg.StringPtg;
import com.shvet.poi.ss.formula.ptg.SubtractPtg;
import com.shvet.poi.ss.formula.ptg.UnaryMinusPtg;
import com.shvet.poi.ss.formula.ptg.UnaryPlusPtg;
import com.shvet.poi.ss.formula.ptg.UnionPtg;
import com.shvet.poi.ss.formula.ptg.ValueOperatorPtg;
import com.shvet.poi.ss.usermodel.ErrorConstants;
import com.shvet.poi.ss.util.AreaReference;
import com.shvet.poi.ss.util.CellReference;
import com.shvet.poi.ss.util.CellReference.NameType;
 ...
 * Inspired by Lets Build a Compiler, by Jack Crenshaw BNF for the formula
 * expression is : <expression> ::= <term> [<addop> <term>]* <term> ::= <factor>
 * [ <mulop> <factor> ]* <factor> ::= <number> | (<expression>) | <cellRef> |
 * <function> <function> ::= <functionName> ([expression [, expression]*])
 * <p/>
...
public final class FormulaParser {
     ...
     * {@link org.apache.poi.ss.util.CellReference#CELL_REF_PATTERN})
     */
    private static final Pattern CELL_REF_PATTERN = Pattern
            .compile("(\\$?[A-Za-z]+)?(\\$?[0-9]+)?");
    ...
    /**
     * Lookahead Character. gets value '\0' when the input string is exhausted
     */
    private char look;
    /**
    ...
    /**
     * Create the formula parser, with the string that is to be parsed against
     * the supplied workbook. A later call the parse() method to return ptg list
     * in rpn order, then call the getRPNPtg() to retrieve the parse results.
     * This class is recommended only for single threaded use.
     * <p/>
     * If you only have a usermodel.HSSFWorkbook, and not a model.Workbook, then
     * use the convenience method on usermodel.HSSFFormulaEvaluator
     */
    private FormulaParser(String formula, FormulaParsingWorkbook book,
        ...
        _pointer = 0;
        _book = book;
        _ssVersion = book == null ? SpreadsheetVersion.EXCEL97 : book
     ...
     * @param sheetIndex  the 0-based index of the sheet this formula belongs to. The
     *                    sheet index is required to resolve sheet-level names.
     *                    <code>-1</code> means that the scope of the name will be
     *                    ignored and the parser will match names only by name
     * @return array of parsed tokens
     * @throws FormulaParseException if the formula has incorrect syntax or is otherwise invalid
     */
    public static Ptg[] parse(String formula, FormulaParsingWorkbook workbook,
                              int formulaType, int sheetIndex) {
    ...
    /**
     * From OOO doc: "Whenever one operand of the reference subexpression is a
     * function, a defined name, a 3D reference, or an external reference (and
     * no error occurs), a tMemFunc token is used"
     */
     ...
     */
    private static void checkValidRangeOperand(String sideName,
                                               int currentParsePosition, ParseNode pn) {
    ...
    /**
     * @return <code>false</code> if sub-expression represented the specified
     * ParseNode definitely cannot appear on either side of the range
     * (':') operator
     */
    ...
    /**
     * @return <code>true</code> if the specified character may be used in a
     * defined name
     */
    ...
    /**
     * Get a PTG for an integer from its string representation. return Int or
     * Number Ptg based on size of input
     */
    private static Ptg getNumberPtgFromString(String number1, String number2,
                                              String exponent) {
    ...
    /**
     * Consumes the next input character if it is equal to the one specified
     * otherwise throws an unchecked exception. This method does <b>not</b>
     * consume whitespace (before or after the matched character).
     */
    ...
    /**
     * Parses area refs (things which could be the operand of ':') and simple
     * factors Examples
     * <p/>
     * <pre>
     *   A$1
                ...
                if (look == '#') { // error ref like MySheet!#REF!
                    ...
                    if (nameXPtg == null) {
                        throw new FormulaParseException("Specified name '"
                                + name + "' for sheet "
                                + sheetIden.asFormulaString() + " not found");
                    }
        ...
        if (look == ':') {
            ...
            if (part2 == null) {
                ...
                if (!part1.isCell()) {
                    ...
                    throw new FormulaParseException(prefix + part1.getRep()
                            + "' is not a proper reference.");
                }
        ...
        if (look == '.') {
            ...
            if (whiteAfterPart1 || whiteBeforePart2) {
                if (part1.isRowOrColumn() || part2.isRowOrColumn()) {
                    ...
                    // and there's no other valid expression that fits this
                    // grammar
                    throw new FormulaParseException(
            ...
            if (part1.isRowOrColumn() || part2.isRowOrColumn()) {
                if (dotCount != 2) {
                    throw new FormulaParseException(
                            "Dotted range (full row or column) expression '"
                                    + part1And2 + "' must have exactly 2 dots.");
                }
            }
            return createAreaRefParseNode(sheetIden, part1, part2);
        }
     ...
     * i.e. '!', ':'(and equiv '...') do not appear Examples
     * <p/>
     * <pre>
     *   my.named...range.
     *   foo.bar(123.456, "abc")
    ...
    private ParseNode parseNonRange(int savePointer) {
        ...
        if (_book == null) {
            ...
            throw new IllegalStateException("Need book to evaluate name '"
                    + name + "'");
        }
        EvaluationName evalName = _book.getName(name, _sheetIndex);
        if (evalName == null) {
            throw new FormulaParseException("Specified named range '" + name
     ...
     */
    private ParseNode createAreaRefParseNode(SheetIdentifier sheetIden,
                                             SimpleRangePart part1, SimpleRangePart part2)
    ...
    private AreaReference createAreaRef(SimpleRangePart part1,
        ...
        if (part1.isRow()) {
            return AreaReference.getWholeRow(_ssVersion, part1.getRep(),
                    part2.getRep());
        }
        if (part1.isColumn()) {
    ...
    /**
     * Parses out a potential LHS or RHS of a ':' intended to produce a plain
     * AreaRef. Normally these are proper cell references but they could also be
     * row or column refs like "$AC" or "10"
     *
     * @return <code>null</code> (and leaves {@link #_pointer} unchanged if a
     * proper range part does not parse out
     */
    ...
    /**
     * Note - caller should reset {@link #_pointer} upon <code>null</code>
     * result
     *
     * @return The sheet name as an identifier <code>null</code> if '!' is not
    ...
    /**
     * If we have something that looks like [book]Sheet1: or Sheet1, see if it's
     * actually a range eg Sheet1:Sheet2!
     */
    private SheetIdentifier parseSheetRange(String bookname,
                                            NameIdentifier sheet1Name) {
        GetChar();
    ...
    private boolean isValidCellReference(String str) {
        ...
        if (result) {
            /**
             * Check if the argument is a function. Certain names can be either
             * a cell reference or a function name depending on the contenxt.
             * Compare the following examples in Excel 2007: (a) LOG10(100) + 1
             * (b) LOG10 + 1 In (a) LOG10 is a name of a built-in function. In
             * (b) LOG10 is a cell reference
             */
            boolean isFunc = FunctionMetadataRegistry.getFunctionByName(str
            ...
            if (isFunc) {
                ...
                // the returning value should be false, i.e.
    ...
    /**
     * Note - Excel function names are 'case aware but not case sensitive'. This
     * method may end up creating a defined name record in the workbook if the
     * specified name is not an internal Excel function, and has not been
     * encountered before.
     *
     * @param name case preserved function name (as it was entered/appeared in
     *             the formula).
     */
     ...
     * @return Ptg a null is returned if we're in an IF formula, it needs
     * extreme manipulation and is handled in this function
     */
    private ParseNode getFunction(String name, Ptg namePtg, ParseNode[] args) {

        FunctionMetadata fm = FunctionMetadataRegistry.getFunctionByName(name
                .toUpperCase(Locale.ROOT));
        int numArgs = args.length;
        if (fm == null) {
            if (namePtg == null) {
                throw new IllegalStateException(
                        "NamePtg must be supplied for external functions");
            }
        ...
        if (namePtg != null) {
            throw new IllegalStateException(
                    "NamePtg no applicable to internal functions");
        }
        boolean isVarArgs = !fm.hasFixedArgsLength();
        int funcIx = fm.getIndex();
        if (funcIx == FunctionMetadataRegistry.FUNCTION_INDEX_SUM
                && args.length == 1) {
            ...
            // The code below would encode tFuncVar(SUM) which seems to do no
            // harm
        }
    ...
    private ParseNode parseSimpleFactor() {
        ...
        if (IsAlpha(look) || Character.isDigit(look) || look == '\''
                || look == '[') {
    ...
    private ParseNode parseUnary(boolean isPlus) {
        ...
        return new ParseNode(isPlus ? UnaryPlusPtg.instance
                : UnaryMinusPtg.instance, factor);
    }
    ...
    private void checkRowLengths(Object[][] values2d, int nColumns) {
        for (int i = 0; i < values2d.length; i++) {
            ...
            if (rowLen != nColumns) {
                throw new FormulaParseException("Array row " + i
                        + " has length " + rowLen + " but row 0 has length "
                        + nColumns);
            }
        }
    }
    ...
    /**
     * procedure Assignment; var Name: string[8]; begin Name := GetName;
     * Match('='); Expression;
     * <p/>
     * end;
     **/
    ...
    private static final class SimpleRangePart {
        ...
        public SimpleRangePart(String rep, boolean hasLetters,
                               boolean hasNumbers) {
            _rep = rep;
        ...
        /**
         * @return <code>true</code> if the two range parts can be combined in
         * an {@link AreaPtg} ( Note - the explicit range operator (:)
         * may still be valid when this method returns
         * <code>false</code> )
         */
            ...
            public static Type get(boolean hasLetters, boolean hasDigits) {
                ...
                if (!hasDigits) {
                    throw new IllegalArgumentException(
                            "must have either letters or numbers");
                }
                return ROW;
            }
        }
