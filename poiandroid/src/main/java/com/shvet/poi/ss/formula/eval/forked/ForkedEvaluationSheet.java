 ...
 * this class contain only the cells from the master workbook. By calling
 * {@link #getOrCreateUpdatableCell(int, int)}, the master cell object is
 * logically replaced with a {@link ForkedEvaluationCell} instance, which will
 * be used in all subsequent evaluations.
 *
 * @author Josh Micich
 */
final class ForkedEvaluationSheet implements EvaluationSheet {
    ...
    /**
     * Only cells which have been split are put in this map. (This has been done
     * to conserve memory).
     */
    ...
    public ForkedEvaluationCell getOrCreateUpdatableCell(int rowIndex,
                                                         int columnIndex) {
        RowColKey key = new RowColKey(rowIndex, columnIndex);
