...
final class FormulaUsedBlankCellSet {
    ...
    public void addCell(int bookIndex, int sheetIndex, int rowIndex,
                        int columnIndex) {
    ...
    public static final class BookSheetKey {
        ...
        public boolean equals(Object obj) {
            ...
            return _bookIndex == other._bookIndex
                    && _sheetIndex == other._sheetIndex;
        }
    }
    ...
    private static final class BlankCellSheetGroup {
        ...
        public void addCell(int rowIndex, int columnIndex) {
            if (_currentRowIndex == -1) {
                ...
                if (_currentRowIndex == rowIndex
                        && _lastColumnIndex + 1 == columnIndex) {
                    _lastColumnIndex = columnIndex;
                } else {
                    // cell does not fit on end of current row
                    if (_currentRectangleGroup == null) {
                        _currentRectangleGroup = new BlankCellRectangleGroup(
                                _currentRowIndex, _firstColumnIndex,
                                _lastColumnIndex);
                    } else {
                        if (!_currentRectangleGroup.acceptRow(_currentRowIndex,
        ...
        public boolean containsCell(int rowIndex, int columnIndex) {
            ...
            if (_currentRectangleGroup != null
                    && _currentRectangleGroup.containsCell(rowIndex,
                    columnIndex)) {
                return true;
            }
            if (_currentRowIndex != -1 && _currentRowIndex == rowIndex) {
                if (_firstColumnIndex <= columnIndex
                        && columnIndex <= _lastColumnIndex) {
                    return true;
                }
            }
            return false;
    ...
    private static final class BlankCellRectangleGroup {
        ...
        public BlankCellRectangleGroup(int firstRowIndex, int firstColumnIndex,
                                       int lastColumnIndex) {
        ...
        public boolean acceptRow(int rowIndex, int firstColumnIndex,
                                 int lastColumnIndex) {
        ...
        public String toString() {
            ...
            CellReference crA = new CellReference(_firstRowIndex,
                    _firstColumnIndex, false, false);
            CellReference crB = new CellReference(_lastRowIndex,
                    _lastColumnIndex, false, false);
            sb.append(getClass().getName());
            sb.append(" [").append(crA.formatAsString()).append(':')
                    .append(crB.formatAsString()).append("]");
            return sb.toString();
        }
    }
}
