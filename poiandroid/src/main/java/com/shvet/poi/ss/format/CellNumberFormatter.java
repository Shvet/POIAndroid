...
==================================================================== */
package com.shvet.poi.ss.format;

import com.shvet.poi.ss.format.CellFormatPart.PartHandler;
...
public class CellNumberFormatter extends CellFormatter {
    ...
    public CellNumberFormatter(String format) {
        ...
        if (decimalPoint == null) {
            ...
            if (precision == 0) {
                // This means the format has a ".", but that output should have
                // no decimals after it.
        ...
        if (precision == 0)
            ...
            fractionalSpecials = specials.subList(
                    specials.indexOf(decimalPoint) + 1, fractionalEnd());
        if (exponent == null)
        ...
        if (exponent == null) {
            ...
            fmtBuf.append('E');
            placeZeros(fmtBuf,
                    exponentSpecials.subList(2, exponentSpecials.size()));
            DecimalFormatSymbols dfs = DecimalFormatSymbols
                    .getInstance(LocaleUtil.getUserLocale());
            decimalFmt = new DecimalFormat(fmtBuf.toString(), dfs);
        }

        if (exponent != null)
            scale = 1; // in "e" formats,% and trailing commas have no scaling
        // effect
    ...
    private int calculateIntegerPartWidth() {
        ...
        while (it.hasNext()) {
            Special s = it.next();
            // !! Handle fractions: The previous set of digits before that is
            // the numerator, so we should stop short of that
            if (s == afterInteger)
                break;
    ...
    private int interpretPrecision() {
        if (decimalPoint == null) {
            ...
            int precision = 0;
            ListIterator<Special> it = specials.listIterator(specials
                    .indexOf(decimalPoint));
            if (it.hasNext())
    ...
    private void interpretCommas(StringBuffer sb) {
        // In the integer part, commas at the end are scaling commas; other
        // commas mean to show thousand-grouping commas
        ...
        // Now strip them out -- we only need their interpretation, not their
        // presence
    ...
    private int integerEnd() {
        if (decimalPoint != null)
            ...
            afterInteger = null;
        return afterInteger == null ? specials.size() : specials
                .indexOf(afterInteger);
    }
    ...
    private int fractionalEnd() {
        ...
        end = afterFractional == null ? specials.size() : specials
                .indexOf(afterFractional);
        return end;
    }
            ...
            while (nextChange != null && s == nextChange.special) {
                ...
                switch (nextChange.op) {
                    ...
                    case StringMod.REPLACE:
                        int delPos = s.pos; // delete starting pos in original
                        // coordinates
                        ...
                        int delEndPos = nextChange.end.pos; // delete end point in
                        // original
                        if (nextChange.endInclusive)
                            delEndPos++;

                        int modEndPos = delEndPos + adjust; // delete end point in
                        // current
    ...
    private void writeScientific(double value, StringBuffer output,
		 ...
		 *
		 * In DecimalFormat, you never put the sign in the format, and the sign
		 * only comes out of the format if it is negative.
		 *
		 * In Excel, you always say whether to always show the sign ("e+") or
		 * only show negative signs ("e-").
		 *
		 * Also in Excel, where you put the sign in the format is NOT where it
		 * comes out in the result. In the format, the sign goes with the "e";
		 * in the output it goes with the exponent value. That is, if you say
		 * "#e-|#" you get "1e|-5", not "1e-|5". This makes sense I suppose, but
		 * it complicates things.
		 *
		 * Finally, everything else in this formatting code assumes that the
		 * base of the result is the original format, and that starting from
		 * that situation, the indexes of the original special characters can be
		 * used to place the new characters. As just described, this is not true
		 * for the exponent's sign. <p/> So here is how we handle it:
		 *
		 * (1) When parsing the format, remove the sign from after the 'e' and
		 * put it before the first digit of the exponent (where it will be
		 * shown).
		 *
		 * (2) Determine the result's sign.
		 *
		 * (3) If it's missing, put the sign into the output to keep the result
		 * lined up with the output. (In the result, "after the 'e'" and "before
		 * the first digit" are the same because the result has no extra chars
		 * to be in the way.)
		 *
		 * (4) In the output, remove the sign if it should not be shown ("e-"
		 * was used and the sign is negative) or set it to the correct value.
		 */
        ...
        if (expSignRes != '-') {
            ...
            expSignRes = '+';
            // (3) If it's missing, put the sign into the output to keep the
            // result
            // lined up with the output.
            result.insert(signPos, '+');
        }

        // Now the result lines up like it is supposed to with the specials'
        // indexes
        ListIterator<Special> it = exponentSpecials.listIterator(1);
        Special expSign = it.next();
        char expSignFmt = expSign.ch;

        // (4) In the output, remove the sign if it should not be shown or set
        // it to
        // the correct value.
        ...
        StringBuffer exponentNum = new StringBuffer(
                result.substring(signPos + 1));
    ...
    private void writeFraction(double value, StringBuffer result,
        ...
        // Figure out if we are to suppress either the integer or fractional
        // part.
        // With # the suppressed part is removed; with ? it is replaced with
        // spaces.
        if (!improperFraction) {
            // If fractional part is zero, and numerator doesn't have '0', write
            // out
            ...
            if (fractional == 0 && !hasChar('0', numeratorSpecials)) {
                ...
                Special end = denominatorSpecials.get(denominatorSpecials
                        .size() - 1);
                if (hasChar('?', integerSpecials, numeratorSpecials,
                        denominatorSpecials)) {
                    // if any format has '?', then replace the fraction with
                    // spaces
                ...
                boolean willShowFraction = fractional != 0
                        || hasChar('0', numeratorSpecials);
                boolean removeBecauseZero = allZero
                        && (hasOnly('#', integerSpecials) || !hasChar('0',
                        numeratorSpecials));
                boolean removeBecauseFraction = !allZero && value == 0
                        && willShowFraction && !hasChar('0', integerSpecials);
                if (removeBecauseZero || removeBecauseFraction) {
                    Special start = integerSpecials
                            .get(integerSpecials.size() - 1);
                    if (hasChar('?', integerSpecials, numeratorSpecials)) {
        ...
        try {
            ...
            if (fractional == 0 || (improperFraction && fractional % 1 == 0)) {
                ...
                SimpleFraction frac = SimpleFraction
                        .buildFractionMaxDenominator(fractional, maxDenominator);
                n = frac.getNumerator();
    ...
    private void writeInteger(StringBuffer result, StringBuffer output,
                              List<Special> numSpecials, Set<StringMod> mods, boolean showCommas) {
        ...
        while (it.hasPrevious()) {
            ...
            if (pos >= 0)
                ...
                // If result is shorter than field, pretend there are leading
                // zeros
                resultCh = '0';
            }
            Special s = it.previous();
        ...
        if (pos >= 0) {
            // We ran out of places to put digits before we ran out of digits;
            // put this aside so we can add it later
            ++pos; // pos was decremented at the end of the loop above when the
            // iterator was at its end
     ...
     * modifications that need to be made. Finally, we go through and apply them
     * all, dealing with overlapping modifications.
     */
    ...
    private class NumPartHandler implements PartHandler {
        ...
        public String handlePart(Matcher m, String part, CellFormatType type,
            ...
            switch (firstCh) {
                case 'e':
                case 'E':
                    // See comment in writeScientific -- exponent handling is
                    // complex.
                    // (1) When parsing the format, remove the sign from after the
                    // 'e' and
                ...
                case '%':
                    // don't need to remember because we don't need to do anything
                    // with these
                    scale *= 100;
                    break;
