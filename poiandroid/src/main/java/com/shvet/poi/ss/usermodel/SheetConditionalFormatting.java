...
public interface SheetConditionalFormatting {
     ...
     * @param regions - list of rectangular regions to apply conditional formatting
     *                rules
     * @param cfRules - set of up to conditional formatting rules (max 3 for Excel
     *                pre-2007)
     * @return index of the newly created Conditional Formatting object
     */
    int addConditionalFormatting(CellRangeAddress[] regions,
     ...
     * <p>
     * This method could be used to copy ConditionalFormatting object from one
     * sheet to another. For example:
     * </p>
     * <p/>
     * <pre>
     ...
     * <p>
     * The created conditional formatting rule compares a cell value to a
     * formula calculated result, using the specified operator. The type of the
     * created condition is {@link ConditionType#CELL_VALUE_IS}
     * </p>
     *
     * @param comparisonOperation - MUST be a constant value from
     *                            <tt>{@link ComparisonOperator}</tt>:
     *                            <p>
     *                            <ul>
     *                            <li>BETWEEN</li>
     *                            <li>NOT_BETWEEN</li>
     ...
     *                            {@link ComparisonOperator#BETWEEN}) and
     *                            {@link ComparisonOperator#NOT_BETWEEN} operations)
     */
    ConditionalFormattingRule createConditionalFormattingRule(
            byte comparisonOperation, String formula1, String formula2);

    /**
     * Create a conditional formatting rule that compares a cell value to a
     * formula calculated result, using an operator *
     * <p>
     * The type of the created condition is {@link ConditionType#CELL_VALUE_IS}
     * </p>
     *
     * @param comparisonOperation MUST be a constant value from
     *                            <tt>{@link ComparisonOperator}</tt> except BETWEEN and
     *                            NOT_BETWEEN
     * @param formula             the formula to determine if the conditional formatting is
     *                            applied
     */
    ConditionalFormattingRule createConditionalFormattingRule(
            byte comparisonOperation, String formula);

    /**
     * Create a conditional formatting rule based on a Boolean formula. When the
     ...
     * {@link ConditionalFormattingRule#getDataBarFormatting()} then
     * {@link DataBarFormatting#getMinThreshold()} and
     ...
     * <p/>
     * The thresholds for it will be created, but will be empty and require
     * configuring with
     * {@link ConditionalFormattingRule#getMultiStateFormatting()} then
     ...
     * require configuring with
     * {@link ConditionalFormattingRule#getColorScaleFormatting()} then
     * {@link ColorScaleFormatting#getThresholds()} and
     ...
     * @throws IllegalArgumentException if the index is outside of the allowable range (0 ...
     *                                  numberOfFormats-1)
     */
