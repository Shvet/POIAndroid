...
import com.shvet.poi.ss.usermodel.DataFormatter;
...
public class SheetUtil {
    ...
    /**
     * Excel measures columns in units of 1/256th of a character width but the
     * docs say nothing about what particular character is used. '0' looks to be
     * a good choice.
     */
    private static final char defaultChar = '0';

    /**
     * This is the multiple that the font height is scaled by when determining
     * the boundary of rotated text.
     */
    private static final double fontHeightMultiple = 2.0;

    /**
     * Dummy formula evaluator that does nothing. YK: The only reason of having
     * this class is that
     * {@link org.apache.poi.ss.usermodel.DataFormatter#formatCellValue(org.apache.poi.ss.usermodel.Cell)}
     * returns formula string for formula cells. Dummy evaluator makes it to
     * format the cached formula result.
     * <p/>
     * See Bugzilla #50021
     */
    private static final FormulaEvaluator dummyEvaluator = new FormulaEvaluator() {
        ...
        public void setupReferencedWorkbooks(
                Map<String, FormulaEvaluator> workbooks) {
        }
    ...
    public static double getCellWidth(Cell cell, int defaultCharWidth,
        ...
        for (int i = 0; i < sheet.getNumMergedRegions(); i++) {
            ...
            if (containsCell(region, row.getRowNum(), column)) {
                if (!useMergedCells) {
                    // If we're not using merged cells, skip this one and move
                    // on to the next.
                    return -1;
                }
        ...
        // for formula cells we compute the cell width for the cached formula
        // result
    ...
    private static double getCellWidth(int defaultCharWidth, int colspan,
        ...
        if (style.getRotation() != 0) {
            /*
             * Transform the text using a scale so that it's height is increased
			 * by a multiple of the leading, and then rotate the text before
			 * computing the bounds. The scale results in some whitespace around
			 * the unrotated top and bottom of the text that normally wouldn't
			 * be present if unscaled, but is added by the standard Excel
			 * autosize.
			 */
            AffineTransform trans = new AffineTransform();
            trans.concatenate(AffineTransform.getRotateInstance(style
                    .getRotation() * 2.0 * Math.PI / 360.0));
            trans.concatenate(AffineTransform.getScaleInstance(1,
                    fontHeightMultiple));
            width = Math.max(width, ((layout.getOutline(trans).getBounds()
                    .getWidth() / colspan) / defaultCharWidth)
                    + style.getIndention());
        } else {
            width = Math
                    .max(width,
    ...
    public static double getColumnWidth(Sheet sheet, int column,
        ...
        return getColumnWidth(sheet, column, useMergedCells,
     ...
     */
    public static double getColumnWidth(Sheet sheet, int column,
                                        boolean useMergedCells, int firstRow, int lastRow) {
        Workbook wb = sheet.getWorkbook();
        ...
        for (int rowIdx = firstRow; rowIdx <= lastRow; ++rowIdx) {
            ...
            if (row != null) {
                ...
                double cellWidth = getCellWidth(cell, defaultCharWidth,
                        formatter, useMergedCells);
    ...
    /**
     * Check if the Fonts are installed correctly so that Java can compute the
     * size of columns.
     * <p/>
     * If a Cell uses a Font which is not available on the operating system then
     * Java may fail to return useful Font metrics and thus lead to an
     * auto-computed size of 0.
     * <p/>
     * This method allows to check if computing the sizes for a given Font will
     * succeed or not.
     *
     * @param font The Font that is used in the Cell
     * @return true if computing the size for this Font will succeed, false
     * otherwise
     */
    public static boolean canComputeColumnWidht(Font font) {
        // not sure what is the best value sample-here, only "1" did not work on
        // some platforms...
     ...
     * <p/>
     * <p/>
     * If the cell at the given co-ordinates is a merged cell, this will return
     * the primary (top-left) most cell of the merged region.
     * <p/>
     * If the cell at the given co-ordinates is not in a merged region, then
     * will return the cell itself.
     * <p/>
