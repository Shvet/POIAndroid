...
import com.shvet.poi.ss.formula.ptg.TblPtg;
...
public class Formula {
    ...
    private Formula(byte[] byteEncoding, int encodedTokenLen) {
        ...
        // LittleEndianByteArrayInputStream in = new
        // LittleEndianByteArrayInputStream(byteEncoding);
    ...
    /**
     * When there are no array constants present, <tt>encodedTokenLen</tt>==
     * <tt>totalEncodedLen</tt>
     *
     * @param encodedTokenLen number of bytes in the stream taken by the plain formula
     *                        tokens
     * @param totalEncodedLen the total number of bytes in the formula (includes trailing
     *                        encoding for array constants, but does not include 2 bytes for
     *                        initial <tt>ushort encodedTokenLen</tt> field.
     ...
     * @return Never <code>null</code> (Possibly empty if the supplied
     * <tt>ptgs</tt> is <code>null</code>)
     */
    ...
    /**
     * Gets the {@link Ptg} array from the supplied {@link Formula}. Handles
     * <code>null</code>s OK.
     *
     * @param formula may be <code>null</code>
     * @return possibly <code>null</code> (if the supplied <tt>formula</tt> is
     * <code>null</code>)
     */
    ...
    public Ptg[] getTokens() {
        LittleEndianInput in = new LittleEndianByteArrayInputStream(
                _byteEncoding);
    ...
    /**
     * This method is often used when the formula length does not appear
     * immediately before the encoded token data.
     *
     * @return the encoded length of the plain formula tokens. This does
     * <em>not</em> include the leading ushort field, nor any trailing
     * array constant data.
     */
     ...
     * {@link org.apache.poi.hssf.record.SharedFormulaRecord},
     * {@link org.apache.poi.hssf.record.ArrayRecord} or
     * {@link org.apache.poi.hssf.record.TableRecord} if this formula belongs to
     * such a grouping. The {@link CellReference} returned by this method will
     * match the top left corner of the range of that grouping. The return value
     * is usually not the same as the location of the cell containing this
     * formula.
     *
     * @return the firstRow & firstColumn of an array formula or shared formula
     * that this formula belongs to. <code>null</code> if this formula
     * is not part of an array or shared formula.
     */
    public CellReference getExpReference() {
        byte[] data = _byteEncoding;
        if (data.length != 5) {
            // tExp and tTbl are always 5 bytes long, and the only ptg in the
            // formula
            return null;
        }
        switch (data[0]) {
