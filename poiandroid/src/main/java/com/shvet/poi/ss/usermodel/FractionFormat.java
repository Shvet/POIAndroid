...
import com.shvet.poi.ss.format.SimpleFraction;
 ...
 * <p/>
 * <p>
 * As of this writing, this is still not 100% accurate, but it does a reasonable
 * job of trying to mimic Excel's fraction calculations. It does not currently
 * maintain Excel's spacing.
 * </p>
 * <p/>
 * <p>
 * This class relies on a method lifted nearly verbatim from
 * org.apache.math.fraction. If further uses for Commons Math are found, we will
 * consider adding it as a dependency. For now, we have in-lined the one method
...
public class FractionFormat extends Format {
    private final static Pattern DENOM_FORMAT_PATTERN = Pattern
            .compile("(?:(#+)|(\\d+))");
    ...
    public String format(Number num) {
        ...
        try {
            ...
            if (exactDenom > 0) {
                fract = SimpleFraction.buildFractionExactDenominator(decPart,
                        exactDenom);
            } else {
                fract = SimpleFraction.buildFractionMaxDenominator(decPart,
                        maxDenom);
            }
        ...
        if ("".equals(wholePartFormatString)) {
            int trueNum = (fract.getDenominator() * (int) wholePart)
                    + fract.getNumerator();
        ...
        sb.append(fract.getNumerator()).append("/")
                .append(fract.getDenominator());
        return sb.toString();
    }
