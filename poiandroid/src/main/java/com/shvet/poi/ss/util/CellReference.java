 ...
 * references, and POI usermodel style row=0, column=0 style references. Handles
 * sheet-based and sheet-free references as well, eg "Sheet1!A1" and "$B$72"
 */
public class CellReference {
    ...
    /**
     * The character (') used to quote sheet names when they contain special
     * characters
     */
    private static final char SPECIAL_NAME_DELIMITER = '\'';
    /**
     * Matches a run of one or more letters followed by a run of one or more
     * digits. The run of letters is group 1 and the run of digits is group 2.
     * Each group may optionally be prefixed with a single '$'.
     */
    private static final Pattern CELL_REF_PATTERN = Pattern
            .compile("\\$?([A-Za-z]+)\\$?([0-9]+)");
    /**
     * Matches a run of one or more letters. The run of letters is group 1. The
     * text may optionally be prefixed with a single '$'.
     */
    private static final Pattern COLUMN_REF_PATTERN = Pattern
            .compile("\\$?([A-Za-z]+)");
    /**
     * Matches a run of one or more digits. The run of digits is group 1. The
     * text may optionally be prefixed with a single '$'.
     */
    private static final Pattern ROW_REF_PATTERN = Pattern
            .compile("\\$?([0-9]+)");
    /**
     * Named range names must start with a letter or underscore. Subsequent
     * characters may include digits or dot. (They can even end in dot).
     */
    private static final Pattern NAMED_RANGE_NAME_PATTERN = Pattern
            .compile("[_A-Za-z][_.A-Za-z0-9]*");
    ...
    /**
     * Create an cell ref from a string representation. Sheet names containing
     * special characters should be delimited and escaped as per normal syntax
     * rules for formulas.
     */
    ...
    public CellReference(String pSheetName, int pRow, int pCol,
                         boolean pAbsRow, boolean pAbsCol) {
        ...
        if (pRow < -1) {
            throw new IllegalArgumentException(
                    "row index may not be negative, but had " + pRow);
        }
        if (pCol < -1) {
            throw new IllegalArgumentException(
                    "column index may not be negative, but had " + pCol);
        }
    ...
    /**
     * Classifies an identifier as either a simple (2D) cell reference or a
     * named range name
     *
     * @return one of the values from <tt>NameType</tt>
     */
    public static NameType classifyCellReference(String str,
        ...
        // If str looks like a cell reference, but is out of (row/col) range, it
        // is a valid
        // named range name
        // This behaviour is a little weird. For example, "IW123" is a valid
        // named range name
        // because the column "IW" is beyond the maximum "IV". Note - this
        // behaviour is version
        // dependent. In BIFF12, "IW123" is not a valid named range name, but in
        // BIFF8 it is.
    ...
    private static NameType validateNamedRangeName(String str,
                                                   SpreadsheetVersion ssVersion) {
    ...
    /**
     * Used to decide whether a name of the form "[A-Z]*[0-9]*" that appears in
     * a formula can be interpreted as a cell reference. Names of that form can
     * be also used for sheets and/or named ranges, and in those circumstances,
     * the question of whether the potential cell reference is valid (in range)
     * becomes important.
     * <p/>
     * Note - that the maximum sheet size varies across Excel versions:
     * <p/>
     * <blockquote>
     * <table border="0" cellpadding="1" cellspacing="0" * summary="Notable cases.">
     * <tr>
     * <th>Version&nbsp;&nbsp;</th>
     * <th>File Format&nbsp;&nbsp;</th>
     * <th>Last Column&nbsp;&nbsp;</th>
     ...
     * </tr>
     * </table>
     * </blockquote> POI currently targets BIFF8 (Excel 97-2003), so the
     * following behaviour can be observed for this method: <blockquote>
     * <table border="0" cellpadding="1" cellspacing="0" * summary="Notable cases.">
     * <tr>
     * <th>
     * Input&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
    ...
    public static boolean cellReferenceIsWithinRange(String colStr,
                                                     String rowStr, SpreadsheetVersion ssVersion) {
        if (!isColumnWithnRange(colStr, ssVersion)) {
            return false;
        }
    ...
    public static boolean isRowWithnRange(String rowStr,
                                          SpreadsheetVersion ssVersion) {
        ...
        if (rowNum == 0) {
            // execution gets here because caller does first pass of
            // discriminating
     ...
     *
     * @param reference is a string that identifies a cell within the sheet or
     *                  workbook reference may not refer to a cell in an external
     *                  workbook reference may be absolute or relative.
     ...
     * markers if they existed in the input reference.
     */
    private static CellRefParts separateRefParts(String reference) {
        ...
        // step over column name chars until first digit (or dollars) for row
        // number.
        for (; loc < length; loc++) {
    ...
    private static String parseSheetName(String reference,
        ...
        for (int i = 1; i < lastQuotePos; i++) { // Note boundaries - skip outer
            // quotes
            ...
            if (i < lastQuotePos) {
                if (reference.charAt(i + 1) == SPECIAL_NAME_DELIMITER) {
                    // two consecutive quotes is the escape sequence for a
                    // single one
            ...
            throw new RuntimeException("Bad sheet name quote escaping: ("
                    + reference + ")");
        }
        return sb.toString();
    }
    ...
    /**
     * @return possibly <code>null</code> if this is a 2D reference. Special
     * characters are not escaped or delimited
     */
     ...
     * <tr>
     * <td>A1</td>
     * <td>Cell reference without sheet</td>
     * </tr>
     * <tr>
     * <td>Sheet1!A1</td>
     * <td>Standard sheet name</td>
     * </tr>
     * <tr>
     * <td>'O''Brien''s Sales'!A1'&nbsp;</td>
     * <td>Sheet name with special characters</td>
     * </tr>
     * </table>
     *
     * @return the text representation of this cell reference as it would appear
     * in a formula.
     */
    ...
    /**
     * Returns the three parts of the cell reference, the Sheet name (or null if
     * none supplied), the 1 based row number, and the A based column letter.
     * This will not include any markers for absolute references, so use
     * {@link #formatAsString()} to properly turn references into strings.
     *
     * @return String array of { sheetName, rowString, colString }
     */
    public String[] getCellRefParts() {
        return new String[]{_sheetName, Integer.toString(_rowIndex + 1),
     ...
     * representations ({@link #formatAsString()} are equal.
     * </p>
     */
    @Override
    public boolean equals(Object o) {
        ...
        return _rowIndex == cr._rowIndex && _colIndex == cr._colIndex
                && _isRowAbs == cr._isRowAbs && _isColAbs == cr._isColAbs;
    }
