...
public class DataSources {
    ...
    public static ChartDataSource<Number> fromNumericCellRange(Sheet sheet,
    ...
    public static ChartDataSource<String> fromStringCellRange(Sheet sheet,
                                                              CellRangeAddress cellRangeAddress) {
    ...
    private static class ArrayDataSource<T> implements ChartDataSource<T> {
        ...
        public boolean isNumeric() {
            Class<?> arrayComponentType = elements.getClass()
                    .getComponentType();
        ...
        public String getFormulaString() {
            throw new UnsupportedOperationException(
                    "Literal data source can not be expressed by reference.");
        }
    }

    private abstract static class AbstractCellRangeDataSource<T> implements
            ChartDataSource<T> {
        ...
        protected AbstractCellRangeDataSource(Sheet sheet,
                                              CellRangeAddress cellRangeAddress) {
            this.sheet = sheet;
            ...
            this.evaluator = sheet.getWorkbook().getCreationHelper()
        ...
        protected CellValue getCellValueAt(int index) {
            if (index < 0 || index >= numOfCells) {
                throw new IndexOutOfBoundsException(
                        "Index must be between 0 and " + (numOfCells - 1)
                                + " (inclusive), given: " + index);
            }
            ...
            return (row == null) ? null : evaluator.evaluate(row
                    .getCell(cellIndex));
        }
    }
}
