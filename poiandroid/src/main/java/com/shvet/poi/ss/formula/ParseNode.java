...
/**
 * Represents a syntactic element from a formula by encapsulating the
 * corresponding <tt>Ptg</tt> token. Each <tt>ParseNode</tt> may have child
 * <tt>ParseNode</tt>s in the case when the wrapped <tt>Ptg</tt> is non-atomic.
 *
 * @author Josh Micich
 */
final class ParseNode {
    ...
    public ParseNode(Ptg token, ParseNode[] children) {
        ...
        if (_isIf) {
            // there will be 2 or 3 extra tAttr tokens according to whether the
            // false param is present
    ...
    private static boolean isIf(Ptg token) {
        if (token instanceof FuncVarPtg) {
            ...
            if (FunctionMetadataRegistry.FUNCTION_NAME_IF
                    .equals(func.getName())) {
                return true;
            }
        }
    ...
    public int getEncodedSize() {
        int result = _token instanceof ArrayPtg ? ArrayPtg.PLAIN_TOKEN_SIZE
                : _token.getSize();
    ...
    /**
     * The IF() function gets marked up with two or three tAttr tokens. Similar
     * logic will be required for CHOOSE() when it is supported
     * <p/>
    ...
    private void collectIfPtgs(TokenCollector temp) {
        ...
        int trueParamSize = temp.sumTokenSizes(ifAttrIndex + 1,
                skipAfterTrueParamIndex);

        AttrPtg attrIf = AttrPtg.createIf(trueParamSize + 4); // distance to
        // start of
        // false
        // parameter/tFuncVar.
        // +4 for
        ...
        if (getChildren().length > 2) {
            ...
            int falseParamSize = temp.sumTokenSizes(
                    skipAfterTrueParamIndex + 1, skipAfterFalseParamIndex);

            AttrPtg attrSkipAfterTrue = AttrPtg
                    .createSkip(falseParamSize + 4 + 4 - 1); // 1 less than
            // distance to
            // end of if
            // FuncVar(size=4).
            // +4 for attr
            // skip before
            AttrPtg attrSkipAfterFalse = AttrPtg.createSkip(4 - 1); // 1 less
            // than
            // distance
            // to end of
            // if
            ...
            AttrPtg attrSkipAfterTrue = AttrPtg.createSkip(4 - 1); // 1 less
            // than
            // distance
            // to end of
            // if
    ...
    private static final class TokenCollector {
        ...
        public void setPlaceholder(int index, Ptg token) {
            if (_ptgs[index] != null) {
                throw new IllegalStateException("Invalid placeholder index ("
                        + index + ")");
            }
            _ptgs[index] = token;
        }
